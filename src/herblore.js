...
class HerbloreRecipe extends CategorizedArtisanRecipe {
  constructor(namespace, data, game, skill) {
    ...
    this.potions = data.potionIDs.map((potionID) => {
      const potion = game.items.potions.getObjectByID(potionID);
      if (potion === undefined)
        throw new Error(
          `Error constructing HerbloreRecipe. Item with id: ${potionID} is not registered.`
        );
      return potion;
    });
  }
  get name() {
    if (this.isModded) {
      ...
      return getLangString("POTION_NAME", this.localID);
    }
  }
  get media() {
...
class Herblore extends ArtisanSkill {
  constructor(namespace, game) {
    super(namespace, "Herblore", game);
    this._media = "assets/media/skills/herblore/herblore.svg";
  ...
  get noCostsMessage() {
    return getLangString("TOASTS", "MATERIALS_REQUIRED_TO_BREW");
  }
  get actionItem() {
    ...
    (_b = data.recipes) === null || _b === void 0
      ...
      : _b.forEach((recipeData) => {
          this.actions.registerObject(
            new HerbloreRecipe(namespace, recipeData, this.game, this)
          );
        });
    if (data.deadlyToxinsItem !== undefined)
      this.deadlyToxinsItem = this.getItemForRegistration(
        data.deadlyToxinsItem
      );
  }
  postDataRegistration() {
    super.postDataRegistration();
    this.actions.forEach((recipe) => {
      recipe.potions.forEach((potion) =>
        this.potionToRecipeMap.set(potion, recipe)
      );
    });
  ...
  getPotionTier(recipe) {
    ...
    for (let i = Herblore.tierMasteryLevels.length - 1; i >= 0; i--) {
      if (masteryLevel >= Herblore.tierMasteryLevels[i]) return i;
    }
    return 0;
  }
  getUncappedDoublingChance(action) {
    let chance = super.getUncappedDoublingChance(action);
    if (this.isPoolTierActive(3)) chance += 10;
    return chance;
  }
  ...
  recordCostPreservationStats(costs) {
    costs.recordBulkItemStat(
      this.game.stats.Herblore,
      HerbloreStats.ItemsPreserved
    );
  }
  ...
  get actionRewards() {
    ...
    const actionEvent = new HerbloreActionEvent(this, this.activeRecipe);
    let potionQuantity = this.actionItemQuantity;
    if (rollPercentage(this.actionDoublingChance)) potionQuantity *= 2;
    potionQuantity *= Math.pow(
      2,
    ...
    if (rollPercentage(extraItemChance)) potionQuantity++;
    rewards.addItem(this.actionItem, potionQuantity);
    actionEvent.productQuantity = potionQuantity;
    this.game.stats.Herblore.add(HerbloreStats.PotionsMade, potionQuantity);
    if (
      rollPercentage(
        this.game.modifiers.increasedChanceRandomPotionHerblore -
          this.game.modifiers.decreasedChanceRandomPotionHerblore
      )
    ) {
      const randomPotion = getRandomArrayElement(this.activeRecipe.potions);
      rewards.addItem(randomPotion, potionQuantity);
      this.game.stats.Herblore.add(HerbloreStats.PotionsMade, potionQuantity);
    }
    if (
      this.deadlyToxinsItem !== undefined &&
      this.activeRecipe.id === "melvorF:Lethal_Toxins_Potion" &&
      this.game.modifiers.increasedDeadlyToxinsFromHerblore > 0
    )
      rewards.addItem(
        this.deadlyToxinsItem,
        this.game.modifiers.increasedDeadlyToxinsFromHerblore
      );
  ...
  postAction() {
    ...
    this.game.stats.Herblore.add(
      HerbloreStats.TimeSpent,
  ...
  setFromOldOffline(offline, idMap) {
    const recipe = this.actions.getObjectByID(
      idMap.herbloreRecipes[offline.action]
    );
    if (recipe !== undefined) {
