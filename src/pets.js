...
class Pet extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    if (data.skillID !== undefined) {
      const skill = game.skills.getObjectByID(data.skillID);
      if (skill === undefined)
        throw new Error(
          `Error constructing Pet with id: ${this.id}, skill with id: ${data.skillID} is not registered.`
        );
      this.skill = skill;
    }
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString("PET_NAME", this.localID);
    }
  }
  get media() {
  ...
  get acquiredBy() {
    if (this.isModded && this._hint !== undefined) {
      ...
      return getLangString(this._langHint.category, this._langHint.id);
    }
  ...
  get description() {
    ...
    if (!this.isModded && this._patreonName !== undefined) {
      descriptionLines.push(
        `<small>${getLangString("MISC_STRING", "1")} ${
          this._patreonName
        }</small>`
      );
    }
    ...
    if (this._langCustomDescription !== undefined) {
      descriptionLines.push(
        getLangString(
          this._langCustomDescription.category,
          this._langCustomDescription.id
        )
      );
    }
...
class PetManager {
  constructor(game) {
    ...
    this.raidStats = {
      ...
      enemyModifiers: new TargetModifiers(),
    };
  ...
  rollForSkillPet(pet, actionInterval, forceSkill) {
    ...
    else chanceForPet = (actionInterval * virtualLevel) / 25000000;
  ...
  unlockPetByID(petID) {
    ...
    if (pet === undefined)
      throw new Error(
        `Error unlocking pet. Pet with id: ${petID} is not registered.`
      );
    this.unlockPet(pet);
  }
  petPet(pet) {
    imageNotify(
      pet.media,
      templateLangString("COMPLETION", "LOG_PETS_Pet", { petName: pet.name }),
      "success"
    );
  ...
  firePetUnlockModal(pet) {
    addModalToQueue({
      title: getLangString("COMPLETION", "LOG_PETS_UNLOCKED"),
      html: `<span class="text-success">${
        pet.name
      }</span><br><small class="text-info">${
        pet.description
  ...
  computeProvidedStats(updatePlayers = true) {
    ...
    this.unlocked.forEach((pet) => {
      if (pet.activeInRaid) {
        this.raidStats.modifiers.addModifiers(pet.modifiers);
        if (pet.enemyModifiers !== undefined)
          this.raidStats.enemyModifiers.addModifiers(pet.enemyModifiers);
      } else {
        this.modifiers.addModifiers(pet.modifiers);
        if (pet.enemyModifiers !== undefined)
          this.enemyModifiers.addModifiers(pet.enemyModifiers);
      }
    });
    if (updatePlayers) {
