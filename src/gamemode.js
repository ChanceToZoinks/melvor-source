"use strict";
const COMBAT_TRIANGLE_IDS = { Standard: 0, Hardcore: 1, InvertedHardcore: 2 };
class Gamemode extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    this._combatTriangle = COMBAT_TRIANGLE_IDS[data.combatTriangle];
    ...
    const page = game.pages.getObjectByID(data.startingPage);
    if (page === undefined)
    ...
    if (data.startingSkills !== undefined)
      this.startingSkills = new Set(
        data.startingSkills.map((skillID) => {
    ...
    this.playerModifiers = game.getPlayerModifiersFromData(
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString("GAMEMODES", `GAMEMODE_NAME_${this.localID}`);
    }
  }
  get description() {
    ...
    if (this.isModded) {
      ...
      return getLangString("GAMEMODES", `GAMEMODE_DESC_${this.localID}`);
    }
  }
  get rules() {
    if (this.isModded) {
      ...
      return this._rules.map((_, i) =>
        getLangString("GAMEMODES", `GAMEMODE_RULES_${this.localID}_${i}`)
      );
    }
  }
  get media() {
  ...
  get combatTriangle() {
    return combatTriangle[this._combatTriangle];
  }
}
class DummyGamemode extends Gamemode {
  get name() {
    if (this.isModded) {
      return `Unregistered Modded Gamemode: ${this.id}`;
    } else {
      return `Full Version Only Gamemode`;
    }
  }
  get media() {
