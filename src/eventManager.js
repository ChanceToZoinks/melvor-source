...
class EventManager {
  ...
  loadEventContainerElements(event) {
    ...
    if (container) container.appendChild(containerElement);
  }
  ...
  updateGameLogo(event) {
    ...
    logo.forEach((l) => {
      l.src = `assets/media/main/${event.logo}`;
    });
  }
...
const EVENTS = [
  {
    ...
    get name() {
      return getLangString("EVENTS", `NAME_${this.id}`);
    },
    get media() {
      return "assets/media/main/sidebar_logo_xmas_2021.svg";
    },
