...
class ItemUpgrade {
  constructor(data, game) {
    ...
    this.rootItems = data.rootItemIDs.map((itemID) => {
      ...
      if (item === undefined)
        throw new UnregisteredConstructionError(this, Item.name, itemID);
      if (
        ...
        throw new Error(
          `Error constructing item upgrade, ${item.id} is set as root item, but is not a cost`
        );
      return item;
    });
    const upgradedItem = game.items.getObjectByID(data.upgradedItemID);
    if (upgradedItem === undefined)
...
class Bank {
  constructor(game, maxTabs = 12, baseSlots = 12) {
    this.game = game;
  ...
  get selectedItemValue() {
    let totalValue = 0;
    this.selectedItems.forEach((bankItem) => {
      totalValue += bankItem.stackValue;
    });
    return totalValue;
  }
  ...
  encode(writer) {
    ...
    writer.writeMap(this.defaultItemTabs, writeNamespaced, (tabID, writer) =>
      writer.writeUint8(tabID)
    );
    writer.writeArray(this.customSortOrder, writeNamespaced);
    writer.writeSet(this.glowingItems, writeNamespaced);
    writer.writeMap(
      this.tabIcons,
      (tabID) => writer.writeUint8(tabID),
      writeNamespaced
    );
    return writer;
  }
  decode(reader, version) {
    ...
    this.defaultItemTabs = reader.getMap(getItem, (reader) =>
      reader.getUint8()
    );
    this.customSortOrder = reader.getArray(getItem);
    this.glowingItems = reader.getSet(getItem);
    this.tabIcons = reader.getMap(
      (reader) => reader.getUint8(),
      readNamespacedReject(this.game.items)
    );
  ...
  convertFromOldFormat(save, idMap) {
    ...
    if (save.bank !== undefined) {
      save.bank.forEach((oldBankItem) => {
        ...
        const tabID = clampValue(oldBankItem.tab, 0, this.maxTabs - 1);
    ...
    if (save.bankTabIcons !== undefined) {
      save.bankTabIcons.forEach((itemID, tabID) => {
        if (tabID >= 0 && tabID < this.maxTabs && itemID !== -1) {
    ...
    if (
      ((_b =
        (_a = save.SETTINGS) === null || _a === void 0 ? void 0 : _a.bank) ===
        null || _b === void 0
        ? void 0
      ...
      save.SETTINGS.bank.defaultItemTab.forEach(({ itemID, tab }) => {
        const item = getItem(itemID);
        const tabID = clampValue(tab, 0, this.maxTabs - 1);
        if (item !== undefined) this.defaultItemTabs.set(item, tabID);
      });
    }
    this.itemsByTab.forEach((tabItems) => {
      tabItems.forEach(
        (bankItem, tabPosition) => (bankItem.tabPosition = tabPosition)
      );
    });
  }
  addDummyItemOnLoad(itemID, quantity) {
    ...
    this.postLoadItems.set(
      item,
      ((_a = this.postLoadItems.get(item)) !== null && _a !== void 0 ? _a : 0) +
        quantity
    );
  }
  onLoad() {
    ...
    if (
      this.game.settings.bankSortOrder === 5 &&
      this.customSortOrder.length === 0
    )
  ...
  onEquipmentChange() {
    if (
      this.selectedBankItem !== undefined &&
      this.selectedBankItem.item instanceof EquipmentItem
    ) {
  ...
  isItemInPosition(item, tab, tabPosition) {
    ...
    return (
      (bankItem === null || bankItem === void 0 ? void 0 : bankItem.item) ===
      item
    );
  }
  ...
  isItemSelected(item) {
    var _a;
    return (
      ((_a = this.selectedBankItem) === null || _a === void 0
        ? void 0
        : _a.item) === item
    );
  }
  ...
  getTabMedia(tabID) {
    ...
    if (firstItemInTab === undefined)
      return cdnMedia("assets/media/skills/combat/food_empty.svg");
  ...
  render() {
    if (this.renderQueue.items.size > 0) {
      ...
      this.renderQueue.items.forEach((item) => {
        ...
        if (existingIcon !== undefined && bankItem !== undefined) {
          existingIcon.updateQuantity(
            bankItem,
            this.game.settings.enableAccessibility
          );
          if (
            ((_a = this.selectedBankItem) === null || _a === void 0
              ? void 0
          ...
          if (
            ((_b = this.selectedBankItem) === null || _b === void 0
              ? void 0
              : _b.item) === item
          ) {
        ...
        if (item === spendMasteryMenu.currentToken) {
          spendMasteryMenu.updateTokenQuantity(
            (_c =
              bankItem === null || bankItem === void 0
                ? void 0
                : bankItem.quantity) !== null && _c !== void 0
              ? _c
              : 0
          );
        }
      ...
      this.renderQueue.bankValue = true;
      this.renderQueue.space = true;
      if (renderRunes) combatMenus.runes.updateCounts();
    ...
    if (this.renderQueue.space) {
      document
        .querySelectorAll(".bank-space-nav")
        .forEach((element) => this.updateSpaceElement(element));
      bankTabMenu.updateBankSpace(this);
      this.renderQueue.space = false;
    }
  }
  updateSpaceElement(element) {
    element.textContent = `${this.occupiedSlots} / ${this.maximumSlots}`;
  ...
  getItemDefaultTab(item) {
    var _a;
    return (_a = this.defaultItemTabs.get(item)) !== null && _a !== void 0
      ? _a
      : 0;
  }
  getItemSalePrice(item, quantity = 1) {
    let salePrice = item.sellsFor * quantity;
    let priceModifier =
      this.game.modifiers.increasedGPFromSales -
      this.game.modifiers.decreasedGPFromSales;
    if (item.type === "Logs" && this.game.woodcutting.isPoolTierActive(2))
      priceModifier += 50;
    salePrice *= 1 + priceModifier / 100;
    return Math.max(Math.floor(salePrice), 0);
  }
  getTabValue(tabID) {
    return this.itemsByTab[tabID].reduce((totalValue, bankItem) => {
      return totalValue + bankItem.stackValue;
    }, 0);
  }
  getBankValue() {
    return this.itemsByTab.reduce((totalValue, _, tabID) => {
      return totalValue + this.getTabValue(tabID);
    }, 0);
  }
  getSnapShot() {
  ...
  checkForClueChasers() {
    const clueChasers = this.game.items.getObjectByID(
  ...
  addItemByID(itemID, quantity, logLost, found, ignoreSpace = false) {
    const item = this.game.items.getObjectByID(itemID);
    if (item === undefined)
      throw new Error(
        `Error adding item to bank by id. Item with id: ${itemID} is not registered.`
      );
    this.addItem(item, quantity, logLost, found, ignoreSpace);
  }
  addItem(item, quantity, logLost, found, ignoreSpace = false, notify = true) {
    var _a;
    if (quantity <= 0)
    ...
    if (bankItem !== undefined) {
      ...
      bankItem = new BankItem(
        ...
        quantity,
        tab,
        this.itemsByTab[tab].length
      );
      this.items.set(item, bankItem);
      this.itemsByTab[tab].push(bankItem);
      if (
        this.game.settings.bankSortOrder === 5 &&
        !this.customSortOrder.includes(item)
      )
        this.storeCustomSortOrder();
      success = true;
    ...
    if (success) {
      ...
      if (notify)
        this.game.combat.notifications.add({ type: "BankFull", args: [] });
      if (logLost) {
        this.lostItems.set(
          item,
          ((_a = this.lostItems.get(item)) !== null && _a !== void 0 ? _a : 0) +
            quantity
        );
      }
    }
  ...
  getQty(item) {
    var _a, _b;
    return (_b =
      (_a = this.items.get(item)) === null || _a === void 0
        ? void 0
        : _a.quantity) !== null && _b !== void 0
      ? _b
      : 0;
  }
  ...
  onItemDoubleClick(item) {
    ...
    if (
      ...
      item.validSlots[0] !== "Summon1"
    ) {
      this.game.combat.player.equipCallback(item, item.validSlots[0], Infinity);
    } else if (item instanceof OpenableItem && !bankItem.locked) {
      this.openItemOnClick(item, Infinity);
    } else if (item instanceof BoneItem && !bankItem.locked) {
  ...
  toggleItemLock(bankItem) {
    ...
    bankTabMenu.updateItemLockBorder(
      bankItem,
      this.game.settings.useDefaultBankBorders
    );
  ...
  reassignBankItemPositions(tabID, startingPosition) {
    const tab = this.itemsByTab[tabID];
    for (
      let tabPosition = startingPosition;
      tabPosition < tab.length;
      tabPosition++
    ) {
  ...
  deselectBankItem() {
    ...
    bankTabMenu.setItemUnselected(
      this.selectedBankItem.item,
      this.itemSelectionMode
    );
    this.selectedBankItem = undefined;
  ...
  toggleItemForSelection(bankItem) {
    if (this.selectedBankItem === bankItem) {
      ...
      if (this.game.settings.defaultToCurrentEquipSet)
        this.game.combat.player.changeEquipToSet(
          this.game.combat.player.selectedEquipmentSet
        );
  ...
  moveSelectedItemsToTab(newTabID) {
    ...
    selectedItemsByTab.forEach((tabArray, tabID) => {
      ...
      let itemsRemoved = 0;
      tabArray.forEach((bankItem) => {
        const removedItems = tabToRemoveFrom.splice(
          bankItem.tabPosition - itemsRemoved,
          1
        );
  ...
  sellAllSelectedItems() {
    ...
    this.selectedItems.forEach((bankItem) => {
      if (!bankItem.locked) {
        itemCount++;
        totalGP += this.getItemSalePrice(bankItem.item, bankItem.quantity);
      }
    });
    this.fireBulkItemSaleConfirmation(totalGP, itemCount, () => {
  ...
  setLockOfAllItemsOnClick(locked) {
    SwalLocale.fire({
      title: getLangString("BANK_STRING", "52"),
  ...
  fireBulkItemSaleConfirmation(totalGP, count, onConfirm) {
    SwalLocale.fire({
      title: templateString(getLangString("MENU_TEXT", "SELL_NUM_ITEMS"), {
        num: `${numberWithCommas(count)}`,
      }),
      html: `<span class="text-dark"><img class="skill-icon-xxs mr-1" src="${cdnMedia(
        "assets/media/main/coins.svg"
      )}"><span id="bank-sell-item-summary-gp">${numberWithCommas(
        totalGP
      )} ${getLangString("MENU_TEXT", "GP")}</span>`,
      imageUrl: cdnMedia("assets/media/main/coins.svg"),
      imageWidth: 64,
      imageHeight: 64,
      imageAlt: `${numberWithCommas(totalGP)} ${getLangString(
        "MENU_TEXT",
        "GP"
      )}`,
      showCancelButton: true,
      confirmButtonText: getLangString("BANK_STRING", "22"),
    }).then((result) => {
      if (result.value) {
        onConfirm();
      }
  ...
  sortButtonOnClick() {
    ...
    switch (this.game.settings.bankSortOrder) {
      case 0:
        ...
        break;
      case 1:
        sortFunction = (a, b) => b.itemSellValue - a.itemSellValue;
        break;
      case 2:
        sortFunction = (a, b) => a.itemSellValue - b.itemSellValue;
        break;
      case 3:
        sortFunction = (a, b) => b.stackValue - a.stackValue;
        break;
      case 4:
        sortFunction = (a, b) => a.stackValue - b.stackValue;
        break;
      case 5:
      ...
      default:
        throw new Error(
          `Error sorting bank, sort order setting: ${this.game.settings.bankSortOrder} is invalid.`
        );
    }
    this.itemsByTab.forEach((tab, tabID) => {
      const tabOrder = tab
        .sort(sortFunction)
        .map((bankItem) => bankItem.item.id);
  ...
  sellItemOnClick(item, quantity) {
    ...
    if (
      ...
      this.game.tutorial.bannedItemSales.has(item)
    ) {
      SwalLocale.fire({
        title: getLangString("MENU_TEXT", "ITEM_SALE_LOCKED"),
        html: `<h5 class="font-w400 text-combat-smoke font-size-sm"><img class="skill-icon-xs mr-1" src="${
          item.media
        }"> ${getLangString("MENU_TEXT", "ITEM_NEEDED_TUTORIAL")}</h5>`,
        icon: "warning",
      });
      return;
    }
    ...
    if (this.game.settings.showSaleConfirmations) {
      SwalLocale.fire({
        title: getLangString("BANK_STRING", "23"),
  ...
  buryItemOnClick(item, quantity) {
    ...
    this.game.combat.player.addPrayerPoints(item.prayerPoints * quantity);
    this.game.combat.player.render();
    notifyPlayer(
      this.game.prayer,
      templateLangString("MENU_TEXT", "PRAYER_POINTS", {
  ...
  openItemOnClick(item, quantity) {
    ...
    if (
      ...
      this.getQty(item.keyItem.item) < item.keyItem.quantity
    )
      return;
    quantity = Math.min(bankItem.quantity, quantity);
    let interval = 500;
    if (
      item.id === "melvorD:Chest_of_Witwix" &&
      this.game.characterName === "witwix"
    )
      interval = 5000;
    SwalLocale.fire({
      html: `<span id="item-open-contents" class="text-dark"><p>${templateLangString(
        "BANK_STRING",
        "41",
        { itemName: item.name, qty: numberWithCommas(quantity) }
      )}</p><div class="skill-icon-md spinner-border spinner-border-sm text-danger" role="status"></span>`,
      imageUrl: item.media,
      imageWidth: 64,
      imageHeight: 64,
      imageAlt: item.name,
      ...
      if (count < addedArray.length) {
        $("#item-open-contents").append(
          `<small class="js-animation-object animated fadeInLeft text-dark">${numberWithCommas(
            addedArray[count][1]
          )} <img class="skill-icon-xs mr-2" src="${
            addedArray[count][0].media
          }">${addedArray[count][0].name}<br></small>`
        );
        count++;
      } else {
  ...
  claimItemOnClick(item, quantity) {
    ...
    if (item.modifiers.masteryToken !== undefined) {
      ...
      if (!(skill instanceof SkillWithMastery) || !skill.hasMastery)
        throw new Error(
          `Error claiming token. Mastery Token skill does not have mastery.`
        );
      const xpPerToken = Math.floor(
        (skill.baseMasteryPoolCap * tokenPercent) / 100
      );
      ...
      if (quantity === tokensToFillPool)
        notifyPlayer(
          skill,
          templateLangString("TOASTS", "MAX_POOL_TOKENS", {
      ...
      notifyPlayer(
        skill,
        templateLangString("TOASTS", "POOL_XP_GRANTED", {
          xp: numberWithCommas(totalXpToAdd),
        }),
        "success"
      );
      this.removeItemQuantity(item, quantity, true);
      $("#mastery-pool-spend-token-qty").text(
        numberWithCommas(this.getQty(item))
      );
    } else {
      this.removeItemQuantity(item, quantity, true);
  ...
  getMaxUpgradeQuantity(upgrade) {
    ...
    if (upgrade.gpCost > 0) {
      maxUpgrades = Math.min(
        maxUpgrades,
        Math.floor(this.game.gp.amount / upgrade.gpCost)
      );
    }
    if (upgrade.scCost > 0) {
      maxUpgrades = Math.min(
        maxUpgrades,
        Math.floor(this.game.slayerCoins.amount / upgrade.scCost)
      );
    }
    upgrade.itemCosts.forEach(({ item, quantity }) => {
      maxUpgrades = Math.min(
        maxUpgrades,
        Math.floor(this.getQty(item) / quantity)
      );
    });
    return maxUpgrades;
  }
  checkUpgradePotionRequirement(upgrade) {
    ...
    if (upgrade.upgradedItem instanceof PotionItem) {
      const recipe = this.game.herblore.getRecipeForPotion(
        upgrade.upgradedItem
      );
      requirementsMet =
        recipe !== undefined &&
        this.game.herblore.getMasteryLevel(recipe) >=
          Herblore.tierMasteryLevels[upgrade.upgradedItem.tier];
    }
  ...
  fireItemUpgradeModal(upgrade, rootItem) {
    ...
    $("#item-view-name-upgrade").html(upgradedItem.name);
    $("#item-view-img-upgrade").attr("src", upgradedItem.media);
    if (upgradedItem instanceof EquipmentItem) {
      ...
      if (upgradedItem instanceof WeaponItem) {
        $("#item-view-interval-upgrade-cont").removeClass("d-none");
      } else {
        $("#item-view-interval-upgrade-cont").addClass("d-none");
      }
      $("#item-view-interval-upgrade").text(
        templateLangString("MENU_TEXT", "SECONDS_SHORT", {
          seconds: formatFixed(statComparison.attackSpeed / 1000, 2),
        })
      );
      $("#item-view-stab-bonus-upgrade").text(statComparison.stabAttackBonus);
      $("#item-view-slash-bonus-upgrade").text(statComparison.slashAttackBonus);
      $("#item-view-block-bonus-upgrade").text(statComparison.blockAttackBonus);
      $("#item-view-strength-bonus-upgrade").text(
        statComparison.meleeStrengthBonus
      );
      $("#item-view-defence-bonus-upgrade").text(
        statComparison.meleeDefenceBonus
      );
      $("#item-view-ranged-attack-bonus-upgrade").text(
        statComparison.rangedAttackBonus
      );
      $("#item-view-ranged-strength-bonus-upgrade").text(
        statComparison.rangedStrengthBonus
      );
      $("#item-view-ranged-defence-bonus-upgrade").text(
        statComparison.rangedDefenceBonus
      );
      $("#item-view-magic-attack-bonus-upgrade").text(
        statComparison.magicAttackBonus
      );
      $("#item-view-magic-damage-bonus-upgrade").text(
        formatPercent(statComparison.magicDamageBonus)
      );
      $("#item-view-magic-defence-bonus-upgrade").text(
        statComparison.magicDefenceBonus
      );
      $("#item-view-damage-reduction-upgrade").text(
        formatPercent(statComparison.damageReduction)
      );
      if (rootItem instanceof EquipmentItem)
        statComparison.subtractStats(rootItem.equipmentStats);
      const formatDiff = (id, diff, percent = false, interval = false) => {
        const diffElem = $(id);
        let diffText = "";
        if (diff !== 0) {
          if (interval)
          ...
          else if (percent) diffText = formatPercent(diff);
          else diffText = `${diff}`;
        }
        if (diff > 0) {
          diffText = `+${diffText}`;
        }
        const isPositive = diff > 0 === !interval;
        diffElem.text(diffText);
        if (isPositive) {
          diffElem.removeClass("text-danger");
          diffElem.addClass("text-success");
        } else {
          diffElem.removeClass("text-success");
          diffElem.addClass("text-danger");
        }
      };
      formatDiff(
        "#item-view-interval-upgrade-diff",
        statComparison.attackSpeed,
        false,
        true
      );
      formatDiff(
        "#item-view-slash-bonus-upgrade-diff",
        statComparison.slashAttackBonus
      );
      formatDiff(
        "#item-view-stab-bonus-upgrade-diff",
        statComparison.stabAttackBonus
      );
      formatDiff(
        "#item-view-block-bonus-upgrade-diff",
        statComparison.blockAttackBonus
      );
      formatDiff(
        "#item-view-strength-bonus-upgrade-diff",
        statComparison.meleeStrengthBonus
      );
      formatDiff(
        "#item-view-defence-bonus-upgrade-diff",
        statComparison.meleeDefenceBonus
      );
      formatDiff(
        "#item-view-ranged-attack-bonus-upgrade-diff",
        statComparison.rangedAttackBonus
      );
      formatDiff(
        "#item-view-ranged-strength-bonus-upgrade-diff",
        statComparison.rangedStrengthBonus
      );
      formatDiff(
        "#item-view-ranged-defence-bonus-upgrade-diff",
        statComparison.rangedDefenceBonus
      );
      formatDiff(
        "#item-view-magic-attack-bonus-upgrade-diff",
        statComparison.magicAttackBonus
      );
      formatDiff(
        "#item-view-magic-damage-bonus-upgrade-diff",
        statComparison.magicDamageBonus,
        true
      );
      formatDiff(
        "#item-view-magic-defence-bonus-upgrade-diff",
        statComparison.magicDefenceBonus
      );
      formatDiff(
        "#item-view-damage-reduction-upgrade-diff",
        statComparison.damageReduction,
        true
      );
      $("#modal-upgrade-item-stats").removeClass("d-none");
      $("#modal-upgrade-item-stats-info").addClass("d-none");
      if (upgradedItem.specialAttacks.length > 0) {
        const attackHTML = upgradedItem.specialAttacks
          .map((attack, id) => {
            let chance = attack.defaultChance;
            if (upgradedItem.overrideSpecialChances !== undefined)
              chance = upgradedItem.overrideSpecialChances[id];
          ...
          .join("");
        $("#item-upgrade-special-attack-list").html(attackHTML);
        $("#item-upgrade-special-attack").removeClass("d-none");
      } else {
        $("#item-upgrade-special-attack").addClass("d-none");
      }
      let levelReqHTML = "";
      const levelReqs = upgradedItem.equipRequirements.filter(
      ...
      if (levelReqs.length > 0) {
        ...
        $("#item-upgrade-description-levels").removeClass("d-none");
      } else $("#item-upgrade-description-levels").addClass("d-none");
      $("#item-upgrade-description-levels").html(levelReqHTML);
    } else {
      $("#modal-upgrade-item-stats").addClass("d-none");
      $("#modal-upgrade-item-stats-info").removeClass("d-none");
      $("#item-upgrade-special-attack-list").text("");
      $("#item-upgrade-special-attack").addClass("d-none");
      $("#item-upgrade-description-levels").addClass("d-none");
    }
    const potionRequirementMet = this.checkUpgradePotionRequirement(upgrade);
    if (upgradedItem instanceof PotionItem) {
      const textClass = potionRequirementMet ? "text-success" : "text-danger";
      $("#item-upgrade-cost-mastery-text").text(
        Herblore.tierMasteryLevels[upgradedItem.tier]
      );
      $("#item-upgrade-cost-mastery-text").removeClass("text-danger");
      $("#item-upgrade-cost-mastery-text").removeClass("text-success");
      $("#item-upgrade-cost-mastery-text").addClass(textClass);
      $("#item-upgrade-cost-mastery").removeClass("d-none");
    } else $("#item-upgrade-cost-mastery").addClass("d-none");
    if (upgradedItem.hasDescription)
      $("#item-upgrade-description").html(upgradedItem.description);
    else $("#item-upgrade-description").html("");
    if (upgrade.gpCost > 0) {
      $("#item-upgrade-cost-gp").text(formatNumber(upgrade.gpCost));
      $("#item-upgrade-cost-gp").removeClass("text-danger");
      $("#item-upgrade-cost-gp").removeClass("text-success");
      $("#item-upgrade-cost-gp").addClass(
        this.game.gp.canAfford(upgrade.gpCost) ? "text-success" : "text-danger"
      );
      $("#item-upgrade-cost-gp").removeClass("d-none");
      $("#item-upgrade-cost-gp-img").removeClass("d-none");
    } else {
      $("#item-upgrade-cost-gp").addClass("d-none");
      $("#item-upgrade-cost-gp-img").addClass("d-none");
    }
    if (upgrade.scCost > 0) {
      $("#item-upgrade-cost-sc").text(formatNumber(upgrade.scCost));
      $("#item-upgrade-cost-sc").removeClass("text-danger");
      $("#item-upgrade-cost-sc").removeClass("text-success");
      $("#item-upgrade-cost-sc").addClass(
        this.game.slayerCoins.canAfford(upgrade.scCost)
          ? "text-success"
          : "text-danger"
      );
      $("#item-upgrade-cost-sc").removeClass("d-none");
      $("#item-upgrade-cost-sc-img").removeClass("d-none");
    } else {
      $("#item-upgrade-cost-sc").addClass("d-none");
      $("#item-upgrade-cost-sc-img").addClass("d-none");
    }
    $("#item-upgrade-cost").html("");
    upgrade.itemCosts.forEach(({ item, quantity }) => {
      const owned = this.getQty(item);
      $("#item-upgrade-cost").append(
        `<img class="skill-icon-sm js-tooltip-enabled mr-1" src="${
          item.media
        }" data-toggle="tooltip" data-html="true" data-placement="bottom" title="" data-original-title="${
          item.name
        }"><span class="font-w600 ${
          owned >= quantity ? "text-success" : "text-danger"
        }">${quantity}</span>`
      );
    });
    const upgradeButtons = [
      { button: document.getElementById("view-upgrade-btn"), quantity: 1 },
      { button: document.getElementById("view-upgrade-btn-10"), quantity: 10 },
      {
        button: document.getElementById("view-upgrade-btn-100"),
        quantity: 100,
      },
      {
        button: document.getElementById("view-upgrade-btn-1000"),
        quantity: 1000,
      },
      {
        button: document.getElementById("view-upgrade-btn-all"),
        quantity: Infinity,
      },
    ];
    const maxUpgrades = this.getMaxUpgradeQuantity(upgrade);
    upgradeButtons.forEach(({ button, quantity }) => {
      if (
        potionRequirementMet &&
        (maxUpgrades >= quantity || (quantity === Infinity && maxUpgrades > 0))
      ) {
        showElement(button);
        button.onclick = () => this.upgradeItemOnClick(upgrade, quantity);
      } else {
        hideElement(button);
      }
    });
  ...
  upgradeItemOnClick(upgrade, upgradeQuantity) {
    ...
    upgradeQuantity = Math.min(
      this.getMaxUpgradeQuantity(upgrade),
      upgradeQuantity
    );
    const costs = new Costs(this.game);
    costs.addGP(upgrade.gpCost * upgradeQuantity);
    costs.addSlayerCoins(upgrade.scCost * upgradeQuantity);
    upgrade.itemCosts.forEach(({ item, quantity }) => {
      costs.addItem(item, quantity * upgradeQuantity);
    });
    if (this.addItem(upgrade.upgradedItem, upgradeQuantity, false, true)) {
      costs.consumeCosts();
    }
  }
  ...
  findAFriendOnClick(cracker) {
    SwalLocale.fire({
      ...
      imageUrl: cdnMedia("assets/media/main/question.svg"),
      imageWidth: 64,
      imageHeight: 64,
      imageAlt: "Offline",
    ...
    const success = (data) => {
      if (data === "true") {
        SwalLocale.fire({
          title: ":D",
          html: getLangString("BANK_STRING", "FOUND_A_FRIEND"),
          imageUrl: cdnMedia("assets/media/bank/friendship_bracelet.png"),
          imageWidth: 64,
          imageHeight: 64,
          imageAlt: getLangString("BANK_STRING", "YAY_FRIEND"),
        });
        this.addItemByID("melvorD:Friendship_Bracelet", 1, false, true);
      } else {
        SwalLocale.fire({
          title: ":(",
          html: getLangString("MENU_TEXT", "NO_FRIENDS_FOUND"),
        });
      }
    };
    setTimeout(() => {
  ...
  onBankSearchChange(query) {
    if (query === "") {
      ...
      bankTabMenu.updateForSearchResult(foundItems, foundTabs);
    ...
    this.itemsByTab.forEach((tab, tabID) => {
      tab.forEach((bankItem, tabPos) => {
        if (bankItem.tab !== tabID)
          console.warn(
            `${bankItem.item.name} has a mismatched tab. Real: ${tabID}, Item: ${bankItem.tab}`
          );
...
class BankItem {
  ...
  get itemSellValue() {
    return this.bank.getItemSalePrice(this.item);
  }
  get stackValue() {
    return this.bank.getItemSalePrice(this.item, this.quantity);
  }
  get locked() {
