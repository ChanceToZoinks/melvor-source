...
class TutorialStageDisplayElement extends HTMLElement {
  constructor() {
    ...
    this.stageStatus = getElementFromFragment(
      this._content,
      "stage-status",
      "span"
    );
    this.stageName = getElementFromFragment(
      this._content,
      "stage-name",
      "span"
    );
    this.stageDescription = getElementFromFragment(
      this._content,
      "stage-description",
      "span"
    );
    this.taskCompletion = getElementFromFragment(
      this._content,
      "task-completion",
      "h2"
    );
    ...
    this.taskContainer = getElementFromFragment(
      this._content,
      "task-container",
      "div"
    );
    this.claimRewardsButton = getElementFromFragment(
      this._content,
  ...
  setStage(stage, tutorial) {
    ...
    this.pageLink.onclick = () =>
      changePage(stage.taskPage, -1, undefined, false, false);
    ...
    if (stage.rewards.gp > 0) {
      addRewardSpan(cdnMedia("assets/media/main/coins.svg"), stage.rewards.gp);
    }
    if (stage.rewards.slayerCoins > 0) {
      addRewardSpan(
        cdnMedia("assets/media/main/slayer_coins.svg"),
        stage.rewards.slayerCoins
      );
    }
  ...
  updateTasks(stage) {
    if (this._displayedStage !== stage) {
      ...
      stage.tasks.forEach((task) => {
        ...
        container.append(
          newTask.taskIcon,
          ...templateStringWithNodes(description, newTask, {}, false)
        );
  ...
  getTaskCount(task) {
    return `${
      task.complete ? task.eventCount : task.eventCount - task.progress
    }`;
  }
  getTaskIconClass(task) {
    return `fa mr-2 ${
      task.complete ? "fa-check text-success" : "fa-exclamation text-warning"
    }`;
  }
  updateTaskCompletion(completedTasks, totalTasks) {
    if (completedTasks >= totalTasks) {
      this.taskCompletion.innerHTML = `<img class="resize-40" src="${cdnMedia(
        "assets/media/main/tick.png"
      )}">`;
    } else {
  ...
  setActive() {
    ...
    this.stageStatus.textContent = getLangString("MENU_TEXT", "ACTIVE");
  ...
  setCompleted() {
    ...
    this.stageStatus.textContent = getLangString(
      "MISC_STRING",
      "READY_TO_CLAIM"
    );
    this.stageStatus.classList.replace("bg-success", "bg-info");
  ...
  setClaimed() {
    this.setHeaderClass("bg-tutorial-claimed");
    this.stageStatus.textContent = getLangString("MISC_STRING", "CLAIMED");
...
window.customElements.define(
  "tutorial-stage-display",
  TutorialStageDisplayElement
);
class TutorialProgressDisplayElement extends HTMLElement {
  constructor() {
    ...
    this.skipButton = getElementFromFragment(
      this._content,
      "skip-button",
      "button"
    );
    this.stagesCompleted = getElementFromFragment(
      this._content,
      "stages-completed",
      "span"
    );
    this.totalStages = getElementFromFragment(
      this._content,
      "total-stages",
      "span"
    );
...
window.customElements.define(
  ...
  TutorialProgressDisplayElement
);
