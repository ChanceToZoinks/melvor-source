...
function rollForOffItem(baseChance) {
  return rollPercentage(
    baseChance * (1 + game.modifiers.increasedOffItemChance / 100)
  );
}
function generateGaussianNumber($mean, $stdDev) {
  ...
  const $randNumNorm =
    ...
    Math.cos(
      2.0 * 3.141592653589793238462643383279502884197169399375 * $randNumB
    );
...
function toggleDangerSuccess(elem, success) {
  if (success) elem.classList.replace("text-danger", "text-success");
  else elem.classList.replace("text-success", "text-danger");
}
function removeElementID(elem) {
  elem.removeAttribute("id");
}
const levelUnlockSum = (skill) => (previous, current) => {
  ...
  if (game.settings.showItemNotifications) {
    ...
    itemNotifyTimer = setTimeout(function () {
      for (let i = 0; i < itemNotifyToProcess.length; i++)
        processItemNotify(
          itemNotifyToProcess[i].item,
          itemNotifyToProcess[i].quantity
        );
      itemNotifyToProcess = [];
    }, 50);
  }
...
function processItemNotify(item, quantity) {
  ...
  if (game.settings.showQuantityInItemNotifications)
    qtyInBank =
      " <span class='ml-2'>(" +
      numberWithCommas(game.bank.getQty(item)) +
      ")</span> ";
  imageNotify(
    item.media,
    `+${numberWithCommas(quantity)}${qtyInBank}${access}`,
    "success"
  );
}
function stunNotify(damage) {
  fireBottomToast(
    `<div class="text-center"><img class="notification-img" src="${cdnMedia(
      "assets/media/skills/thieving/thieving.svg"
    )}"><span class="badge badge-warning">${getLangString(
      "TOASTS",
      "STUNNED"
    )} </span> <span class="badge badge-danger"> ${templateLangString(
      "TOASTS",
      "MINUS_HP",
      { damage: `${damage}` }
    )}</span></div>`
...
function bankFullNotify() {
  imageNotify(
    cdnMedia("assets/media/main/bank_header.svg"),
    getLangString("TOASTS", "FULL_BANK"),
    "danger"
  );
}
...
function notifyPlayer(skill, message, messageTheme = "success") {
  let img = "";
  if (skill === -1) img = CDNDIR + "assets/media/main/xmas_present.svg";
  else img = skill.media;
  imageNotify(img, message, messageTheme);
}
function notifyItemCharges(item) {
  imageNotify(item.media, getLangString("TOASTS", "GLOVES_DEGRADED"), "danger");
}
function tutorialNotify() {
  fireTopToast(
    `<div class="block block-rounded-double bg-dark p-2">
  <div class="media d-flex align-items-center push">
    <div class="mr-2"><img class="skill-icon-md" src="${cdnMedia(
      "assets/media/main/tutorial_island.svg"
    )}"></div>
    <div class="media-body text-left">
...
function currencyNotify(media, amount) {
  imageNotify(
    media,
...
function notify99ItemMastery(action) {
  ...
  <div class="media d-flex align-items-center push">
    ...
    <div class="media-body text-left">
      ...
      <div class="font-size-sm">
        <img class="skill-icon-xs mr-1" src="${
          action.media
        }">${templateLangString("COMPLETION", "MASTERY_LEVEL_99", {
...
function notifyCompletionTotH() {
  let html = `<h5 class="font-w600 text-success">${getLangString(
    "MENU_TEXT",
    "ACHIEVED_COMPLETION_TOTH"
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString(
    "MENU_TEXT",
    "COMPLETION_MESSAGE_TOTH"
  )}</h5>`;
  const stat = game.stats.General.get(GeneralStats.AccountCreationDate);
  if (stat === 0) return;
  html += `<h5 class="font-w400 font-size-sm">This achievement took you ${formatAsTimePeriod(
...
function notifyCompletionEverything() {
  let html = `<h5 class="font-w600 text-success">${getLangString(
    "MENU_TEXT",
    "ACHIEVED_COMPLETION_ALL"
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString(
    "MENU_TEXT",
    "COMPLETION_MESSAGE_ALL"
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString(
    "MENU_TEXT",
    "COMPLETION_BUY_CAPE_TOTH"
  )}</h5>`;
  const stat = game.stats.General.get(GeneralStats.AccountCreationDate);
  if (stat === 0) return;
  html += `<h5 class="font-w400 font-size-sm">This achievement took you ${formatAsTimePeriod(
...
class ExperienceCalculator {
  ...
  xpToLevel(xp) {
    if (xp <= 0) return 1;
    let levelEstimate =
      Math.floor(7 * Math.log2(this.estConstA + this.estConstB * xp)) - 1;
    if (xp > this.level_to_xp(levelEstimate + 1)) {
      levelEstimate++;
    }
...
function getItemBaseStatsBreakdown(item) {
  let html = "";
  item.equipmentStats.forEach((statPair) => {
    const isPositive = statPair.value > 0;
    const statDesc = Equipment.getEquipStatDescription(
      statPair.key,
      statPair.value
    );
...
function cdnMedia(media) {
  if (useCDN) return `${CDNDIR}${media}`;
  else return media;
}
function compareNameValuePairs(currentPairs, oldPairs) {
  ...
  curPairMap.forEach((curVal, name) => {
    ...
    if (oldVal === undefined)
      ...
      console.log(
        `Value of ${name} is different. Current: ${curVal} Old: ${oldVal}.`
      );
  });
...
function templateLangString(category, identifier, templateData) {
  return templateString(getLangString(category, identifier), templateData);
}
...
function getUnlockedAtNodes(skill, level) {
  const unlockText = templateLangString("MENU_TEXT", "UNLOCKED_AT", {
    skillImage: "$$$",
    level: `${level}`,
  });
  ...
  return [
    document.createTextNode(splitText[0]),
    skillImage,
    document.createTextNode(splitText[1]),
  ];
}
function templateLangStringWithNodes(
  category,
  id,
  nodeData,
  textData,
  ...
  return templateStringWithNodes(
    getLangString(category, id),
    nodeData,
    textData,
    clone
  );
...
function getMSAsTime(time) {
  ...
  const milliseconds = Math.floor(
    (time % msPerUnit.seconds) / msPerUnit.milliseconds
  );
...
function formatAsTimePeriod(timeInMs) {
  ...
  if (time.years > 0) {
    if (time.years === 1) {
      timePeriods.push(getLangString("TIME_UNIT", "year"));
    } else {
      timePeriods.push(
        templateLangString("TIME_UNIT", "years", { years: `${time.years}` })
      );
    }
  }
  if (time.days > 0) {
    if (time.days === 1) {
      timePeriods.push(getLangString("TIME_UNIT", "day"));
    } else {
      timePeriods.push(
        templateLangString("TIME_UNIT", "days", { days: `${time.days}` })
      );
    }
  }
  if (time.hours > 0) {
    if (time.hours === 1) {
      timePeriods.push(getLangString("TIME_UNIT", "hour"));
    } else {
      timePeriods.push(
        templateLangString("TIME_UNIT", "hours", { hours: `${time.hours}` })
      );
    }
  }
  if (time.minutes > 0) {
    if (time.minutes === 1) {
      timePeriods.push(getLangString("TIME_UNIT", "minute"));
    } else {
      timePeriods.push(
        templateLangString("TIME_UNIT", "minutes", {
  ...
  if (time.seconds === 1) {
    timePeriods.push(getLangString("TIME_UNIT", "second"));
  } else {
    timePeriods.push(
      templateLangString("TIME_UNIT", "seconds", { seconds: `${time.seconds}` })
    );
  }
  ...
  return listFormat.format(
    timePeriods.map((part) => part.replace(" ", "\xa0"))
  );
}
function formatAsShorthandTimePeriod(timeInMs) {
  const time = getMSAsTime(timeInMs);
  const timePeriods = [];
  if (time.years > 0) {
...
function getElementFromFragment(fragment, elementID, tagName) {
  const elem = fragment.getElementById(elementID);
  if (elem === null)
    throw new Error(`Fragment does not contain element with id: ${elementID}`);
  if (elem.tagName !== tagName.toUpperCase())
    throw new Error("Element does not have the correct tag name");
  elem.removeAttribute("id");
  return elem;
}
...
function mapOrder(array, order, key) {
  ...
  array.sort(sortByOrder(order, key));
  return array;
}
...
const unregisteredMessage = (type) =>
  `Error converting old save data. ${type} is not registered.`;
class UnregisteredConstructionError extends Error {
  constructor(object, unregisteredName, id) {
    super(
      `Error constructing ${object.constructor.name}. ${unregisteredName} with id: ${id} is not registered.`
    );
...
class UnregisteredDataModError extends Error {
  constructor(unregisteredName, id) {
    super(
      `Error applying data modification to ${unregisteredName} with id: ${id}. Object is not registered.`
    );
    this.name = this.constructor.name;
  }
}
...
class DropTable {
  ...
  get averageDropValue() {
    return (
      this.drops.reduce(
        (totalValue, { item, weight, minQuantity, maxQuantity }) => {
          return (
            totalValue +
            (weight * item.sellsFor * (minQuantity + maxQuantity)) / 2
          );
        },
        0
      ) / this.totalWeight
  ...
  registerDrops(game, data) {
    data.forEach((data) => {
      const item = game.items.getObjectByID(data.itemID);
      if (item === undefined)
        throw new Error(
          `Error registering drop table data, item with ID: ${data.itemID} is not registered`
        );
  ...
  getDrop() {
    ...
    let itemWeight = 0;
    const lootIndex = this.drops.findIndex((drop) => {
      itemWeight += drop.weight;
      return dropRoll < itemWeight;
    });
    const drop = this.drops[lootIndex];
...
function generateComponentClass(templateID, tagName, className) {
  ...
  idElements.forEach((element) => {
    ...
    getters.push(
      `    this.${propertyName} = getElementFromFragment(this._content, '${id}','${tagName}');`
    );
  });
...
function generateModifierDataSchema() {
  const combatSchema = { type: "object", properties: {} };
  const allCombatKeys = new Set();
  Object.entries(new CombatModifiers()).forEach(([key]) => {
    combatSchema.properties[key] = {
      type: "number",
      minimum: 0,
      description: modifierData[key].description,
    };
    allCombatKeys.add(key);
  });
  const standardKeys = [];
  const skillKeys = [];
  const standardSkillSchema = {
    type: "object",
    allOf: [{ $ref: "#/$defs/CombatModifierBase" }],
    properties: {},
    unevaluatedProperties: false,
  };
  Object.entries(modifierData).forEach((entry) => {
    if (isSkillEntry(entry)) {
      standardSkillSchema.properties[entry[0]] = {
        description: entry[1].description,
        allOf: [{ $ref: "#/$defs/SkillModDataArray" }],
      };
      skillKeys.push(entry[0]);
    } else if (!allCombatKeys.has(entry[0])) {
      standardSkillSchema.properties[entry[0]] = {
        description: entry[1].description,
        type: "number",
        minimum: 0,
      };
      standardKeys.push(entry[0]);
    }
  });
  return JSON.stringify({
    CombatModifierBase: combatSchema,
    CombatModifierData: {
      type: "object",
      allOf: [{ $ref: "#/$defs/CombatModifierBase" }],
      unevaluatedProperties: false,
    },
    PlayerModifierData: standardSkillSchema,
    CombatModifierKey: { type: "string", enum: [...allCombatKeys] },
    StandardModifierKeys: {
      oneOf: [
        { $ref: "#/$defs/CombatModifierKey" },
        { type: "string", enum: standardKeys },
      ],
    },
    SkillModifierKeys: { type: "string", enum: skillKeys },
  });
}
...
(function (TownshipResourceTypeID) {
  ...
  TownshipResourceTypeID[(TownshipResourceTypeID["Product"] = 2)] = "Product";
})(TownshipResourceTypeID || (TownshipResourceTypeID = {}));
var BuildingTypeID;
(function (BuildingTypeID) {
  BuildingTypeID[(BuildingTypeID["House"] = 0)] = "House";
  BuildingTypeID[(BuildingTypeID["Gathering"] = 1)] = "Gathering";
  BuildingTypeID[(BuildingTypeID["Production"] = 2)] = "Production";
  BuildingTypeID[(BuildingTypeID["Storage"] = 3)] = "Storage";
  BuildingTypeID[(BuildingTypeID["Education"] = 4)] = "Education";
  BuildingTypeID[(BuildingTypeID["Other"] = 5)] = "Other";
})(BuildingTypeID || (BuildingTypeID = {}));
var CitizenSource;
(function (CitizenSource) {
  CitizenSource[(CitizenSource["Birth"] = 0)] = "Birth";
  CitizenSource[(CitizenSource["Migration"] = 1)] = "Migration";
})(CitizenSource || (CitizenSource = {}));
