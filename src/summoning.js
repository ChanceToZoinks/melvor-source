...
class SummoningRecipe extends CategorizedArtisanRecipe {
  constructor(namespace, data, game, skill) {
    ...
    const product = game.items.equipment.getObjectByID(data.productID);
    if (product === undefined)
      throw new Error(
        `Error constructing SummoningRecipe with id: ${this.id}. Product with id: ${data.productID} is not registered.`
      );
    this.product = product;
    this.baseQuantity = data.baseQuantity;
    this.nonShardItemCosts = data.nonShardItemCosts.map((itemID) => {
      const item = game.items.getObjectByID(itemID);
      if (item === undefined)
        throw new Error(
          `Error constructing SummoningRecipe with id: ${this.id}. Non shard item cost with id: ${itemID} is not registered.`
        );
      return item;
    });
    this.tier = data.tier;
    this.skills = data.skillIDs.map((skillID) => {
      ...
      if (skill === undefined)
        throw new Error(
          `Error constructing SummoningRecipe with id: ${this.id}. Skill with id: ${skillID} is not registered.`
        );
      return skill;
    });
  }
...
class DummySummoningRecipe extends SummoningRecipe {
  constructor(namespace, id, game) {
    super(
      ...
      {
        id,
        markMedia: "assets/media/main/question.svg",
        nonShardItemCosts: [],
        tier: 1,
        skillIDs: [],
        productID: game.emptyEquipmentItem.id,
        baseQuantity: 0,
        categoryID: "melvorD:Marks",
        itemCosts: [],
        gpCost: -1,
        scCost: -1,
        baseExperience: -1,
  ...
  constructor(data, game, summoning) {
    ...
    if (data.conditionalModifiers !== undefined)
      this.conditionalModifiers = data.conditionalModifiers.map(
        (conditionalData) => new ConditionalModifier(conditionalData, game)
      );
    const summon0 = summoning.actions.getObjectByID(data.summonIDs[0]);
    if (summon0 === undefined)
      throw new Error(
        `Error constructing SummoningSynergy. Summon with id ${data.summonIDs[0]} is not registered.`
      );
    const summon1 = summoning.actions.getObjectByID(data.summonIDs[1]);
    if (summon1 === undefined)
      throw new Error(
        `Error constructing SummoningSynergy. Summon with id ${data.summonIDs[1]} is not registered.`
      );
    if (summon0 === summon1)
      throw new Error(
        `Error constructing SummoningSynergy. Cannot have synergy where both summons are identical.`
      );
  ...
  get description() {
    if (this._customDescription !== undefined) {
      return getLangString(
        "SUMMONING_SYNERGY",
        `DESC_${this.summons[0].localID}_${this.summons[1].localID}`
      );
    } else {
      return describeModifierDataPlainLineBreak(this.modifiers);
    }
  }
}
class Summoning extends ArtisanSkill {
  constructor(namespace, game) {
    super(namespace, "Summoning", game);
    this._media = "assets/media/skills/summoning/summoning.svg";
  ...
  get totalSynergiesUnlocked() {
    return this.synergies.reduce((prev, synergy) => {
      if (this.isSynergyUnlocked(synergy)) prev++;
      return prev;
    }, 0);
  }
  get menu() {
  ...
  get actionItemQuantity() {
    ...
    if (this.isPoolTierActive(3)) quantity += 10;
    if (masteryLevel >= 99) quantity += 10;
    quantity +=
      this.game.modifiers.increasedSummoningCreationCharges -
      this.game.modifiers.decreasedSummoningCreationCharges;
    let percentBonus = 0;
    switch (this.activeRecipe.id) {
      case "melvorF:Salamander":
        percentBonus +=
          this.game.modifiers.increasedSalamanderCreationCharges -
          this.game.modifiers.decreasedSalamanderCreationCharges;
        break;
      case "melvorF:Leprechaun":
        quantity +=
          this.game.modifiers.increasedLeprechaunCreationCharges -
          this.game.modifiers.decreasedLeprechaunCreationCharges;
        break;
      case "melvorF:Cyclops":
        quantity +=
          this.game.modifiers.increasedCyclopsCreationCharges -
          this.game.modifiers.decreasedCyclopsCreationCharges;
        break;
    }
    if (
      this.game.combat.player.equipment.checkForItem(this.activeRecipe.product)
    )
      quantity +=
        this.game.modifiers.increasedSummoningCreationChargesForEquippedTablets;
    if (percentBonus !== 0)
      quantity = Math.floor(quantity * (1 + percentBonus / 100));
    ...
    (_a = data.categories) === null || _a === void 0
      ...
      : _a.forEach((category) => {
          this.categories.registerObject(
            new SkillCategory(namespace, category, this)
          );
        });
    (_b = data.recipes) === null || _b === void 0
      ...
      : _b.forEach((recipe) => {
          this.actions.registerObject(
            new SummoningRecipe(namespace, recipe, this.game, this)
          );
        });
    (_c = data.synergies) === null || _c === void 0
      ? void 0
      : _c.forEach((synergyData) => {
          ...
          if (
            (summon0Map === null || summon0Map === void 0
              ? void 0
              : summon0Map.get(summon1.product)) !== undefined
          )
            throw new Error(
              `Error registering summon synergy between ${summon0.id} and ${summon1.id}. Synergy already exists.`
            );
  ...
  postDataRegistration() {
    ...
    this.actions.forEach((recipe) => {
      ...
      recipe.skills.forEach((skill) => {
        let recipeArray = this.recipesBySkill.get(skill);
        if (recipeArray === undefined) {
          recipeArray = [];
          this.recipesBySkill.set(skill, recipeArray);
        }
  ...
  getPercentageIntervalModifier(action) {
    ...
    if (this.game.combat.player.equipment.checkForItem(action.product))
      modifier -=
        this.game.modifiers.decreasedSummoningIntervalPercentForEquippedTablets;
    if (action.id === "melvorF:Octopus")
      modifier -= this.game.modifiers.decreasedSummoningIntervalForOctopus;
    return modifier;
  }
  ...
  onLevelUp(oldLevel, newLevel) {
    ...
    this.actions.forEach((recipe) => {
      if (this.level >= recipe.level) this.renderQueue.markState.add(recipe);
    });
  }
  getErrorLog() {
    const altRecipeLog = [];
...
${altRecipeLog.join("\n")}`;
  ...
  getNonShardCostReductionModifier(recipe) {
    ...
    if (masteryLevel >= 99) modifier += 5;
    const equippedModifier =
      this.game.modifiers.decreasedNonShardCostForEquippedTablets -
      this.game.modifiers.increasedNonShardCostForEquippedTablets;
    const summon1 = this.game.combat.player.equipment.slots.Summon1;
    const summon2 = this.game.combat.player.equipment.slots.Summon2;
    if (recipe.product === summon1.item || recipe.product === summon2.item)
  ...
  modifyItemCost(item, quantity, recipe) {
    const masteryLevel = this.getMasteryLevel(recipe);
    if (item.type === "Shard") {
      if (masteryLevel >= 50) quantity--;
      if (masteryLevel >= 99) quantity--;
      quantity +=
        this.game.modifiers.increasedSummoningShardCost -
        this.game.modifiers.decreasedSummoningShardCost;
      if ((recipe.tier === 1 || recipe.tier === 2) && this.isPoolTierActive(1))
        quantity--;
      if (recipe.tier === 3 && this.isPoolTierActive(3)) quantity--;
    }
    return Math.max(1, quantity);
  }
  modifyGPCost(recipe) {
    let gpCost = super.modifyGPCost(recipe);
    gpCost *= 1 - this.getNonShardCostReductionModifier(recipe) / 100;
    return Math.max(1, Math.floor(gpCost));
  }
  modifySCCost(recipe) {
    let scCost = super.modifySCCost(recipe);
  ...
  addNonShardCosts(recipe, altID, costs) {
    ...
    const itemValueRequired =
      Summoning.recipeGPCost *
      (1 - this.getNonShardCostReductionModifier(recipe) / 100);
  ...
  onLoad() {
    ...
    switchSummoningCategory(this.categories.firstObject);
  }
  onEquipmentChange() {
  ...
  getRecipeCosts(recipe) {
    ...
    return this.getAltRecipeCosts(
      recipe,
      (_a = this.setAltRecipes.get(recipe)) !== null && _a !== void 0 ? _a : 0
    );
  }
    ...
    if (this.selectedRecipe !== undefined) {
      if (this.activeRecipe.nonShardItemCosts.length > 1) {
        this.menu.setRecipeDropdown(
          this.activeRecipe.nonShardItemCosts.map((cost, i) => {
            const costs = this.getAltRecipeCosts(this.activeRecipe, i);
            return {
  ...
  renderMarkState() {
    if (this.renderQueue.markState.size === 0) return;
    this.renderQueue.markState.forEach((mark) => {
      const markDiscovery = markDiscoveryMenus.get(mark);
      if (markDiscovery === undefined) return;
      if (mark.level > this.level) {
        markDiscovery.setLocked(mark);
      } else if (this.getMarkCount(mark) === 0) {
        markDiscovery.setUndiscovered(mark);
      } else {
        markDiscovery.setDiscovered(mark);
      }
    });
    this.renderQueue.markState.clear();
  }
  renderMarkProgress() {
    if (this.renderQueue.markCount.size === 0) return;
    this.renderQueue.markCount.forEach((mark) => {
      var _a;
      (_a = markDiscoveryMenus.get(mark)) === null || _a === void 0
        ? void 0
        : _a.updateDiscoveryCount(mark);
    });
    this.renderQueue.markCount.clear();
  }
  renderSynergyUnlock() {
    if (!this.renderQueue.synergyUnlock) return;
  ...
  renderSynergyQuantity() {
    if (!this.renderQueue.synergyQuantities) return;
    if (summoningSearchMenu.offsetParent !== null)
      summoningSearchMenu.updateVisibleElementQuantities();
    this.renderQueue.synergyQuantities = false;
  }
  queueMarkDiscoveryModal(mark) {
    if (!this.game.settings.showSummoningMarkDiscoveryModals) return;
    const discoveryCount = this.getMarkCount(mark);
    let html = `<small class="text-info">${getLangString(
      "MENU_TEXT",
      "MARK_DISCOVERED_TEXT0"
    )}<br><br>${getLangString("MENU_TEXT", "MARK_DISCOVERED_TEXT1")}</small>`;
    if (discoveryCount > 1)
      html = `<small class="text-info">${templateLangString(
        "MENU_TEXT",
        "MARK_DISCOVERED_TEXT2",
        {
          markName: `<span class="font-w700 text-success">${this.getMarkName(
            mark
          )}</span>`,
        }
      )}<br><br>${getLangString("MENU_TEXT", "MARK_DISCOVERED_TEXT3")}</small>`;
    const modal = {
      title: getLangString("MENU_TEXT", "MARK_DISCOVERED"),
      html: html,
      imageUrl: mark.markMedia,
      imageWidth: 64,
      imageHeight: 64,
      imageAlt: getLangString("MENU_TEXT", "MARK_DISCOVERED"),
    };
    addModalToQueue(modal);
  }
  queueMarkLevelUpModal(mark) {
    const markLevel = this.getMarkLevel(mark);
    const title = templateLangString("MENU_TEXT", "MARK_LEVEL", {
      level: `${markLevel}`,
    });
    let html = `<small>${templateLangString("MENU_TEXT", "MARK_LEVELUP_TEXT0", {
      markName: `<span class="font-w700 text-success">${this.getMarkName(
        mark
      )}</span>`,
    })}<br><br>${getLangString(
      "MENU_TEXT",
      "MARK_LEVELUP_TEXT1"
    )}<br><br><span class="font-w700 text-warning">${getLangString(
      "MENU_TEXT",
      "MARK_LEVELUP_TEXT2"
    )}</span></small>`;
    if (markLevel >= 2) {
      html = `<small>${templateLangString("MENU_TEXT", "MARK_LEVELUP_TEXT3", {
        markName: `<span class="font-w700 text-success">${this.getMarkName(
          mark
        )}</span>`,
      })}<br><br>${templateLangString("MENU_TEXT", "MARK_LEVELUP_TEXT4", {
        tierNum: `${markLevel - 1}`,
        markLevel: `${markLevel}`,
      })}</small>`;
    }
  ...
  recordCostPreservationStats(costs) {
    costs.recordBulkItemStat(
      this.game.stats.Summoning,
      SummoningStats.ItemsPreserved
    );
    costs.recordGPStat(this.game.stats.Summoning, SummoningStats.GPPreserved);
    costs.recordSCStat(this.game.stats.Summoning, SummoningStats.SCPreserved);
  }
  recordCostConsumptionStats(costs) {
    costs.recordBulkItemStat(
      this.game.stats.Summoning,
      SummoningStats.ItemsUsed
    );
    costs.recordGPStat(this.game.stats.Summoning, SummoningStats.GPUsed);
    costs.recordSCStat(this.game.stats.Summoning, SummoningStats.SCUsed);
  }
  preAction() {}
  get actionRewards() {
    const rewards = new Rewards(this.game);
    const actionEvent = new SummoningActionEvent(this, this.activeRecipe);
    ...
    if (qtyToAdd > 0) rewards.addItem(itemID, qtyToAdd);
  ...
  postAction() {
    ...
    this.game.stats.Summoning.add(
      SummoningStats.TimeSpent,
  ...
  encode(writer) {
    super.encode(writer);
    writer.writeMap(this.setAltRecipes, writeNamespaced, (value, writer) =>
      writer.writeUint8(value)
    );
    writer.writeMap(this.marksUnlocked, writeNamespaced, (value, writer) =>
      writer.writeUint8(value)
    );
    return writer;
  }
  ...
  deserialize(reader, version, idMap) {
    ...
    for (let i = 0; i < numSetAlts; i++) {
      ...
      if (recipe !== undefined) this.setAltRecipes.set(recipe, altID);
    }
    ...
    for (let i = 0; i < numMarksUnlocked; i++) {
      ...
      if (recipe === undefined)
        recipe = this.actions.getDummyObject(
          recipeID,
          DummySummoningRecipe,
          this.game
        );
  ...
  convertFromOldFormat(summoningData, idMap) {
    Object.entries(summoningData.MarksDiscovered).forEach(([key, count]) => {
      ...
      if (mark === undefined)
        mark = this.actions.getDummyObject(
          newID,
          DummySummoningRecipe,
          this.game
        );
    ...
    if (summoningData.defaultRecipe === undefined) return;
    Object.entries(summoningData.defaultRecipe).forEach(([key, altID]) => {
      ...
      if (
        mark === undefined ||
        altID <= 0 ||
        altID >= mark.nonShardItemCosts.length
      )
        return;
      this.setAltRecipes.set(mark, altID);
    });
  }
  ...
  setFromOldOffline(offline, idMap) {
    const recipe = this.actions.getObjectByID(
      idMap.summoningMarks[offline.action]
    );
    if (recipe !== undefined) {
  ...
  getMarkCount(mark) {
    var _a;
    return (_a = this.marksUnlocked.get(mark)) !== null && _a !== void 0
      ? _a
      : 0;
  }
  getMarkImage(mark) {
  ...
  getMarkName(mark) {
    if (this.level < mark.level || this.getMarkCount(mark) === 0) {
      return getLangString("MENU_TEXT", "QUESTION_MARKS");
    } else {
      return templateString(getLangString("MENU_TEXT", "MARK_OF_THE"), {
  ...
  getMarkLevel(mark) {
    ...
    const index = Summoning.markLevels.findIndex(
      (countRequired) => count < countRequired
    );
    if (index === -1) return Summoning.markLevels.length;
    else return index;
  }
  getSynergyData(summon1, summon2) {
    var _a;
    return (_a = this.synergiesByItem.get(summon1)) === null || _a === void 0
      ? void 0
      : _a.get(summon2);
  }
  ...
  isSynergyUnlocked(synergy) {
    ...
    return (
      mark1.tier < this.getMarkLevel(mark2) &&
      mark2.tier < this.getMarkLevel(mark1)
    );
  }
  getChanceForMark(mark, skill, modifiedInterval) {
    ...
    if (this.game.combat.player.equipment.checkForItem(mark.product)) {
      if (skill.hasMastery) equippedModifier = 2.5;
      else equippedModifier = 2;
    }
    return (
      (equippedModifier * modifiedInterval) /
      (2000 * Math.pow(mark.tier + 1, 2))
    );
  }
  rollForMark(mark, skill, modifiedInterval) {
    ...
    const cantRoll =
      ...
      !mark.skills.includes(skill) ||
      (markLevel > 0 && this.game.stats.itemFindCount(mark.product) < 1) ||
      markLevel >= Summoning.markLevels.length;
    if (
      !cantRoll &&
      rollPercentage(this.getChanceForMark(mark, skill, modifiedInterval))
    ) {
  ...
  discoverMark(mark) {
    ...
    if (prevLevel !== curLevel) {
      ...
      if (curLevel === 1) this.renderQueue.selectionTabs = true;
  ...
  checkForPetMark() {
    const unlock = !this.actions.some((mark) => {
      return (
        mark.level <= 99 && this.getMarkCount(mark) < Summoning.markLevels[3]
      );
    });
    if (unlock) this.game.petManager.unlockPetByID("melvorF:Mark");
  }
  rollMarksForSkill(skill, modifiedInterval) {
    if (!skill.isUnlocked) return;
    const recipes = this.recipesBySkill.get(skill);
  ...
  getMarkForSkill(skill) {
    const skills = this.recipesBySkill.get(skill);
  ...
  static updateSearchArray() {
    Summoning.searchArray = game.summoning.synergies.map((synergy) => {
      ...
      return {
        ...
        name1,
        name2,
        name1long: templateLangString("MENU_TEXT", "THE_FAMILIAR", {
          name: name1,
        }),
        name2long: templateLangString("MENU_TEXT", "THE_FAMILIAR", {
          name: name2,
        }),
      };
    });
...
function localizeSummoning() {
  summoningCategoryMenu.localize();
  summoningArtisanMenu.localize();
  summoningSelectionTabs.forEach((tab) => tab.localize());
  document.getElementById("mark-info-3-header").innerHTML = getLangString(
    "MENU_TEXT",
    "MARK_INFO_3"
  );
}
