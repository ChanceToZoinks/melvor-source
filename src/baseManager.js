...
class BaseManager extends NamespacedObject {
  constructor(game, namespace, id) {
    ...
    this.notifications = new NotificationQueue(10);
    ...
    this.rendersRequired = {
      location: false,
      pause: false,
      slayerAreaEffects: false,
      eventMenu: false,
      areaRequirements: false,
      spellBook: false,
    };
  ...
  render() {
    ...
    if (this.rendersRequired.location) this.renderLocation();
...
-- End Enemy Information --`;
  ...
  renderSpellBook() {
    if (!this.rendersRequired.spellBook) return;
    updateSpellbook(
      selectedSpellbook,
      this.player,
      this.ignoreSpellRequirements ? 120 : this.game.altMagic.level,
      this.ignoreSpellRequirements
    );
    this.rendersRequired.spellBook = false;
  }
  checkDeath() {
    const playerDied = this.player.hitpoints <= 0;
    const enemyDied =
      this.enemy.state === EnemyState.Alive && this.enemy.hitpoints <= 0;
    if (playerDied) {
      this.onPlayerDeath();
    }
  ...
  onEnemyDeath() {
    ...
    if (
      this.game.currentGamemode.id == "melvorF:HardcoreAdventureSpeedrun" &&
  ...
  addMonsterStat(statID, amount = 1) {
    if (this.enemy.monster === undefined) return;
  ...
  onSelection() {
    ...
    this.rendersRequired.location = true;
    this.loadNextEnemy();
    if (!loadingOfflineProgress) saveData("all");
  }
  stop(fled = true) {
    this.game.renderQueue.combatMinibar = true;
    this.game.renderQueue.title = true;
  ...
  uniqueUpdatesOnEnemySpawn() {
    this.player.applyUniqueSpawnEffects();
    this.enemy.applyUniqueSpawnEffects();
  }
  statUpdateOnEnemySpawn() {
    this.player.addCombatAreaEffectModifiers();
    this.player.addTargetModifiers();
    this.enemy.addTargetModifiers();
  ...
  onCombatPageChange() {
    this.player.rendersRequired.attackBar = true;
    this.player.rendersRequired.attackBarMinibar = true;
    this.player.rendersRequired.summonBar = true;
    this.enemy.rendersRequired.attackBar = true;
    this.enemy.rendersRequired.attackBarMinibar = true;
    this.player.rendersRequired.activeSkillModifierChange = true;
