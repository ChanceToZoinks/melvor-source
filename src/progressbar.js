...
class ProgressBar {
  ...
  animateProgressFromTimer(timer) {
    this.animateProgress(
      (timer.maxTicks - timer.ticksLeft) * TICK_INTERVAL,
      timer.maxTicks * TICK_INTERVAL
    );
  }
  ...
  setAnimation(animation) {
    ...
    if (this.isReversed) this.barElem.style.animationDirection = "reverse";
  }
  setFixedPosition(percent) {
    if (this.barElem.style.animation !== "") this.stopAnimation();
