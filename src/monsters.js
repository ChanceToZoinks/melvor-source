...
class Monster extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    data.specialAttacks.forEach((attackID, i) => {
      ...
      if (attack === undefined)
        throw new Error(
          `Error constructing monster, attack with id: ${attackID} is not registered`
        );
      const chance =
        (_b =
          (_a = data.overrideSpecialChances) === null || _a === void 0
            ? void 0
            : _a[i]) !== null && _b !== void 0
          ? _b
    ...
    this.passives = data.passives.map((passiveID) => {
      const passive = game.combatPassives.getObjectByID(passiveID);
      if (passive === undefined)
        throw new Error(
          `Error constructing Monster with id: ${this.localID}. Combat Passive with id :${passiveID} is not registered.`
        );
      return passive;
    });
    this.lootChance = data.lootChance;
    this.lootTable = new DropTable(game, data.lootTable);
    this.gpDrops = data.gpDrops;
    if (data.bones !== undefined) {
      const item = game.items.getObjectByID(data.bones.itemID);
      if (item === undefined)
        throw new Error(
          `Error constructing monster, bone with id: ${data.bones.itemID} is not registered`
        );
      this.bones = { item, quantity: data.bones.quantity };
    }
    this.canSlayer = data.canSlayer;
    this.isBoss = data.isBoss;
    const spell = game.standardSpells.getObjectByID(data.selectedSpell);
    if (spell === undefined)
      throw new Error(
        `Error constructing Monster: ${this.localID}. Standard Spell with id: ${data.selectedSpell} is not registered.`
      );
    ...
    if (data.pet !== undefined) {
      const pet = game.pets.getObjectByID(data.pet.id);
      if (pet === undefined)
        throw new Error(
          `Error constructing Monster: ${this.id}. Pet with id: ${data.pet.id} is not registered.`
        );
      this.pet = { pet, kills: data.pet.quantity };
    }
  }
  get media() {
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString("MONSTER_NAME", this.localID);
    }
  }
  get description() {
    ...
    if (this.isModded) {
      ...
      return getLangString("MONSTER_DESCRIPTION", this.localID);
    }
  }
  get combatLevel() {
    const prayer = 1;
    const base =
      0.25 *
      (this.levels.Defence + this.levels.Hitpoints + Math.floor(prayer / 2));
