...
class StatTracker {
  ...
  add(stat, qty) {
    ...
    this.stats.set(
      stat,
      ((_a = this.stats.get(stat)) !== null && _a !== void 0 ? _a : 0) + qty
    );
  ...
  encode(writer) {
    writer.writeMap(
      this.stats,
      (key, writer) => writer.writeUint32(key),
      (key, writer) => writer.writeFloat64(key)
    );
    return writer;
  }
  decode(reader, version) {
    this.stats = reader.getMap(
  ...
  convertFromOldStatArray(oldStats) {
    oldStats.forEach((stat, statID) => {
      if (stat.count !== 0) this.add(statID, stat.count);
    });
  }
}
class MappedStatTracker {
  constructor(registry, dummyConstructor, game) {
  ...
  decode(reader, version) {
    this.statsMap = reader.getMap(
      (reader) => {
        const object = reader.getNamespacedObject(this.registry);
        if (typeof object === "string") {
          if (object.startsWith("melvor"))
            return this.registry.getDummyObject(
              object,
              this.dummyConstructor,
              this.game
            );
  ...
  deserialize(reader, version, idMap) {
    ...
    for (let i = 0; i < numIds; i++) {
      ...
      if (newID !== undefined) {
        ...
        if (key === undefined)
          key = this.registry.getDummyObject(
            idMap[id],
            this.dummyConstructor,
            this.game
          );
