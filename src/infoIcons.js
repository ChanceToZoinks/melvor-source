...
class InfoIcon extends ContainedComponent {
  constructor(parent, pillClass, size) {
    ...
    this.image = this.container.appendChild(
      createElement("img", {
    ...
    this.text = this.container
      ...
      .appendChild(
        createElement("small", { classList: ["badge-pill", pillClass] })
      );
...
class XPIcon extends InfoIcon {
  constructor(parent, xp, size = 48) {
    ...
    this.xp = xp;
    this.setXP(xp);
    this.setImage(InfoIcon.media.skillXP);
  }
  setXP(xp) {
  ...
  localize() {
    this.setTooltip(this.getTooltipContent(this.xp));
  }
  getTooltipContent(xp) {
    return `<div class="text-center">${templateLangString(
      "MENU_TEXT",
      "TOOLTIP_SKILL_XP",
      { xp: `${xp}` }
    )}<br><small>${getLangString(
      "MENU_TEXT",
      "INCLUSIVE_OF_BONUSES"
    )}</small></div>`;
  }
}
class STRXPIcon extends InfoIcon {
  constructor(parent, xp, size = 48) {
    super(parent, "bg-secondary", size);
    this.xp = xp;
    this.setImage(cdnMedia(InfoIcon.media.strXP));
    this.setXP(xp);
  }
  setXP(xp) {
    if (xp <= 0) this.hide();
  ...
  getTooltipContent(xp) {
    return `<div class="text-center">${xp} ${getLangString(
      "SKILL_NAME",
      "Strength"
    )} XP<br><small>${getLangString(
      "MENU_TEXT",
      "INCLUSIVE_OF_BONUSES"
    )}</small></div>`;
  }
...
class IntervalIcon extends InfoIcon {
  ...
  setMedia(altMedia) {
    this.setImage(
      altMedia ? InfoIcon.media.intervalAlt : InfoIcon.media.interval
    );
  }
  localize() {
    this.setTooltip(
      `<div class="text-center text-warning">${getLangString(
        "MENU_TEXT",
        "TOOLTIP_INTERVAL"
      )}<br><small>${getLangString(
        "MENU_TEXT",
  ...
  setInterval(interval) {
    this.setText(`${formatFixed(interval / 1000, 2)}s`);
  }
}
class DoublingIcon extends InfoIcon {
  constructor(parent, chance, size = 48) {
    ...
    this.setImage(InfoIcon.media.doublingChance);
  }
  localize() {
  ...
  getTooltipContent() {
    return `
    <h5 class="font-w400 font-size-sm mb-1 text-warning text-center">${getLangString(
      "MENU_TEXT",
      "TOOLTIP_DOUBLE"
    )}</h5>
    <h5 class="font-w400 font-size-sm mb-1 text-danger text-center">${templateString(
      getLangString("MENU_TEXT", "TOOLTIP_CAPPED"),
      { chance: "100" }
    )}</h5>
...
class PreservationIcon extends InfoIcon {
  constructor(parent, chance, size = 48) {
    ...
    this.localize();
    this.setChance(chance);
    this.setImage(InfoIcon.media.preserveChance);
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  }
...
class PerfectCookIcon extends InfoIcon {
  constructor(parent, chance, size = 48) {
    super(parent, "bg-primary", size);
    this.localize();
    this.setChance(chance);
    this.setImage(InfoIcon.media.perfectCook);
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  }
...
class CookingSuccessIcon extends InfoIcon {
  constructor(parent, chance, size = 48) {
    super(parent, "bg-primary", size);
    this.localize();
    this.setChance(chance);
    this.setImage(InfoIcon.media.successfulCook);
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  }
...
class MasteryXPIcon extends InfoIcon {
  constructor(parent, xp, size = 48) {
    ...
    this.xp = xp;
    this.setXP(xp);
    this.setImage(InfoIcon.media.masteryXP);
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  }
...
class MasteryPoolIcon extends InfoIcon {
  constructor(parent, xp, size = 48) {
    ...
    this.xp = xp;
    this.setXP(xp);
    this.setImage(InfoIcon.media.poolXP);
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  }
...
class StealthIcon extends InfoIcon {
  constructor(parent, size = 48) {
    super(parent, "bg-primary", size);
    this.setImage(game.thieving.media);
  }
  setNPC(npc) {
    this.npc = npc;
    this.setText(`${game.thieving.getStealthAgainstNPC(npc)}`);
    this.localize();
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  ...
  getTooltipContent() {
    ...
    <h5 class="font-w600 font-size-sm mb-1 text-white">${templateString(
      getLangString("MENU_TEXT", "TOOLTIP_STEALTH_VS"),
      {
        stealth: `${game.thieving.getStealthAgainstNPC(this.npc)}`,
        perception: `${this.npc.perception}`,
      }
    )}</h5>
    <h5 class="font-w400 font-size-sm mb-1">${getLangString(
      "MENU_TEXT",
      "THIS_GIVES_YOU"
    )}</h5>
    <small>
        <span> ${templateLangString("MENU_TEXT", "SUCCESS_RATE", {
          value: `<span class="text-success">${formatPercent(
            game.thieving.getNPCSuccessRate(this.npc),
            2
          )}</span>`,
        })}</span><br>
        <span> ${templateLangString("MENU_TEXT", "TOOLTIP_INCREASED_DOUBLING", {
          value: `<span class="text-success">${formatPercent(
            game.thieving.getNPCSleightOfHand(this.npc),
            2
          )}</span>`,
        })}</span><br>
        <span> ${templateLangString("MENU_TEXT", "TOOLTIP_NPC_UNIQUE_CHANCE", {
          value: `<span class="text-success">${formatPercent(
            game.thieving.getNPCPickpocket(this.npc),
            3
          )}</span>`,
        })}</span><br>
    </small>
...
class MeteoriteChanceIcon extends InfoIcon {
  constructor(parent, size = 48) {
    super(parent, "bg-secondary", size);
    this.setImage(cdnMedia("assets/media/skills/astrology/meteorite.svg"));
    this.localize();
  }
  localize() {
    this.setTooltip(this.getTooltipContent());
  ...
  getTooltipContent() {
    return `<div class="text-center">${getLangString(
      "ORE_NAME",
      "Meteorite_Ore"
    )}</div>`;
  }
...
class ItemQtyIcon extends QtyIcon {
  constructor(parent, quickBuy = false, qty = 0, size = 48) {
    ...
    this.autoBuyIcon = createElement("img", {
      ...
      attributes: [["src", InfoIcon.media.shopIcon]],
    });
...
class CookingStockpileIcon extends ItemQtyIcon {
  constructor(parent, category, size = 48) {
    super(parent, false, 0, size);
    this.container.onclick = () =>
      game.cooking.onCollectStockpileClick(category);
    this.container.classList.remove("m-1");
  ...
  unsetItem() {
    this.setText("0");
    hideElement(this.image);
    this.tooltip.disable();
  }
  setItem(item, qty) {
    super.setItem(item, qty);
    showElement(this.image);
    this.tooltip.enable();
  }
}
class GPQtyIcon extends QtyIcon {
  constructor(parent, qty, size = 48) {
    super(parent, qty, size);
    this.setImage(InfoIcon.media.gp);
    this.localize();
  }
  getCurrentQty() {
    return game.gp.amount;
  }
  getName() {
    return getLangString("MENU_TEXT", "GP");
  }
}
class SCQtyIcon extends QtyIcon {
  constructor(parent, qty, size = 48) {
    super(parent, qty, size);
    this.localize();
    this.setImage(InfoIcon.media.slayerCoins);
  }
  getCurrentQty() {
    return game.slayerCoins.amount;
  }
  getName() {
    return getLangString("MENU_TEXT", "SLAYER_COINS");
  }
}
class QtyCurrentIcon extends InfoIcon {
  constructor(parent, requiredQty, size = 48) {
    super(parent, "bg-primary", size);
  ...
  updateQuantity() {
    this.currentQuantity = this.getCurrentQty();
    this.setText(formatNumber(this.currentQuantity));
  ...
  onMouseover() {
    this.setText(numberWithCommas(this.currentQuantity));
  }
  onMouseleave() {
    this.setText(formatNumber(this.currentQuantity));
  }
...
class ItemCurrentIcon extends QtyCurrentIcon {
  constructor(
    ...
    super(parent, requiredQty, size);
    this.item = item;
    this.init();
    this.setImage(altMedia ? item.altMedia : item.media);
    const purchase = game.shop.getQuickBuyPurchase(item);
    if (quickBuy && purchase !== undefined) {
      const autoBuyIcon = createElement("img", {
        classList: ["skill-icon-xxs", "is-in-shop"],
        attributes: [["src", InfoIcon.media.shopIcon]],
      });
      this.container.append(autoBuyIcon);
      this.container.onclick = () => game.shop.quickBuyItemOnClick(purchase);
    }
  }
  getCurrentQty() {
...
class GPCurrentIcon extends QtyCurrentIcon {
  constructor(parent, requiredQty, size = 48) {
    super(parent, requiredQty, size);
    this.setImage(InfoIcon.media.gp);
    this.init();
...
class SCCurrentIcon extends QtyCurrentIcon {
  constructor(parent, requiredQty, size = 48) {
    super(parent, requiredQty, size);
    this.setImage(InfoIcon.media.slayerCoins);
    this.init();
