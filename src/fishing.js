...
class FishingArea extends NamespacedObject {
  constructor(namespace, data, fishing, game) {
    ...
    this.fish = data.fishIDs.map((id) => {
      const fish = fishing.actions.getObjectByID(id);
      if (fish === undefined)
        throw new Error(
          `Error constructing FishingArea with id ${this.id}. Fish with id: ${id} is not registered.`
        );
      return fish;
    });
    this._description = data.description;
    if (data.requiredItemID !== undefined) {
      const item = game.items.equipment.getObjectByID(data.requiredItemID);
      if (item === undefined)
        throw new Error(
          `Error constructing FishingArea. Item with id: ${data.requiredItemID} is not registered.`
        );
      this.requiredItem = item;
    }
    if (data.isSecret) this.isSecret = true;
  }
  get name() {
    if (this.isModded) {
      return this._name;
    } else {
      return getLangString("FISHING", `AREA_NAME_${this.localID}`);
    }
  }
  get description() {
    if (this._description === undefined) return undefined;
    if (this.isModded) {
      return this._description;
    } else {
      return getLangString("FISHING", `AREA_DESCRIPTION_${this.localID}`);
    }
  }
}
class Fishing extends GatheringSkill {
  constructor(namespace, game) {
    super(namespace, "Fishing", game);
    this._media = "assets/media/skills/fishing/fishing.svg";
    ...
    this.junkItems = [];
    this.specialItems = new DropTable(this.game, []);
  ...
  get chanceForLostChest() {
    return this.game.modifiers.increasedChanceToFindLostChest;
  }
  get actionInterval() {
  ...
  get chanceForOneExtraFish() {
    return (
      this.game.modifiers.increasedChanceForOneExtraFish -
      this.game.modifiers.decreasedChanceForOneExtraFish
    );
  }
  get activeFish() {
    ...
    if (fish === undefined)
      throw new Error(
        "Tried to get active fish from area, but area has no fish selected"
      );
    return fish;
  }
    ...
    (_b = data.areas) === null || _b === void 0
      ...
      : _b.forEach((area) => {
          this.areas.registerObject(
            new FishingArea(namespace, area, this, this.game)
          );
        });
    (_c = data.junkItemIDs) === null || _c === void 0
      ? void 0
      : _c.forEach((itemID) => {
          const junkItem = this.game.items.getObjectByID(itemID);
          if (junkItem === undefined)
            throw new Error(
              `Error registering fishing skill data, junk item with id: ${itemID} is not registered.`
            );
          this.junkItems.push(junkItem);
        });
    if (data.specialItems !== undefined)
      this.specialItems.registerDrops(this.game, data.specialItems);
    if (data.easterEgg !== undefined) {
      const original = this.game.items.getObjectByID(data.easterEgg.originalID);
      const equipped = this.game.items.equipment.getObjectByID(
        data.easterEgg.equippedID
      );
  ...
  unlockSecretArea() {
    if (this.secretAreaUnlocked) return;
  ...
  getUncappedDoublingChance(action) {
    ...
    if (this.isPoolTierActive(2)) chance += 5;
    return chance;
  }
  ...
  getAreaChances(area) {
    ...
    const fishToSpecialShift =
      ...
      this.game.modifiers.decreasedFishingSpecialChance;
    let bonusSpecialChance =
      this.game.modifiers.increasedBonusFishingSpecialChance;
    let noJunk = this.isPoolTierActive(1);
    const fish = this.selectedAreaFish.get(area);
    if (fish !== undefined) {
      const masteryLevel = this.getMasteryLevel(fish);
      if (masteryLevel >= 50) bonusSpecialChance += 3;
      if (masteryLevel >= 65) noJunk = true;
    }
    chances.addBonusSpecialChance(bonusSpecialChance);
    chances.shiftFishToSpecial(fishToSpecialShift);
    if (noJunk) chances.shiftJunkToFish(chances.junk);
    return chances;
  }
  preAction() {}
  get actionRewards() {
    ...
    const actionEvent = new FishingActionEvent(
      this,
      fish,
      this.activeFishingArea
    );
    let rewardType = chances.rollForRewardType();
    if (!this.game.tutorial.complete) rewardType = FishingRewardType.Fish;
    const masteryLevel = this.masteryLevel;
    let rewardQty = 1;
    if (masteryLevel >= 99 || rollPercentage(masteryLevel * 0.4))
      rewardQty *= 2;
    const doublingChance = this.getDoublingChance(this.masteryAction);
    if (rollPercentage(doublingChance)) rewardQty *= 2;
    rewardQty *= Math.pow(
      2,
    ...
    if (rollPercentage(chanceForExtraResource)) rewardQty++;
    if (rewardType === FishingRewardType.Fish) {
      if (rollPercentage(this.chanceForOneExtraFish)) rewardQty++;
    }
    let rewardItem;
    switch (rewardType) {
    ...
    if (cookedItem !== undefined) {
      ...
      if (rollPercentage(this.game.modifiers.increasedFishingCookedChance))
        rewards.addItem(cookedItem, 1);
    }
    if (
      this.lostChestItem !== undefined &&
      rollPercentage(this.chanceForLostChest)
    )
      rewards.addItem(this.lostChestItem, 1);
    this.game.processEvent(actionEvent, this.currentActionInterval);
    return rewards;
  }
  postAction() {
    this.game.stats.Fishing.inc(FishingStats.Actions);
    this.game.stats.Fishing.add(
...
Active Area: ${
      (_a = this.activeFishingArea) === null || _a === void 0 ? void 0 : _a.id
    }
Selected Area Fish: 
...
`;
  ...
  onLoad() {
    ...
    this.areas.forEach((area) => {
      var _a;
      (_a = fishingAreaMenus.get(area)) === null || _a === void 0
        ? void 0
        : _a.setAreaData(area);
    });
  ...
  onStop() {
    var _a;
    if (this.activeFishingArea !== undefined)
      (_a = fishingAreaMenus.get(this.activeFishingArea)) === null ||
      _a === void 0
        ? void 0
        : _a.setActionInactive();
  }
  ...
  renderHiddenAreas() {
    this.areas.forEach((area) => {
      ...
      else menu === null || menu === void 0 ? void 0 : menu.showAreaPanel();
    });
  }
  onAreaHeaderClick(area) {
    ...
    if (this.hiddenAreas.has(area)) {
      ...
      (_b = fishingAreaMenus.get(area)) === null || _b === void 0
        ? void 0
        : _b.hideAreaPanel();
    }
  }
  onAreaFishSelection(area, fish) {
    ...
    if (
      area === this.activeFishingArea &&
      previousSelection !== fish &&
      this.isActive &&
      !this.stop()
    )
      return;
  ...
  renderSelectedAreaFish() {
    if (!this.renderQueue.selectedAreaFish) return;
    this.areas.forEach((area) => {
      const menu = fishingAreaMenus.get(area);
  ...
  renderSelectedFishRates() {
    if (!this.renderQueue.selectedAreaFishRates) return;
  ...
  renderAreaChances() {
    if (!this.renderQueue.areaChances) return;
    this.areas.forEach((area) => {
      var _a;
      (_a = fishingAreaMenus.get(area)) === null || _a === void 0
        ? void 0
        : _a.setChances(this.getAreaChances(area), area);
    });
    this.renderQueue.areaChances = false;
  }
  renderAreaButtons() {
    if (!this.renderQueue.areaButtons) return;
    this.areas.forEach((area) => {
      var _a;
      (_a = fishingAreaMenus.get(area)) === null || _a === void 0
        ? void 0
        : _a.updateButtons(area);
    });
    this.renderQueue.areaButtons = false;
  }
  renderAreaUnlock() {
    if (!this.renderQueue.areaUnlock) return;
    this.areas.forEach((area) => {
      const menu = fishingAreaMenus.get(area);
      if (menu === undefined) return;
      if (
        (area.isSecret && !this.secretAreaUnlocked) ||
        (area.requiredItem !== undefined &&
          !this.game.combat.player.equipment.checkForItem(area.requiredItem))
      ) {
        hideElement(menu);
      } else {
        showElement(menu);
  ...
  renderActiveArea() {
    var _a;
    if (!this.renderQueue.activeArea) return;
  ...
  encode(writer) {
    ...
    if (this.isActive) {
      if (this.activeFishingArea === undefined)
        throw new Error(
          `Error encoding Fishing. Skill is active, but no area is active.`
        );
      writer.writeNamespacedObject(this.activeFishingArea);
    }
    writer.writeMap(
      this.selectedAreaFish,
      (key, writer) => {
  ...
  decode(reader, version) {
    ...
    if (this.isActive) {
      ...
      else this.activeFishingArea = area;
    }
    this.selectedAreaFish = reader.getMap(
      readNamespacedReject(this.areas),
  ...
  deserialize(reader, version, idMap) {
    ...
    if (this.isActive) {
      ...
      if (this.activeFishingArea === undefined) this.shouldResetAction = true;
    }
  ...
  setFromOldOffline(offline, idMap) {
    const area = this.areas.getObjectByID(
      idMap.fishingAreas[offline.action[0]]
    );
...
class FishingAreaChances {
  ...
  rollForRewardType() {
    ...
    if (roll < this.fish) return FishingRewardType.Fish;
    else if (roll < this.fish + this.junk) return FishingRewardType.Junk;
    else return FishingRewardType.Special;
  }
}
