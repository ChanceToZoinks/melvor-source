...
class IconBox extends ContainedComponent {
  constructor(parent, smallName, containerClasses = [], iconContClasses = []) {
    super();
    this.icons = [];
    this.container = createElement("div", { classList: containerClasses });
    const nameHeader = createElement("h5", {
      classList: ["font-w600", "font-size-sm", "mb-1", "text-center"],
      parent: this.container,
    });
    if (smallName) {
      this.name = createElement("small", { parent: nameHeader });
    } else {
    ...
    this.dash = this.iconContainer.appendChild(
...
class RequiresBox extends IconBox {
  ...
  localize() {
    super.localize();
    this.setName(getLangString("MENU_TEXT", this.titleLangID));
  }
  setItems(items, gp = 0, sc = 0, altMedia = false) {
    this.destroyIcons();
    items.forEach(({ item, quantity }) => {
      const reqIcon = new ItemQtyIcon(
        this.iconContainer,
        true,
        quantity,
        this.size
    ...
    if (gp > 0) this.addIcon(new GPQtyIcon(this.iconContainer, gp, this.size));
    if (sc > 0) this.addIcon(new SCQtyIcon(this.iconContainer, sc, this.size));
  }
  setItemsFromRecipe(recipe, altMedia = false) {
    this.destroyIcons();
    recipe.itemCosts.forEach(({ item, quantity }) => {
      const icon = new ItemQtyIcon(
        this.iconContainer,
        true,
        quantity,
        this.size
...
class HavesBox extends IconBox {
  ...
  setItems(items, gp = 0, sc = 0, altMedia = false) {
    this.destroyIcons();
    items.forEach(({ item, quantity }) => {
      const reqIcon = new ItemCurrentIcon(
        this.iconContainer,
        item,
        quantity,
        true,
  ...
  setItemsFromRecipe(recipe, altMedia = false) {
    ...
    if (recipe.gpCost > 0)
      this.addIcon(
        new GPCurrentIcon(this.iconContainer, recipe.gpCost, this.size)
      );
    if (recipe.scCost > 0)
      this.addIcon(
        new SCCurrentIcon(this.iconContainer, recipe.scCost, this.size)
      );
  }
}
class ProducesBox extends IconBox {
  ...
  localize() {
    super.localize();
    this.setName(getLangString("MENU_TEXT", "PRODUCES"));
  }
}
class GrantsBox extends IconBox {
  constructor(
    ...
    super(parent, smallName, containerClasses, iconContClasses);
    this.xpIcon = new XPIcon(this.iconContainer, 69, size);
    this.masteryXPIcon = new MasteryXPIcon(this.iconContainer, 69, size);
  ...
  localize() {
    super.localize();
    this.setName(getLangString("MENU_TEXT", "GRANTS"));
  }
  updateGrants(xp, masteryXP, poolXP) {
    this.xpIcon.setXP(xp);
    this.masteryXPIcon.setXP(masteryXP);
...
class CookingBonusBox extends IconBox {
  constructor(parent, smallName, containerClasses = []) {
    super(parent, smallName, containerClasses);
    this.preserve = new PreservationIcon(this.iconContainer, 0);
    this.double = new DoublingIcon(this.iconContainer, 0);
    this.perfect = new PerfectCookIcon(this.iconContainer, 0);
    this.success = new CookingSuccessIcon(this.iconContainer, 0);
    this.addIcon(this.preserve);
    this.addIcon(this.double);
    this.addIcon(this.perfect);
    this.addIcon(this.success);
    this.setUnselected();
    this.localize();
  }
  setChances(preserve, double, perfect, success) {
    this.preserve.setChance(preserve);
    this.double.setChance(double);
    this.perfect.setChance(perfect);
    this.success.setChance(success);
  }
  localize() {
    super.localize();
    this.setName(getLangString("MENU_TEXT", "BONUSES"));
  }
}
