...
class CombatEvent extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    const getPassives = (passiveID) => {
      const passive = game.combatPassives.getObjectByID(passiveID);
      if (passive === undefined)
        throw new Error(
          `Error constructing CombatEven with id: ${this.id}. Combat passive with id ${passiveID} is not registered.`
        );
      return passive;
    };
    this.itemRewards = data.itemRewardIDs.map((itemID) => {
      const item = game.items.getObjectByID(itemID);
      if (item === undefined)
        throw new Error(
          `Error constructing CombatEvent with id: ${this.id}. Reward item with id: ${itemID} is not registered.`
        );
      return item;
    });
    const pet = game.pets.getObjectByID(data.petID);
    if (pet === undefined)
      throw new Error(
        `Error constructing CombatEvent with id: ${this.id}. Pet with id: ${data.petID} is not registered.`
      );
    this.pet = pet;
    this.slayerAreas = data.slayerAreaIDs.map((areaID) => {
      const area = game.slayerAreas.getObjectByID(areaID);
      if (area === undefined)
        throw new Error(
          `Error constructing CombatEvent with id: ${this.id}. Slayer area with id ${areaID} is not registered.`
        );
      return area;
    });
    this.passiveSelection = data.passiveSelectionIDs.map(getPassives);
    this.enemyPassives = data.enemyPassives.map(getPassives);
    this.bossPassives = data.bossPassives.map(getPassives);
    const firstBoss = game.monsters.getObjectByID(data.firstBossMonster);
    if (firstBoss === undefined)
      throw new Error(
        `Error constructing CombatEvent with id: ${this.id}. Monster with id ${data.firstBossMonster} is not registered.`
      );
    this.firstBossMonster = firstBoss;
    const finalBoss = game.monsters.getObjectByID(data.finalBossMonster);
    if (finalBoss === undefined)
      throw new Error(
        `Error constructing CombatEvent with id: ${this.id}. Monster with id ${data.firstBossMonster} is not registered.`
      );
    this.finalBossMonster = finalBoss;
  }
}
class CombatManager extends BaseManager {
  constructor(game, namespace) {
    ...
    this.openCombatAreaMenu = CombatAreaType.None;
  }
  get media() {
    return cdnMedia("assets/media/skills/combat/combat.svg");
  }
  get name() {
  ...
  get isFightingITMBoss() {
    ...
    return (
      this.selectedArea.id === "melvorF:Into_the_Mist" &&
      this.dungeonProgress > this.selectedArea.monsters.length - 4
    );
  }
  get onSlayerTask() {
    return (
  ...
  get areaRequirements() {
    ...
    return this.selectedArea.entryRequirements;
  }
  get slayerAreaLevelReq() {
    if (this.selectedArea instanceof SlayerArea)
      return this.selectedArea.slayerLevelRequired;
    return 0;
  }
  get playerAreaModifiers() {
    ...
    if (
      ...
      area.areaEffect &&
      !(
        this.player.attackType === "magic" && area.id === "melvorF:Runic_Ruins"
      ) &&
      area.areaEffect.target === "Player"
    ) {
      modifiers[area.areaEffect.modifier] = this.getAreaEffectMagnitude(
        area.areaEffect
      );
    }
    return modifiers;
  ...
  addDungeonCompletion(dungeon) {
    this.dungeonCompletion.set(
      dungeon,
      this.getDungeonCompleteCount(dungeon) + 1
    );
  }
  getDungeonCompleteCount(dungeon) {
    var _a;
    return (_a = this.dungeonCompletion.get(dungeon)) !== null && _a !== void 0
      ? _a
      : 0;
  }
  ...
  getMonsterDropsHTML(monster, respectArea) {
    ...
    if (
      ...
      !(respectArea && this.areaType === CombatAreaType.Dungeon)
    ) {
      drops = monster.lootTable.sortedDropsArray
        .map((drop) => {
          ...
          if (DEBUGENABLED)
            dropText += ` (${(
              (monster.lootChance * drop.weight) /
              monster.lootTable.weight
            ).toFixed(2)}%)`;
          return dropText;
        })
        .join("<br>");
    ...
    let bones = "";
    if (
      monster.bones !== undefined &&
      !(
        respectArea &&
        this.selectedArea instanceof Dungeon &&
        !this.selectedArea.dropBones
      )
    ) {
      bones = `${getLangString(
        "MISC_STRING",
        "7"
      )}<br><small><img class="skill-icon-xs mr-2" src="${
        monster.bones.item.media
      }">${monster.bones.item.name}</small><br><br>`;
    } else {
      bones = getLangString("COMBAT_MISC", "107") + "<br><br>";
    }
    let html = `<span class="text-dark">${bones}<br>`;
    if (drops !== "") {
      html += `${getLangString("MISC_STRING", "8")}<br><small>${getLangString(
        "MISC_STRING",
        "9"
      )}</small><br>${drops}`;
    }
  ...
  getAreaEffectMagnitude(areaEffect) {
    let bonus = 0;
    if (
      this.isEventActive &&
      this.eventPassives.some(
        (passive) => passive.id === "melvorF:EventPassive12"
      )
    ) {
      bonus = 5;
    }
  ...
  initialize() {
    ...
    this.rendersRequired.eventMenu = true;
    this.rendersRequired.areaRequirements = true;
    super.initialize();
...
Current Area ID: ${
      (_a = this.selectedArea) === null || _a === void 0 ? void 0 : _a.id
    }`;
  }
  render() {
    if (this.rendersRequired.slayerAreaEffects) this.renderSlayerAreaEffects();
    super.render();
  ...
  renderLocation() {
    ...
    let countText = "";
    let effectText = "";
    let areaName = getLangString("COMBAT_MISC", "39");
    ...
    if (this.selectedArea !== undefined) {
      ...
      if (this.selectedArea instanceof Dungeon) {
        if (this.selectedArea.floors !== undefined) {
          ...
          floorText = templateLangString("COMBAT_MISC", "FLOOR_COUNT", {
            currentFloor: `${floor}`,
            totalFloors: `${this.selectedArea.floors.length}`,
          });
          countText = `(${this.dungeonProgress - floorCount + 1} / ${
            this.selectedArea.floors[floor - 1]
          })`;
        } else {
          countText = `(${this.dungeonProgress + 1} / ${
            this.selectedArea.monsters.length
          })`;
        }
      } else if (this.selectedArea instanceof SlayerArea) {
        if (this.selectedArea.areaEffect) {
          effectText = templateString(this.selectedArea.areaEffectDescription, {
            effectValue: `${this.getAreaEffectMagnitude(
              this.selectedArea.areaEffect
            )}`,
          });
          combatMenus.locationElements.areaEffect.classList.add("text-danger");
          combatMenus.locationElements.areaEffect.classList.remove(
            "text-success"
          );
        } else {
          effectText = getLangString("COMBAT_MISC", "NO_AREA_EFFECT");
          combatMenus.locationElements.areaEffect.classList.remove(
            "text-danger"
          );
  ...
  renderSlayerAreaEffects() {
    if (this.rendersRequired.slayerAreaEffects)
      areaMenus.slayer.updateAreaEffectValues();
    if (this.areaType === CombatAreaType.Slayer)
      this.rendersRequired.location = true;
    this.rendersRequired.slayerAreaEffects = false;
  }
  renderEventMenu() {
    if (!this.rendersRequired.eventMenu) return;
    if (this.isEventActive) {
      showElement(combatMenus.eventMenu);
      document.body.style.backgroundImage = `url(${cdnMedia(
        "assets/media/main/bg_event.jpg"
      )})`;
    } else {
      hideElement(combatMenus.eventMenu);
      document.body.style.backgroundImage = `url(${cdnMedia(
        "assets/media/main/bg_0.jpg"
      )})`;
    }
    this.rendersRequired.eventMenu = false;
  }
  renderAreaRequirements() {
    if (!this.rendersRequired.areaRequirements) return;
    areaMenus.combat.updateRequirements();
    areaMenus.slayer.updateRequirements();
    areaMenus.dungeon.updateRequirements();
    this.rendersRequired.areaRequirements = false;
  }
  onPlayerDeath() {
    super.onPlayerDeath();
    const shouldDeleteSave = !(
    ...
    if (this.game.currentGamemode.isPermaDeath && shouldDeleteSave) {
      sendDiscordEvent(0, this.enemy.monster);
      let killed = "Thieving";
      if (
        this.game.activeAction !== this.game.thieving &&
        this.enemy.monster !== undefined
      )
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          TotalSkillLevel:
            this.game.completion.skillProgress.currentCount.getSum(),
          killedBy: killed,
  ...
  onEnemyDeath() {
    ...
    this.game.township.tasks.updateMonsterTasks(this.enemy.monster);
    this.player.rewardGPForKill();
    if (this.selectedArea instanceof Dungeon) {
      this.dungeonProgress++;
      this.rendersRequired.location = true;
      if (this.selectedArea.dropBones) this.dropEnemyBones(this.enemy.monster);
      if (this.dungeonProgress === this.selectedArea.monsters.length) {
        this.dungeonProgress = 0;
        const lootQty = rollPercentage(
          this.player.modifiers.combatLootDoubleChance
        )
          ? 2
          : 1;
        this.selectedArea.rewards.forEach((item) => {
          if (this.bank.addItem(item, lootQty, true, true)) {
            this.addCombatStat(CombatStats.DungeonRewards, lootQty);
          }
        });
        if (
          this.selectedArea.oneTimeReward !== undefined &&
          this.game.stats.itemFindCount(this.selectedArea.oneTimeReward) <= 0
        )
          this.bank.addItem(
            this.selectedArea.oneTimeReward,
            1,
            false,
            true,
            true
        ...
        this.dropSignetHalfB(this.enemy.monster);
        this.addDungeonCompletion(this.selectedArea);
        if (this.selectedArea.fixedPetClears) {
          if (
            this.getDungeonCompleteCount(this.selectedArea) >=
            this.selectedArea.pet.weight
          )
            this.game.petManager.unlockPet(this.selectedArea.pet.pet);
        } else {
          this.game.petManager.rollForPet(this.selectedArea.pet);
        }
        this.player.rendersRequired.equipment = true;
        if (this.player.modifiers.bonusCoalOnDungeonCompletion) {
          if (rollPercentage(1))
            this.bank.addItemByID(
              "melvorD:Coal_Ore",
              this.player.modifiers.bonusCoalOnDungeonCompletion,
              true,
              true
            );
        }
        if (
          !(
            this.game.settings.autoRestartDungeon &&
            this.selectedArea.id !== "melvorF:Into_the_Mist"
          )
        ) {
          this.loot.lootAll();
          stopCombat = true;
          addModalToQueue({
            title: getLangString("COMBAT_MISC", "DUNGEON_COMPLETE"),
            text: getLangString("COMBAT_MISC", "DUNGEON_COMPLETE_TEXT"),
            imageUrl: "assets/media/skills/combat/dungeon.svg",
            imageWidth: 64,
            imageHeight: 64,
            imageAlt: getLangString("COMBAT_MISC", "DUNGEON_COMPLETE"),
          });
        }
      }
      ...
      this.rendersRequired.location = true;
      this.dungeonProgress++;
      if (this.dungeonProgress === this.eventDungeonLength) {
        this.loot.lootAll();
        stopCombat = true;
        if (!(this.selectedArea instanceof SlayerArea))
          throw new Error(
            "Error increasing event progress. Selected area is invalid."
          );
        ...
        if (this.activeEventAreas.size === 0) {
          ...
          this.bank.addItem(reward, 1, true, true, true);
          this.increaseEventProgress(this.activeEvent);
        }
      }
    } else {
      this.rewardForEnemyDeath(this.enemy.monster);
    }
    ...
    this.game.processEvent(killedEvent);
    return stopCombat;
  }
  ...
  rewardForEnemyDeath(monster) {
    if (
      this.selectedArea instanceof SlayerArea &&
      this.selectedArea.pet !== undefined
    )
      this.game.petManager.rollForPet(this.selectedArea.pet);
    this.dropEnemyBones(monster);
    this.dropSignetHalfB(monster);
    this.dropEnemyLoot(monster);
    this.dropEnemyGP(monster);
    ...
    if (this.onSlayerTask) {
      ...
      const chanceForDoubleReward =
        this.player.modifiers.increasedChanceDoubleSlayerTaskKill -
        this.player.modifiers.decreasedChanceDoubleSlayerTaskKill;
    ...
    if (slayerXPReward > 0) this.game.slayer.addXP(slayerXPReward);
  }
  dropEnemyLoot(monster) {
    ...
    if (rollPercentage(monster.lootChance)) {
      ...
      if (herbItem !== undefined) {
        if (
          rollPercentage(
            this.player.modifiers.increasedChanceToConvertSeedDrops
          )
        ) {
          item = herbItem;
          quantity += 3;
      ...
      const event = new MonsterDropEvent(
        item,
        quantity,
        herbItem !== undefined
      );
  ...
  dropSignetHalfB(monster) {
    ...
    if (rollPercentage(chance)) {
      ...
      if (this.player.modifiers.allowSignetDrops)
        itemID = "melvorD:Signet_Ring_Half_B";
      this.bank.addItemByID(itemID, 1, true, true);
      if (itemID === "melvorD:Gold_Topaz_Ring")
  ...
  dropEnemyBones(monster) {
    ...
    if (monster.bones !== undefined) {
      ...
      const item = monster.bones.item;
      if (
        item instanceof BoneItem &&
        this.player.modifiers.autoBurying > 0 &&
        this.game.prayer.isUnlocked
      ) {
        ...
        this.player.addPrayerPoints(
          applyModifier(
            itemQty * item.prayerPoints,
            this.player.modifiers.autoBurying
          )
        );
      } else {
        const autoLooted =
          this.player.modifiers.autoLooting &&
          this.bank.addItem(item, itemQty, false, true);
        if (autoLooted) {
  ...
  dropEnemyGP(monster) {
    let gpToAdd = rollInteger(monster.gpDrops.min, monster.gpDrops.max);
    gpToAdd +=
      this.player.modifiers.increasedGPFromMonstersFlat -
      this.player.modifiers.decreasedGPFromMonstersFlat;
    let gpModifier = this.player.modifiers.increasedCombatGP;
    if (this.onSlayerTask)
      gpModifier += this.player.modifiers.increasedGPFromSlayerTaskMonsters;
    if (this.enemy.isBurning)
      gpModifier += this.player.modifiers.increasedGPFromBurningMonsters;
  ...
  startEvent(event) {
    ...
    showCombatArea(CombatAreaType.Slayer);
    this.rendersRequired.eventMenu = true;
    this.renderEventMenu();
  }
  ...
  fireEventStageCompletionModal(event) {
    ...
    let title = getLangString("COMBAT_MISC", "BANE_FLED");
    const itemFlavour = getLangString("COMBAT_MISC", "IN_WAKE_FOUND");
    if (this.eventProgress === 5) {
      title = getLangString("COMBAT_MISC", "BANE_DEFEATED");
    }
    modalBody.append(
      createElement("h5", { text: itemFlavour, classList: ["text-info"] }),
      createElement("img", {
        classList: ["bank-img"],
        attributes: [["src", item.media]],
      }),
      createElement("br"),
      createElement("span", { classList: ["text-success"], text: item.name }),
      createElement("br"),
      createElement("small", {
        classList: ["text-info"],
  ...
  fireEventPassiveModal() {
    ...
    this.eventPassivesBeingSelected.forEach((passive) => {
      const selectButton = createElement("button", {
        classList: ["btn", "btn-outline-danger", "m-2"],
      });
      selectButton.style.width = "80%";
      selectButton.onclick = () => this.onPassiveSelection(passive);
      selectButton.append(
        createElement("small", { text: passive.description })
      );
      modalBody.append(selectButton);
    });
    addModalToQueue({
      title: getLangString("BANE_EVENT", "3"),
      html: modalBody,
      allowOutsideClick: false,
  ...
  showEventPassivesModal() {
    ...
    addModalToQueue({
      title: getLangString("BANE_EVENT", "4"),
      html: modalBody,
    });
  }
  showStartEventModal(event) {
    const modalBody = createElement("div");
    modalBody.append(
      createElement("h5", { text: getLangString("BANE_EVENT", "START_TEXT_0") })
    );
    modalBody.append(
      createElement("p", { text: getLangString("BANE_EVENT", "START_TEXT_1") })
    );
    if (this.game.currentGamemode.isPermaDeath) {
      modalBody.append(
        createElement("p", {
          text: getLangString("BANE_EVENT", "START_TEXT_2"),
        })
      );
    }
    for (let i = 3; i < 9; i++) {
      modalBody.append(
        createElement("p", {
          text: getLangString("BANE_EVENT", `START_TEXT_${i}`),
        })
      );
    }
    SwalLocale.fire({
      title: getLangString("BANE_EVENT", "5"),
      html: modalBody,
      confirmButtonText: getLangString("MENU_TEXT", "START"),
  ...
  showStopEventModal() {
    const modalBody = createElement("div");
    modalBody.append(
      createElement("h5", { text: getLangString("BANE_EVENT", "STOP_WARNING") })
    );
    modalBody.append(
      createElement("p", { text: getLangString("BANE_EVENT", "ARE_YOU_SURE") })
    );
    SwalLocale.fire({
      title: getLangString("BANE_EVENT", "BTN_1"),
      html: modalBody,
      confirmButtonText: getLangString("MISC_STRING", "30"),
      showCancelButton: true,
    }).then((result) => {
      if (result.isConfirmed) {
        this.stopEvent();
  ...
  onPassiveSelection(passive) {
    ...
    if (passive.id === "melvorF:EventPassive12")
      this.rendersRequired.slayerAreaEffects = true;
    Swal.close();
  }
  ...
  increaseEventProgress(event) {
    ...
    if (this.eventProgress === 5) {
      ...
      const eventDungeon = this.game.dungeons.find(
        (dungeon) => dungeon.event === event
      );
      if (eventDungeon !== undefined) this.addDungeonCompletion(eventDungeon);
      this.stopEvent();
      return;
    }
    ...
    this.renderEventAreas();
    this.eventPassivesBeingSelected = getExclusiveRandomArrayElements(
      this.availableEventPassives,
      3
    );
    this.fireEventPassiveModal();
  ...
  renderEventAreas() {
    if (this.isEventActive) {
      areaMenus.combat.updateEvent(new Map());
      areaMenus.dungeon.updateEvent(new Map());
      areaMenus.slayer.updateEvent(this.activeEventAreas);
    } else {
      areaMenus.combat.removeEvent();
      areaMenus.dungeon.removeEvent();
      areaMenus.slayer.removeEvent();
    }
  }
  selectMonster(monster, areaData) {
    if (
      !this.game.tutorial.complete &&
      !this.game.tutorial.allowedMonsters.has(monster)
    ) {
      notifyPlayer(
        this.game.attack,
        getLangString("TOASTS", "FIGHT_WRONG_MONSTER"),
        "danger"
      );
      return;
    }
  ...
  selectDungeon(dungeon) {
    if (!this.game.checkRequirements(dungeon.entryRequirements, true)) {
      return;
    }
    ...
    const canStart = this.preSelection();
    if (canStart) {
      this.selectedArea = dungeon;
      this.dungeonProgress = 0;
      this.onSelection();
    }
  }
  selectEventArea(areaData) {
    if (
      !this.game.checkRequirements(
        areaData.entryRequirements,
        true,
        areaData.slayerLevelRequired
      )
    ) {
      return;
    }
    const areaLength = this.activeEventAreas.get(areaData);
  ...
  loadNextEnemy() {
    let nextMonster;
    if (this.selectedArea instanceof Dungeon) {
      nextMonster = this.selectedArea.monsters[this.dungeonProgress];
    } else if (
      this.selectedArea instanceof CombatArea &&
      this.selectedMonster !== undefined
    ) {
      nextMonster = this.selectedMonster;
    } else {
      throw new Error(
        "Error loading next enemy. Area or Monster is not selected."
      );
    }
    ...
    if (this.activeEvent !== undefined) {
      if (
        this.atLastEventDungeon &&
        this.dungeonProgress === this.eventDungeonLength - 1
      ) {
        nextMonster =
          this.eventProgress === 4
            ? this.activeEvent.finalBossMonster
            : this.activeEvent.firstBossMonster;
      } else {
  ...
  createNewEnemy() {
    ...
    if (this.activeEvent !== undefined) {
      this.enemy.addPassives(this.eventPassives, true, false, false);
      if (
        this.selectedMonster !== this.activeEvent.firstBossMonster &&
        this.selectedMonster !== this.activeEvent.finalBossMonster
      ) {
        this.enemy.addPassives(
          this.activeEvent.enemyPassives,
          true,
          true,
          false
        );
      ...
      if (
        this.dungeonProgress ===
        this.eventDungeonLength - (this.atLastEventDungeon ? 2 : 1)
      ) {
        this.enemy.addPassives(
          this.activeEvent.bossPassives,
          true,
          true,
          false
        );
  ...
  openAreaMenu(areaType) {
    if (areaType !== this.openCombatAreaMenu) {
      if (this.openCombatAreaMenu !== CombatAreaType.None) this.closeAreaMenu();
      if (areaType === CombatAreaType.Slayer && !this.game.slayer.isUnlocked) {
        lockedSkillAlert(this.game.slayer, "SKILL_UNLOCK_EXPLORE_AREA");
        return;
      }
  ...
  closeAreaMenu() {
    ...
    $("#combat-select-" + this.openCombatAreaMenu).removeClass(
  ...
  encode(writer) {
    ...
    if (this.selectedArea !== undefined) {
      if (this.selectedArea instanceof Dungeon) {
        writer.writeUint8(CombatAreaType.Dungeon);
      } else if (this.selectedArea instanceof SlayerArea) {
        writer.writeUint8(CombatAreaType.Slayer);
      } else {
        writer.writeUint8(CombatAreaType.Combat);
      }
      writer.writeNamespacedObject(this.selectedArea);
    }
    writer.writeUint32(this.dungeonProgress);
    ...
    writer.writeMap(
      this.activeEventAreas,
      writeNamespaced,
      (monsterCount, writer) => writer.writeUint32(monsterCount)
    );
    writer.writeUint32(this.eventProgress);
    writer.writeMap(this.dungeonCompletion, writeNamespaced, (count, writer) =>
  ...
  decode(reader, version) {
    ...
    this.dungeonProgress = reader.getUint32();
    if (reader.getBoolean()) {
    ...
    this.eventPassives = reader.getArray(
      readNamespacedReject(this.game.combatPassives)
    );
    this.eventPassivesBeingSelected = reader.getSet(
      readNamespacedReject(this.game.combatPassives)
    );
      ...
      (reader) => {
        ...
        if (typeof dungeon === "string") {
          if (dungeon.startsWith("melvor"))
            return this.game.dungeons.getDummyObject(
              dungeon,
              DummyDungeon,
              this.game
            );
  ...
  deserialize(reader, version, idMap) {
    ...
    const selectedMonster = this.game.monsters.getObjectByID(
      idMap.monsters[reader.getNumber()]
    );
    ...
    if (version >= 7) {
      ...
      if (eventActive) {
        this.activeEvent = this.game.combatEvents.getObjectByID(
          idMap.combatEvents[0]
        );
      ...
      for (let i = 0; i < numAreas; i++) {
        const slayerArea = this.game.slayerAreas.getObjectByID(
          idMap.slayerAreas[reader.getNumber()]
        );
    ...
    while (this.slayerTask.completion.length < SlayerTask.data.length) {
      this.slayerTask.completion.push(0);
    }
    if (this.shouldResetEvent) this.resetEventState();
    if (this.shouldResetAction) this.resetActionState();
    if (!this.isActive) this.selectedArea = undefined;
  ...
  convertFromOldSaveFormat(saveGame, idMap) {
    ...
    this.slayerTask.convertFromOldSaveFormat(
      ...
      (_b = saveGame.slayerTaskCompletion) !== null && _b !== void 0
        ? _b
        : defaultSaveValues.slayerTaskCompletion,
      idMap
    );
  }
  convertDungeonCompletion(dungeonCompletion, idMap) {
    dungeonCompletion.forEach((count, oldDungeonID) => {
      ...
      if (dungeon === undefined)
        dungeon = this.game.dungeons.getDummyObject(
          newID,
          DummyDungeon,
          this.game
        );
...
class Timer {
  ...
  start(time, offsetByTick = false) {
    ...
    if (ticks < 1)
      throw new Error(
        `Tried to start timer: ${this.type} with invalid tick amount: ${ticks}`
      );
    this.active = true;
