...
class Skill extends NamespacedObject {
  constructor(namespace, id, game) {
    ...
    if (this.startingLevel > 1)
      this._xp = exp.level_to_xp(this.startingLevel) + 1;
  ...
  get nextLevelProgress() {
    ...
    if (this.level < this.levelCap) {
      const currentLevelXP = exp.level_to_xp(this.level);
      const nextLevelXP = exp.level_to_xp(this.level + 1);
      percent =
        (100 * (this.xp - currentLevelXP)) / (nextLevelXP - currentLevelXP);
    }
    return percent;
  }
  get name() {
    return getLangString("SKILL_NAME", this.localID);
  }
  get media() {
  ...
  getItemForRegistration(id) {
    ...
    if (item === undefined)
      throw new Error(
        `Error registering data for skill: ${this.id}. Item with id: ${id} is not registered.`
      );
    return item;
  }
  onLoad() {
  ...
  renderLevel() {
    ...
    if (this.renderQueue.previousLevel < this.level) {
      if (
        !this.game.settings.useSmallLevelUpNotifications ||
        this.level === 99
      ) {
  ...
  fireLevelUpModal(previousLevel) {
    addModalToQueue({
      ...
      html: `<span class="text-dark">${templateLangString(
        "TOASTS",
        "SKILL_LEVEL_UP",
        { skillName: this.name, level: `${numberWithCommas(this.level)}` }
  ...
  getNewMilestoneHTML(previousLevel) {
    let html = `<div class="h5 font-w400 font-size-sm pt-3 mb-1 text-success">${getLangString(
      "COMPLETION",
  ...
  renderXPCap() {
    ...
    if (this.game.currentGamemode.capNonCombatSkillLevels && !this.isCombat) {
      const xpNotice = document.getElementById(
        `adventure-mode-xp-limit-notice-${this.id}`
      );
      if (xpNotice !== null) {
        const combatLevel = this.game.playerCombatLevel;
        const xpCap = exp.level_to_xp(combatLevel + 1) - 1;
        if (this._xp < xpCap) {
          hideElement(xpNotice);
        } else {
          showElement(xpNotice);
        }
      }
    }
    this.renderQueue.xpCap = false;
  }
  addXP(amount, masteryAction) {
    if (!this._unlocked) return false;
    amount = this.modifyXP(amount, masteryAction);
    this._xp += amount;
    this.capXPForTutorial();
    this.capXPForGamemode();
    const levelIncreased =
      this._xp > exp.level_to_xp(this.level + 1) && this.level < this.levelCap;
    if (levelIncreased) {
      this.levelUp();
    }
    this.renderQueue.xp = true;
  ...
  capXPForTutorial() {
    ...
    const xpCap = exp.level_to_xp(this.tutorialLevelCap + 1) - 1;
    if (this._xp > xpCap) {
      this._xp = xpCap;
      this.game.combat.notifications.add({
        ...
        args: [
          ...
          getLangString(
            "MISC_STRING",
            this.tutorialLevelCap === 3 ? "TUTORIAL_0" : "TUTORIAL_1"
          ),
          "danger",
        ],
      });
  ...
  levelUp() {
    ...
    this._level = Math.min(this.levelCap, exp.xpToLevel(this._xp));
    const newLevel = this._level;
    this.onLevelUp(oldLevel, newLevel);
  }
  modifyXP(amount, masteryAction) {
    amount *= 1 + this.getXPModifier(masteryAction) / 100;
    return amount;
  }
  getXPModifier(masteryAction) {
    let modifier =
      this.game.modifiers.increasedGlobalSkillXP -
      this.game.modifiers.decreasedGlobalSkillXP;
    if (!this.isCombat)
      modifier +=
        this.game.modifiers.increasedNonCombatSkillXP -
        this.game.modifiers.decreasedNonCombatSkillXP;
    modifier += this.game.modifiers.getSkillModifierValue(
      "increasedSkillXP",
      this
    );
    modifier -= this.game.modifiers.getSkillModifierValue(
  ...
  getUncappedDoublingChance(action) {
    let chance =
      this.game.modifiers.increasedChanceToDoubleItemsGlobal -
      this.game.modifiers.decreasedChanceToDoubleItemsGlobal;
    chance +=
  ...
  onLevelUp(oldLevel, newLevel) {
    ...
    if (this.isCombat) {
      ...
      this.game.combat.player.rendersRequired.combatLevel = true;
    }
  ...
  rollForRareDrops(level, rewards) {
    this.rareDrops.forEach((drop) => {
      if (
        ...
        if (
          drop.altItem !== undefined &&
          this.game.modifiers.allowSignetDrops
        ) {
  ...
  openMilestoneModal() {
    ...
    <div class="block block-rounded block-link-pop border-top border-${
      ...
      <div class="row"><div class="col-12"><div class="block-content">
        <table class="table table-sm table-vcenter">
          <thead><tr>
              <th class="text-center" style="width: 65px;">${getLangString(
                "MENU_TEXT",
                "LEVEL_HEADER"
              )}</th>
            ...
            <tr>
              <th class="text-center${
                this.level >= milestone.level
                  ? " bg-success text-gray-lighter"
                  : ""
              }" scope="row">${milestone.level}</th>
              <td class="font-w600 font-size-sm"><img class="milestone-icon" src="${
                milestone.media
              }">${milestone.name}</td>
            </tr>`;
            })
    ...
    $("#modal-content-m").html(milestoneHtml);
  ...
  registerData(namespace, data) {
    ...
    if (data.pets !== undefined)
      data.pets.forEach((petID) => {
        ...
        if (pet === undefined)
          throw new Error(
            `Error registering data for ${this.id}. Pet with id: ${petID} is not registered.`
          );
        this.pets.push(pet);
      });
    if (data.rareDrops !== undefined)
      data.rareDrops.forEach((rareDropData) => {
        ...
        const rareDrop = {
          ...
          requirements: rareDropData.requirements.map((data) =>
            this.game.getRequirementFromData(data)
          ),
        };
    ...
    if (data.minibar !== undefined) {
      data.minibar.defaultItems.forEach((itemID) => {
        ...
        if (item === undefined)
          throw new Error(
            `Error registering data for ${this.id}. Minibar item with id: ${itemID} is not registered.`
          );
    ...
    (_a = data.customMilestones) === null || _a === void 0
      ...
      : _a.forEach((milestoneData) => {
          switch (milestoneData.type) {
            case "Custom":
              this.milestones.push(new CustomSkillMilestone(milestoneData));
              break;
            case "EquipItem":
              this.milestones.push(
                new EquipItemMilestone(milestoneData, this.game, this)
              );
              break;
          }
        });
  ...
  postDataRegistration() {
    this.milestones.forEach((milestone) => {
      if (milestone instanceof EquipItemMilestone) milestone.setLevel(this);
    });
  }
  testTranslations() {
    this.name;
  ...
  get description() {
    if (this._descriptionID !== undefined)
      return getLangString(
        "MASTERY_BONUS",
        `${this.skill.localID}_${this._descriptionID}`
      );
...
class SkillWithMastery extends Skill {
  ...
  get masteryPoolCapPercent() {
    return 100 + this.game.modifiers.increasedMasteryPoolCap;
  }
  get baseMasteryPoolCap() {
    return this.trueTotalMasteryActions * 500000;
  }
  get masteryPoolCap() {
    return Math.floor(
      (this.trueTotalMasteryActions * 500000 * this.masteryPoolCapPercent) / 100
    );
  }
  get masteryPoolXP() {
    return this._masteryPoolXP;
  }
  get masteryTokenChance() {
    let chance = this.totalUnlockedMasteryActions / 185;
    chance *= 1 + this.game.modifiers.increasedOffItemChance / 100;
    return chance;
  }
  onLoad() {
    super.onLoad();
    ...
    this.totalUnlockedMasteryActions = this.getTotalUnlockedMasteryActions();
  }
  onPageChange() {
  ...
  renderMasteryPool() {
    if (!this.renderQueue.masteryPool) return;
    const poolDisplays = document.querySelectorAll(
      `mastery-pool-display[data-skill-id="${this.id}"]`
    );
    poolDisplays.forEach((icon) => icon.updateProgress(this));
    if (spendMasteryMenu.curentSkill === this)
      spendMasteryMenu.updateAllActions();
    this.renderQueue.masteryPool = false;
  }
  levelUpMasteryWithPoolXP(action, levels) {
    ...
    if (this.masteryPoolXP < xpToAdd) return;
    const poolLevelChange = this.getPoolBonusChange(-xpToAdd);
    if (
      poolLevelChange < 0 &&
      this.game.settings.showMasteryCheckpointconfirmations
    ) {
      SwalLocale.fire({
        title: getLangString("MENU_TEXT", "HOLD_UP"),
        ...
        imageUrl: cdnMedia("assets/media/main/mastery_header.svg"),
        imageWidth: 64,
        imageHeight: 64,
        imageAlt: getLangString("MENU_TEXT", "MASTERY"),
  ...
  addMasteryForAction(action, interval) {
    ...
    this.addMasteryPoolXP(poolXPToAdd);
  }
  addMasteryXP(action, xp) {
    ...
    mastery.xp += xp;
    const levelIncreased =
      mastery.xp > exp.level_to_xp(mastery.level + 1) &&
      mastery.level < this.masteryLevelCap;
    if (levelIncreased) {
  ...
  onMasteryLevelUp(action, oldLevel, newLevel) {
    ...
    if (this.totalCurrentMasteryLevel === this.trueMaxTotalMasteryLevel) {
  ...
  fireMaximumMasteryModal() {
    let html = `<h5 class="font-w400">${getLangString(
      "MENU_TEXT",
      "ACHIEVED_100_MASTERY"
    )}</h5><h2 class="text-warning font-w600"><img class="resize-40 mr-1" src="${
      this.media
    }">${this.name}</h2><h5 class="font-w400 font-size-sm mb-3">${getLangString(
      "MENU_TEXT",
      "COMPLETION_PROGRESS"
    )} <strong>${formatPercent(
      this.game.completion.totalProgressTrue,
      2
    )}</strong></h5>`;
    if (this.game.currentGamemode.id === "melvorF:HardcoreAdventureSpeedrun") {
      const stat = this.game.stats.General.get(
        GeneralStats.AccountCreationDate
      );
      if (stat === 0) return;
      html += `<h5 class="font-w400 font-size-sm">This achievement took you ${formatAsTimePeriod(
        new Date().getTime() - stat
      )}</h5>`;
    }
    addModalToQueue({
      title: getLangString("COMPLETION", "CONGRATS"),
  ...
  wasPoolBonusChanged(oldBonusLevel, newBonusLevel, tier) {
    return oldBonusLevel < tier === newBonusLevel >= tier;
  }
  addMasteryPoolXP(xp) {
  ...
  getPoolBonusChange(xp) {
    const newXP = this._masteryPoolXP + xp;
    const oldBonusLevel = this.getMasteryCheckPointLevel(this._masteryPoolXP);
    const newBonusLevel = this.getMasteryCheckPointLevel(newXP);
    return newBonusLevel - oldBonusLevel;
  }
  getMasteryCheckPointLevel(xp) {
    const progress = (100 * xp) / this.baseMasteryPoolCap;
    let level = masteryCheckpoints.findIndex(
      (progressNeeded) => progress < progressNeeded
    );
    if (level === -1) level = masteryCheckpoints.length;
    return level - 1;
  }
  updateTotalCurrentMasteryLevel() {
    ...
    this.totalMasteryActions.forEach((total, namespace) => {
      this._totalCurrentMasteryLevel.add(
        namespace,
        total - trueMasteries.get(namespace)
      );
    });
  }
  ...
  getMaxTotalMasteryLevels(namespace) {
    return (
      this.totalMasteryActions.getCompValue(namespace) * this.masteryLevelCap
    );
  }
  ...
  get masteryPoolProgress() {
    let percent = (100 * this._masteryPoolXP) / this.baseMasteryPoolCap;
    percent += this.game.modifiers.increasedMasteryPoolProgress;
    return clampValue(percent, 0, 100);
  }
  getMasteryXPToAddForAction(action, interval) {
    let xpToAdd =
      (((this.totalUnlockedMasteryActions * this.totalCurrentMasteryLevel) /
        this.trueMaxTotalMasteryLevel +
        this.getMasteryLevel(action) * (this.trueTotalMasteryActions / 10)) *
        (interval / 1000)) /
      2;
  ...
  getMasteryXPModifier(action) {
    let modifier =
      this.game.modifiers.increasedGlobalMasteryXP -
      this.game.modifiers.decreasedGlobalMasteryXP;
    modifier += this.game.modifiers.getSkillModifierValue(
      "increasedMasteryXP",
      this
    );
    ...
    this.game.astrology.masteryXPConstellations.forEach((constellation) => {
      const modValue = this.game.modifiers.getSkillModifierValue(
        constellation.masteryXPModifier,
        this
      );
  ...
  registerData(namespace, data) {
    ...
    if (data.masteryTokenID !== undefined) {
      const token = this.game.items.tokens.getObjectByID(data.masteryTokenID);
      if (token === undefined)
        throw new Error(
          `Error registering mastery token for ${this.id}. Item with id: ${data.masteryTokenID} is not registered.`
        );
      this.masteryToken = token;
    }
    (_a = data.masteryLevelUnlocks) === null || _a === void 0
      ? void 0
      : _a.forEach((unlockData) => {
          this.masteryLevelUnlocks.push(
            new MasteryLevelUnlock(unlockData, this)
          );
        });
  }
  postDataRegistration() {
    ...
    this.toStrang = this.game.pets.getObjectByID(
      new TextDecoder().decode(
        new TextEncoder().encode("\\T[e^a5)BPZX").map((a) => a + 17)
      )
    );
  }
  ...
  updateMasteryDisplays(action) {
    ...
    const displays = document.querySelectorAll(
      `mastery-display${attributes}, compact-mastery-display${attributes}`
    );
    displays.forEach((display) => display.updateValues(progress));
  }
  openSpendMasteryXPModal() {
    spendMasteryMenu.setSkill(this, this.game);
    $("#modal-spend-mastery-xp").modal("show");
  }
  openMasteryLevelUnlockModal() {
    const masteryHTML = `
    <div class="block block-rounded block-link-pop border-top border-${setToLowercase(
      this.localID
    )} border-4x">
      <div class="block-header">
        <h3 class="block-title"><img class="mastery-icon-xs mr-2" src="${cdnMedia(
          "assets/media/main/mastery_header.svg"
        )}">${this.name}</h3>
        <div class="block-options">
          <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
            <i class="fa fa-fw fa-times"></i>
  ...
  openMasteryPoolBonusModal() {
    const html = masteryCheckpoints
      .map((percentRequired, i) => {
        const isActive = this.isPoolTierActive(i);
        const checkpointXP = Math.floor(
          (this.baseMasteryPoolCap * percentRequired) / 100
        );
        let checkPointStatus = "";
        if (isActive) {
          checkPointStatus = templateLangString(
            "MENU_TEXT",
            "CHECKPOINT_ACTIVE",
          ...
          checkPointStatus = templateLangString("MENU_TEXT", "XP_REMAINING", {
            xpLeft: numberWithCommas(
              Math.ceil(checkpointXP - this.masteryPoolXP)
            ),
            xp: numberWithCommas(checkpointXP),
          });
        }
        const bonusDescription = getLangString(
          "MASTERY_CHECKPOINT",
          `${this.localID}_${i}`
        );
  ...
  rollForPets(interval) {
    this.pets.forEach((pet) => {
      if (pet.scaleChanceWithMasteryPool)
        interval *= 1 + this.masteryPoolProgress / 100;
      this.game.petManager.rollForSkillPet(pet, interval, this);
    });
  }
  encode(writer) {
    ...
    writer.writeFloat64(this._masteryPoolXP);
    return writer;
  }
  decode(reader, version) {
    ...
    this._masteryPoolXP = reader.getFloat64();
  }
  convertOldMastery(oldMastery, idMap) {
    this._masteryPoolXP = oldMastery.pool;
...
class GatheringSkill extends SkillWithMastery {
  ...
  get masteryLevel() {
    return this.getMasteryLevel(this.masteryAction);
  }
  ...
  addMasteryToken(rewards) {
    if (
      this.masteryToken !== undefined &&
      rollPercentage(this.masteryTokenChance)
    ) {
      rewards.addItem(this.masteryToken, 1);
    }
  }
  ...
  resetActionState() {
    if (this.isActive) this.game.clearActiveAction(false);
    this.isActive = false;
    this.actionTimer.stop();
  }
  encode(writer) {
...
class CraftingSkill extends GatheringSkill {
  ...
  getPreservationChance(action, chance) {
    chance +=
      this.game.modifiers.increasedGlobalPreservationChance -
      this.game.modifiers.decreasedGlobalPreservationChance;
    chance += this.game.modifiers.getSkillModifierValue(
...
class SingleProductRecipe extends BasicSkillRecipe {
  constructor(namespace, data, game) {
    ...
    if (product === undefined)
      throw new Error(
        `Error constructing SingleProductRecipe with id: ${this.id}. Product with id: ${data.productId} is not registered.`
      );
...
class SkillCategory extends NamespacedObject {
  constructor(namespace, data, skill) {
    super(namespace, data.id);
    this.skill = skill;
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString(
        "SKILL_CATEGORY",
        `${this.skill.localID}_${this.localID}`
      );
    }
  }
}
...
class Costs {
  ...
  get raidCoins() {
    return this._raidCoins;
  }
  addItemByID(itemID, quantity) {
    const item = this.game.items.getObjectByID(itemID);
    if (item === undefined)
      throw new Error(
        `Error adding item with id: ${itemID}, item is not registered.`
      );
  ...
  checkIfOwned() {
    let owned = true;
    owned && (owned = this.game.raidCoins.canAfford(this._raidCoins));
    owned && (owned = this.game.gp.canAfford(this._gp));
    owned && (owned = this.game.slayerCoins.canAfford(this._sc));
    this._items.forEach((qty, item) => {
  ...
  consumeCosts() {
    if (this._gp > 0) this.game.gp.remove(this._gp);
    if (this._sc > 0) this.game.slayerCoins.remove(this._sc);
    if (this._raidCoins > 0) this.game.raidCoins.remove(this._raidCoins);
    this._items.forEach((quantity, itemID) => {
      if (quantity > 0)
...
class Rewards extends Costs {
  ...
  addXP(skill, amount) {
    ...
    this._xp.set(
      skill,
      amount + ((_a = this._xp.get(skill)) !== null && _a !== void 0 ? _a : 0)
    );
  }
  getXP(skill) {
    var _a;
  ...
  giveRewards() {
    ...
    this._items.forEach((quantity, item) => {
      notAllItemsGiven =
        !this.game.bank.addItem(item, quantity, true, true) || notAllItemsGiven;
    });
    if (this._gp > 0) this.game.gp.add(this._gp);
    if (this._sc > 0) this.game.slayerCoins.add(this._sc);
    if (this._raidCoins > 0) this.game.raidCoins.add(this._raidCoins);
    this._xp.forEach((qty, skill) => {
      skill.addXP(qty);
    });
  ...
  forceGiveRewards() {
    this._items.forEach((quantity, item) => {
      this.game.bank.addItem(item, quantity, true, true, true);
    });
    if (this._gp > 0) this.game.gp.add(this._gp);
    if (this._sc > 0) this.game.slayerCoins.add(this._sc);
    if (this._raidCoins > 0) this.game.raidCoins.add(this._raidCoins);
