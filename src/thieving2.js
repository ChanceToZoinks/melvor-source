...
class ThievingNPC extends BasicSkillRecipe {
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString("THIEVING", `NPC_NAME_${this.localID}`);
    }
  }
  get media() {
...
class ThievingArea extends NamespacedObject {
  constructor(namespace, data, game, thieving) {
    super(namespace, data.id);
    this._name = data.name;
    this.npcs = data.npcIDs.map((npcID) => {
      const npc = thieving.actions.getObjectByID(npcID);
      if (npc === undefined)
        throw new Error(
          `Error constructing ThievingArea with id: ${this.id}. NPC with id: ${npcID} is not registered.`
        );
      return npc;
    });
  ...
  get name() {
    if (this.isModded) {
      ...
      return getLangString("THIEVING", `AREA_NAME_${this.localID}`);
    }
  }
}
...
class Thieving extends GatheringSkill {
  constructor(namespace, game) {
    super(namespace, "Thieving", game);
    this.stunTimer = new Timer("Skill", () => this.stunned());
    this._media = "assets/media/skills/thieving/thieving.svg";
    ...
    this.stunState = 0;
    this.areas = new NamespaceRegistry(game.registeredNamespaces);
  }
  ...
  get areaUniqueChance() {
    ...
    chance += this.game.modifiers.increasedThievingAreaUniqueChance;
    chance -= this.game.modifiers.decreasedThievingAreaUniqueChance;
    return chance;
  }
  get masteryAction() {
  ...
  get actionLevel() {
    if (this.currentNPC === undefined) return 0;
  ...
  get avoidStunChance() {
    let chance = 0;
    chance += this.game.modifiers.increasedChanceToAvoidThievingStuns;
    return chance;
  }
  get stunInterval() {
    return this.getStunInterval();
  }
  registerData(namespace, data) {
    var _a, _b, _c;
    super.registerData(namespace, data);
    (_a = data.npcs) === null || _a === void 0
      ? void 0
      : _a.forEach((npc) => {
          this.actions.registerObject(
            new ThievingNPC(namespace, npc, this.game)
          );
        });
    (_b = data.areas) === null || _b === void 0
      ? void 0
      : _b.forEach((area) => {
          this.areas.registerObject(
            new ThievingArea(namespace, area, this.game, this)
          );
        });
    (_c = data.generalRareItems) === null || _c === void 0
      ? void 0
      : _c.forEach((rareData) => {
          const generalRare = {
          ...
          if (rareData.npcs !== undefined)
            generalRare.npcs = new Set(
              rareData.npcs.map((id) => {
                const npc = this.actions.getObjectByID(id);
                if (npc === undefined)
                  throw new Error(
                    `Error registering general rare item. NPC with id: ${id} is not registered.`
                  );
                return npc;
              })
            );
    ...
    if (data.entLeprechaunItem !== undefined)
      this.entLeprechaunItem = this.getItemForRegistration(
        data.entLeprechaunItem
      );
    if (data.crowLeprechaunItem !== undefined)
      this.crowLeprechaunItem = this.getItemForRegistration(
        data.crowLeprechaunItem
      );
    if (data.bearLeprechaunItem !== undefined)
      this.bearLeprechaunItem = this.getItemForRegistration(
        data.bearLeprechaunItem
      );
    if (data.easterEgg !== undefined) {
      ...
      if (!(equipped instanceof EquipmentItem))
        throw new Error(
          `Error registering easter egg. ${data.easterEgg.equippedID} is not equipment. Blame Coolrox.`
        );
      this.easterEgg = {
        equipped,
  ...
  postDataRegistration() {
    ...
    this.game.smithing.actions.forEach((recipe) => {
      if (recipe.category.id === "melvorD:Bars") {
  ...
  Selected Area: ${
    (_a = this.currentArea) === null || _a === void 0 ? void 0 : _a.id
  }
  Selected NPC: ${
    (_b = this.currentNPC) === null || _b === void 0 ? void 0 : _b.id
  }
  ...
  renderNPCUnlock() {
    if (!this.renderQueue.npcUnlock) return;
    thievingMenu.updateNPCsForLevel(this.level);
  ...
  decode(reader, version) {
    ...
    if (this.isActive) {
      ...
      if (typeof area === "string") this.shouldResetAction = true;
      else this.currentArea = area;
      const npc = reader.getNamespacedObject(this.actions);
      if (typeof npc === "string") this.shouldResetAction = true;
      else this.currentNPC = npc;
    }
  ...
  deserialize(reader, version, idMap) {
    ...
    if (version >= 17) {
      ...
      for (let i = 0; i < numHiddenAreas; i++) {
        ...
        if (area !== undefined) this.hiddenAreas.add(area);
      }
    }
    if (this.shouldResetAction) this.resetActionState();
  }
  ...
  onLoad() {
    ...
    if (
      ...
      thievingMenu.selectNPC(this.currentNPC, this.currentArea);
  ...
  notifyStunBlockingAction() {
    this.game.combat.notifications.add({
      type: "Player",
      args: [this, getLangString("TOASTS", "CANNOT_WHILE_STUNNED"), "danger"],
    });
  }
  renderMenu() {
    if (this.renderQueue.menu) {
      thievingMenu.updateAllAreaPanels();
      thievingMenu.updateNPCButtons();
    }
  ...
  renderStopButton() {
    if (this.renderQueue.stopButton) {
      if (this.isActive && this.currentArea !== undefined)
        thievingMenu.setStopButton(this.currentArea);
      else thievingMenu.removeStopButton();
    }
  ...
  renderProgressBar() {
    ...
    if (this.lastActiveAreaProgressBar !== undefined) {
      (_a = thievingMenu.getProgressBar(this.lastActiveAreaProgressBar)) ===
        null || _a === void 0
        ? void 0
        : _a.stopAnimation();
      this.lastActiveAreaProgressBar = undefined;
    }
    if (this.currentArea === undefined) return;
  ...
  startThieving(area, npc) {
    ...
    this.currentNPC = npc;
    if (this.level >= this.currentNPC.level) {
      this.start();
      this.renderQueue.stopButton = true;
    }
  }
  getStunInterval() {
    ...
    let modifier = 0;
    modifier -= this.game.modifiers.decreasedThievingStunIntervalPercent;
    modifier += this.game.modifiers.increasedThievingStunIntervalPercent;
  ...
  getNPCSuccessRate(npc) {
    return Math.min(
      100,
      (100 * (100 + this.getStealthAgainstNPC(npc))) / (100 + npc.perception)
    );
  }
  ...
  getStealthAgainstNPC(npc) {
    const mastery = this.getMasteryLevel(npc);
    let stealth = this.level + mastery;
    if (mastery >= 99) stealth += 75;
    if (this.isPoolTierActive(0)) stealth += 30;
    if (this.isPoolTierActive(3)) stealth += 100;
    stealth += this.game.modifiers.increasedThievingStealth;
    stealth -= this.game.modifiers.decreasedThievingStealth;
    return stealth;
  }
  getFlatIntervalModifier(action) {
    let modifier = super.getFlatIntervalModifier(action);
    if (this.getMasteryLevel(action) >= 50) modifier -= 200;
    if (this.isPoolTierActive(1)) modifier -= 200;
    return modifier;
  }
  getPercentageIntervalModifier(action) {
    let modifier = super.getPercentageIntervalModifier(action);
    if (action.id === "melvorF:FISHERMAN")
  ...
  getNPCDoublingChance(npc) {
    return Math.min(
      100,
      this.getNPCSleightOfHand(npc) + this.getDoublingChance(npc)
    );
  }
  getNPCGPRange(npc) {
    let minGP = this.getNPCMinGPRoll(npc);
    let maxGP = npc.maxGP;
    const gpFlat =
      this.game.modifiers.increasedGPFromThievingFlat -
      this.game.modifiers.decreasedGPFromThievingFlat;
    const gpModifier = this.getNPCGPMultiplier(npc);
    const modGp = (gp) => {
      gp += gpFlat;
      gp *= 1 + gpModifier / 100;
      gp = Math.floor(gp);
  ...
  getNPCMinGPRoll(npc) {
    return Math.max(
      ...
      Math.floor(
        (npc.maxGP *
          (this.game.modifiers.increasedMinThievingGP -
            this.game.modifiers.decreasedMinThievingGP)) /
          100
      )
    );
  }
  getNPCGPMultiplier(npc) {
    let increasedGPModifier =
      this.game.modifiers.increasedGPGlobal -
      this.game.modifiers.decreasedGPGlobal;
    increasedGPModifier +=
      this.game.modifiers.increasedGPFromThieving -
      this.game.modifiers.decreasedGPFromThieving;
    increasedGPModifier += 1 + this.getMasteryLevel(npc);
    if (this.isPoolTierActive(2)) increasedGPModifier += 100;
    return increasedGPModifier;
  }
  getXPModifier(masteryAction) {
    let modifier = super.getXPModifier(masteryAction);
    if (this.isPoolTierActive(0)) modifier += 3;
    return modifier;
  }
  getMasteryXPModifier(action) {
    let modifier = super.getMasteryXPModifier(action);
    if (this.isPoolTierActive(1)) modifier += 3;
    return modifier;
  }
  stunned() {
    this.stunState = 0;
    this.addStat(
      ...
      this.stunTimer.maxTicks * TICK_INTERVAL
    );
    if (this.game.settings.continueThievingOnStun) this.startActionTimer();
    else this.stop();
  }
  get isStunned() {
    return this.stunState !== 0;
  }
  get actionRewards() {
    ...
    const actionEvent = new ThievingActionEvent(
      this,
      currentNPC,
      this.currentArea
    );
    ...
    if (this.stunState === 0) {
      ...
      let skillXPModifier = 0;
      if (rollPercentage(this.getNPCDoublingChance(currentNPC)))
        quantityMultiplier *= 2;
      quantityMultiplier *= Math.pow(
        2,
        this.game.modifiers.getSkillModifierValue("doubleItemsSkill", this)
      );
      if (this.game.modifiers.summoningSynergy_Leprechaun_Devil > 0) {
        const { itemID } = this.game.getItemFromLootTable(Thieving.DevilTable);
        switch (itemID) {
          case 0:
            gpMultiplier *= 2;
            break;
      ...
      if (
        this.game.modifiers.summoningSynergy_Ent_Leprechaun > 0 &&
        currentNPC.id === "melvorF:LUMBERJACK" &&
        this.entLeprechaunItem !== undefined
      ) {
        giveGP = false;
        rewards.addItem(
          this.entLeprechaunItem,
          this.game.modifiers.summoningSynergy_Ent_Leprechaun
        );
        this.game.stats.Thieving.add(
          ThievingStats.CommonDrops,
          this.game.modifiers.summoningSynergy_Ent_Leprechaun
        );
      }
      if (currentNPC.id === "melvorF:MINER") {
        if (
          this.game.modifiers.summoningSynergy_Mole_Leprechaun > 0 &&
          rollPercentage(this.game.modifiers.summoningSynergy_Mole_Leprechaun)
        )
          rewards.addItem(this.game.randomGemTable.getDrop().item, 1);
        this.game.stats.Thieving.inc(ThievingStats.CommonDrops);
        if (
          this.game.modifiers.increasedRuneEssenceThievingMiner > 0 &&
          this.crowLeprechaunItem !== undefined
        )
          rewards.addItem(
            this.crowLeprechaunItem,
            this.game.modifiers.increasedRuneEssenceThievingMiner
          );
        this.game.stats.Thieving.add(
          ThievingStats.CommonDrops,
          this.game.modifiers.increasedRuneEssenceThievingMiner
      ...
      if (
        this.game.modifiers.summoningSynergy_Octopus_Leprechaun > 0 &&
        currentNPC.id === "melvorF:FISHERMAN"
      ) {
        baseQuantityBonus += 2;
      }
      if (giveItems) {
        if (currentNPC.lootTable.size > 0 && rollPercentage(this.itemChance)) {
          ...
          if (this.game.modifiers.increasedThievingAutoSellPrice > 0) {
            const gpToAdd =
              item.sellsFor *
              this.game.modifiers.increasedThievingAutoSellPrice;
            rewards.addGP(gpToAdd);
            this.game.stats.Thieving.add(ThievingStats.GPStolen, gpToAdd);
          } else {
            const qtyToAdd =
              (quantity + baseQuantityBonus) * quantityMultiplier;
            rewards.addItem(item, qtyToAdd);
            this.game.stats.Thieving.add(ThievingStats.CommonDrops, qtyToAdd);
          }
          if (
            currentNPC.id === "melvorF:MINER" &&
            this.game.modifiers.increasedRandomBarChanceThievingMiner > 0 &&
            rollPercentage(
              this.game.modifiers.increasedRandomBarChanceThievingMiner
            )
          ) {
          ...
          if (
            currentNPC.id === "melvorF:BOB_THE_FARMER" &&
            this.bearLeprechaunItem !== undefined &&
            this.game.modifiers.increasedHerbSackChanceThievingFarmer > 0 &&
            rollPercentage(
              this.game.modifiers.increasedHerbSackChanceThievingFarmer
            )
          ) {
        ...
        this.currentArea.uniqueDrops.forEach((drop) => {
          const chanceMult = this.isPoolTierActive(3) ? 3 : 1;
          if (rollPercentage(this.areaUniqueChance * chanceMult)) {
            const qty =
              (drop.quantity + baseQuantityBonus) * quantityMultiplier;
            rewards.addItem(drop.item, qty);
            this.game.stats.Thieving.add(ThievingStats.AreaDrops, qty);
            this.addJesterHatGP(drop.item, rewards);
          }
        });
        if (
          currentNPC.uniqueDrop !== undefined &&
          rollPercentage(this.getNPCPickpocket(currentNPC))
        ) {
          const qty =
            (currentNPC.uniqueDrop.quantity + baseQuantityBonus) *
            quantityMultiplier;
          rewards.addItem(currentNPC.uniqueDrop.item, qty);
          this.game.stats.Thieving.add(ThievingStats.NpcDrops, qty);
          this.addJesterHatGP(currentNPC.uniqueDrop.item, rewards);
        }
      }
      if (
        this.easterEgg !== undefined &&
        currentNPC.id === "melvorF:LUMBERJACK" &&
        this.masteryLevel >= 99 &&
        this.masteryPoolProgress >= 100 &&
        this.game.combat.player.equipment.checkForItem(
          this.easterEgg.equipped
        ) &&
      ...
      if (giveGP) {
        const minRoll = this.getNPCMinGPRoll(currentNPC);
        let gpToAdd = rollInteger(minRoll, currentNPC.maxGP);
        gpToAdd +=
          this.game.modifiers.increasedGPFromThievingFlat -
          this.game.modifiers.decreasedGPFromThievingFlat;
        const increasedGPModifier = this.getNPCGPMultiplier(currentNPC);
      ...
      rewards.addXP(this, baseXP);
      this.addCommonRewards(rewards);
    } else if (this.stunState === 1) {
      actionEvent.successful = false;
      let damageDealt = rollInteger(
        1,
        Math.floor(currentNPC.maxHit * numberMultiplier)
      );
      damageDealt *= 1 - this.game.combat.player.stats.damageReduction / 100;
      damageDealt = Math.floor(damageDealt);
      if (
        currentNPC.id === "melvorF:CHEF" &&
        this.game.modifiers.thievingChefNoDamage > 0
      ) {
        damageDealt = 0;
      }
      this.addStat(ThievingStats.DamageTakenFromNPCs, damageDealt);
      this.game.combat.player.damage(damageDealt, "Attack", true);
      this.game.combat.notifications.add({ type: "Stun", args: [damageDealt] });
    } else {
      this.stunState = 0;
    }
  ...
  addJesterHatGP(item, rewards) {
    if (
      this.game.combat.player.equipment.checkForItemID("melvorF:Jesters_Hat")
    ) {
      const percent = rollInteger(50, 300);
      const gpToAdd = Math.floor((item.sellsFor * percent) / 100);
      rewards.addGP(gpToAdd);
      this.game.stats.Thieving.add(ThievingStats.GPStolen, gpToAdd);
    }
  }
  ...
  preAction() {
    if (this.currentNPC === undefined)
      throw new Error(
        "Error during Thieving pre-action stage. No NPC is selected."
      );
    if (!rollPercentage(this.getNPCSuccessRate(this.currentNPC))) {
      if (!rollPercentage(this.avoidStunChance)) {
        this.stunState = 1;
      } else this.stunState = 2;
