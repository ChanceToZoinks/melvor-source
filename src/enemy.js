...
class Enemy extends Character {
  ...
  computeAttackSelection() {
    ...
    else this.availableAttacks = [];
    const totalChance = this.availableAttacks.reduce(
      (prev, attack) => prev + attack.chance,
      0
    );
    if (totalChance < 100)
  ...
  addGamemodeModifiers() {
    this.modifiers.addModifiers(this.game.currentGamemode.enemyModifiers);
  }
  getAccuracyValues() {
  ...
  modifyDamageReduction(reduction) {
    if (this.manager.fightInProgress) {
      reduction -= this.target.modifiers.decreasedEnemyDamageReduction;
      reduction += this.target.modifiers.increasedEnemyDamageReduction;
    }
  ...
  processDeath() {
    ...
    this.state = EnemyState.Dead;
    this.rendersRequired.image = true;
  ...
  render() {
    ...
    if (this.rendersRequired.image) this.renderImageAndName();
    if (this.rendersRequired.attacks || this.rendersRequired.passives)
      this.renderAttacksAndPassives();
    super.render();
  }
  renderAttacksAndPassives() {
    if (this.state === EnemyState.Alive) {
      combatMenus.enemyAttackPassive.render(
        this.passives,
        this.availableAttacks,
        this
      );
    } else {
      combatMenus.enemyAttackPassive.hide();
    }
    this.rendersRequired.attacks = false;
    this.rendersRequired.passives = false;
  }
  renderDamageValues() {
    if (this.state === EnemyState.Alive)
      combatMenus.enemyAttackPassive.renderMaxhits(this.availableAttacks, this);
    super.renderDamageValues();
  }
  renderLevels() {
    if (this.state === EnemyState.Alive && this.monster !== undefined) {
      const monster = this.monster;
      this.statElements.levels["Combat"].forEach(
        (elem) => (elem.textContent = `${monster.combatLevel}`)
      );
      Object.entries(this.levels).forEach(([key, level]) => {
        this.statElements.levels[key].forEach(
          (elem) => (elem.textContent = `${level}`)
        );
      });
    } else {
      Object.values(this.statElements.levels).forEach((elems) => {
  ...
  renderImageAndName() {
    ...
    switch (this.state) {
      case EnemyState.Alive: {
        ...
        image.src = this.monster.media;
        if (
          this.game.settings.enableEyebleachMode &&
          this.game.combat.spiderLairMonsters.includes(this.monster)
        ) {
          image.src = cdnMedia(
            `assets/media/monsters/eyebleach_${
        ...
        for (const [passive] of this.passives) {
          if (passive.id === "melvorF:Afflicted") {
            name = templateLangString("COMBAT_MISC", "AFFLICTED_NAME", {
              monsterName: name,
            });
            image.style.filter = "saturate(15%)";
            break;
          }
          if (passive.id === "melvorF:ControlledAffliction") {
            image.style.filter = "saturate(15%)";
            break;
          }
        }
    ...
    this.rendersRequired.image = false;
  }
  renderStats() {
    this.statElements.attackType.src = this.getAttackTypeMedia(this.attackType);
    if (this.state === EnemyState.Alive) {
      super.renderStats();
    } else {
      this.renderNoStats();
  ...
  getAttackTypeMedia(attackType) {
    let attackTypeMedia = "";
    switch (attackType) {
      case "melee":
        attackTypeMedia = this.game.attack.media;
        break;
      case "ranged":
        attackTypeMedia = this.game.ranged.media;
        break;
      case "magic":
        attackTypeMedia = this.game.altMagic.media;
        break;
    }
  ...
  renderNoStats() {
    ...
    this.statElements.minHit.forEach((elem) => (elem.textContent = text));
    this.statElements.maxHit.forEach((elem) => (elem.textContent = text));
    this.statElements.accuracy.forEach((elem) => (elem.textContent = text));
  ...
  setStatsFromMonster(monster) {
    this.spellSelection.standard = monster.selectedSpell;
    if (monster.passives.length > 0)
      this.addPassives(monster.passives, false, true, false);
  ...
  decode(reader, version) {
    ...
    if (this.encodeMonster && reader.getBoolean()) {
      ...
      if (typeof monster === "string") {
        if (this.manager.fightInProgress) this.manager.shouldResetAction = true;
      } else {
        this.monster = monster;
        this.setStatsFromMonster(monster);
      }
  ...
  deserialize(reader, version, idMap) {
    ...
    if (version >= 7) this.randomAttackType = reader.getRandomAttackType();
  }
