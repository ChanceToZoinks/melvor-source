...
class Runecrafting extends ArtisanSkill {
  constructor(namespace, game) {
    super(namespace, "Runecrafting", game);
    this._media = "assets/media/skills/runecrafting/runecrafting.svg";
  ...
  get noCostsMessage() {
    return getLangString("TOASTS", "MATERIALS_REQUIRED_TO_CREATE");
  }
  get actionXP() {
    let xp = super.actionXP;
    if (this.isMakingRunes && this.isPoolTierActive(1)) {
      xp *= 2.5;
    }
    return xp;
  }
  ...
  get actionItemQuantity() {
    ...
    if (this.isMakingRunes) {
      if (this.isPoolTierActive(3)) quantity += 5;
      const masteryLevel = this.masteryLevel;
      quantity += Math.floor(masteryLevel / 15);
      if (masteryLevel >= 99) quantity += 4;
    }
    return quantity;
  }
  get activeRecipe() {
  ...
  get isMakingRunes() {
    return (
      this.activeRecipe.category.id === "melvorF:StandardRunes" ||
      this.activeRecipe.category.id === "melvorF:CombinationRunes"
    );
  }
  get isOctoActive() {
    return (
      this.activeRecipe.category.id === "melvorF:CombinationRunes" &&
      this.activeRecipe.itemCosts.some(
        (cost) => cost.item.id === "melvorD:Water_Rune"
      )
    );
  }
  ...
  getRecipeSubCategory(recipe) {
    if (this.elementalRunes.includes(recipe.product)) return "ElementalRunes";
    if (recipe.product.type === "Magic Staff") return "Staff";
    return "None";
  }
  modifyItemCost(item, quantity, recipe) {
    if (recipe.product instanceof EquipmentItem && item.type === "Rune") {
      const masteryLevel = this.getMasteryLevel(recipe);
      let runeCostReduction = Math.floor(masteryLevel / 10) * 0.05;
      if (masteryLevel >= 99) runeCostReduction += 0.15;
  ...
  getPreservationChance(action, chance) {
    ...
    if (action.product.type === "Magic Staff")
      chance += this.game.modifiers.increasedRunecraftingStavePreservation;
    else if (
      action.category.id === "melvorF:StandardRunes" ||
      action.category.id === "melvorF:CombinationRunes"
    ) {
      chance += this.game.modifiers.increasedRunecraftingEssencePreservation;
    }
  ...
  recordCostPreservationStats(costs) {
    costs.recordBulkItemStat(
      this.game.stats.Runecrafting,
      RunecraftingStats.ItemsPreserved
    );
  }
  recordCostConsumptionStats(costs) {
    costs.recordBulkItemStat(
      this.game.stats.Runecrafting,
      RunecraftingStats.ItemsUsed
    );
  }
  preAction() {}
  get actionRewards() {
    ...
    if (this.isMakingRunes)
      qtyToAdd +=
        this.game.modifiers.increasedAdditionalRunecraftCountRunes -
        this.game.modifiers.decreasedAdditionalRunecraftCountRunes;
    if (this.isOctoActive) {
      qtyToAdd += this.game.modifiers.increasedRunecraftingWaterComboRunes;
    }
    ...
    const bonusRuneChance =
      this.game.modifiers.increasedChanceForElementalRune -
      this.game.modifiers.decreasedChanceForElementalRune;
    if (bonusRuneChance > 0) {
      let bonusRuneQuantity =
        this.game.modifiers.increasedElementalRuneGain -
        this.game.modifiers.decreasedElementalRuneGain;
      bonusRuneQuantity = Math.max(1, bonusRuneQuantity);
      if (rollPercentage(bonusRuneChance)) {
        rewards.addItem(
          getRandomArrayElement(this.elementalRunes),
          bonusRuneQuantity
        );
        this.game.stats.Runecrafting.add(
          RunecraftingStats.ItemsCrafted,
          bonusRuneQuantity
        );
      }
      const activePotion = this.activePotion;
      if (
        this.game.modifiers.giveRandomComboRunesRunecrafting > 0 &&
        ...
        rollPercentage(bonusRuneChance)
      ) {
        rewards.addItem(
          getRandomArrayElement(this.comboRunes),
          bonusRuneQuantity
        );
        this.game.stats.Runecrafting.add(
          RunecraftingStats.ItemsCrafted,
    ...
    if (
      this.game.modifiers.increasedFireRunesWhenMakingElementalRunes > 0 &&
      this.crowDevilItem !== undefined &&
      this.elementalRunes.includes(this.actionItem)
    ) {
      rewards.addItem(
        this.crowDevilItem,
        this.game.modifiers.increasedFireRunesWhenMakingElementalRunes
      );
    }
  ...
  setFromOldOffline(offline, idMap) {
    const recipe = this.actions.getObjectByID(
      idMap.runecraftingOldOffline[offline.action]
    );
    if (recipe !== undefined) {
