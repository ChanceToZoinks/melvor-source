...
class RaidPlayer extends Player {
  ...
  get prayerPointsOnWaveCompletion() {
    return 20 + this.modifiers.golbinRaidIncreasedPrayerPointsWave;
  }
  get activeTriangle() {
    return combatTriangle[this.manager.difficulty.combatTriangle];
  }
  get allowRegen() {
    return true;
  }
  ...
  resetAltAttacks() {
    Object.entries(equipmentSlotData).forEach((entry) => {
      this.altAttacks[entry[0]] = [];
    });
  }
  getSlotAttacks(slot) {
    const altAttacks = this.altAttacks[slot.type];
    if (altAttacks.length > 0) return altAttacks;
    else return super.getSlotAttacks(slot);
  }
  computeLevels() {
    this.levels.Attack =
      this.getSkillLevel("Attack") +
      this.modifiers.getHiddenSkillLevels(this.game.attack);
    this.levels.Strength =
      this.getSkillLevel("Strength") +
      this.modifiers.getHiddenSkillLevels(this.game.strength);
    this.levels.Defence =
      this.getSkillLevel("Defence") +
      this.modifiers.getHiddenSkillLevels(this.game.defence);
    this.levels.Hitpoints =
      this.getSkillLevel("Hitpoints") +
      this.modifiers.getHiddenSkillLevels(this.game.hitpoints);
    this.levels.Ranged =
      this.getSkillLevel("Ranged") +
      this.modifiers.getHiddenSkillLevels(this.game.ranged);
    this.levels.Magic =
      this.getSkillLevel("Magic") +
      this.modifiers.getHiddenSkillLevels(this.game.altMagic);
  ...
  computeModifiers() {
    ...
    this.modifiers.addArrayModifiers(this.manager.randomPlayerModifiers);
  ...
  equipItem(item, set, slot = "Default", quantity = 1, altAttacks = []) {
    ...
    this.equipment.getSlotsToUnequip(item, slot).forEach((slot) => {
      this.altAttacks[slot] = [];
    });
    this.altAttacks[slot] = altAttacks;
    if (slot === "Quiver") quantity += equipment.slots[slot].quantity;
  ...
  setEquipmentToDefault() {
    ...
    if (this.manager.startingAmmo)
      this.equipItem(
        this.manager.startingAmmo.item,
        this.selectedEquipmentSet,
        "Quiver",
        this.manager.startingAmmo.quantity
      );
    this.food.unequipSelected();
    if (this.manager.startingFood)
      this.equipFood(this.manager.startingFood, 10);
    this.prayerPoints = 0;
    this.addPrayerPoints(this.modifiers.golbinRaidIncreasedPrayerPointsStart);
  }
  addMiscModifiers() {
    this.modifiers.addModifiers({
      increasedAutoEatThreshold: 30,
      increasedAutoEatEfficiency: 80,
      increasedAutoEatHPLimit: 60,
    });
  }
  getSkillLevel(skill) {
    let level = this.manager.wave + 1;
    if (skill === "Hitpoints") level += 9;
    if (skill === "Prayer")
      level = 1 + this.modifiers.golbinRaidIncreasedPrayerLevel;
    return level;
  }
  getLevelHistory() {
    return [
  ...
  getEquipmentHistory() {
    ...
    this.equipment.slotArray.forEach((slot) => {
      if (slot.providesStats) history.push(slot.item);
    });
    return history;
  }
  ...
  consumeAmmo() {
    super.consumeAmmo();
    if (this.equipment.slots.Weapon.isEmpty) this.equipDefaultWeapon();
  }
  equipDefaultWeapon() {
    const weaponIndex = Math.min(
      this.modifiers.golbinRaidStartingWeapon,
      this.manager.startingWeapons.length - 1
    );
    const weapon = this.manager.startingWeapons[weaponIndex];
    this.equipItem(weapon, this.selectedEquipmentSet, "Weapon");
  }
  rewardForDamage(damage) {}
  trackItemUsage(costs) {}
  trackWeaponStat(stat, amount = 1) {}
  processCombatEvent(event, interval = 0) {
    this.consumeEquipmentCharges(event, interval);
  }
  ...
  render() {
    super.render();
    if (this.rendersRequired.levels) this.renderLevels();
  }
  renderLevels() {
    combatSkills.forEach((skill) => {
      const level = this.getSkillLevel(skill);
      this.statElements.golbinLevels[skill].textContent = `${level}`;
    });
  ...
  encode(writer) {
    super.encode(writer);
    writer.writeArray(
      Object.entries(this.altAttacks),
      ([slot, attacks], writer) => {
        writer.writeUint8(equipmentSlotData[slot].id);
  ...
  decode(reader, version) {
    if (version >= 24) super.decode(reader, version);
    const altAttackArray = reader.getArray((reader) => {
      const slot = EquipmentSlots[reader.getUint8()];
      const attacks = reader.getArray(
        readNamespacedReject(this.game.specialAttacks)
      );
      if (attacks.length === 0) return undefined;
      return [slot, attacks];
    });
    altAttackArray.forEach(([slot, attacks]) => {
      this.altAttacks[slot] = attacks;
    });
  }
  deserializeAltAttacks(reader, version, idMap) {
    while (!reader.atEnd) {
      ...
      reader.getChunk(numAttacks).forEach((attackID) => {
        const attack = this.game.specialAttacks.getObjectByID(
          idMap.attacks[attackID]
        );
        if (attack !== undefined) altAttackArray.push(attack);
      });
      if (altAttackArray.length > 0)
