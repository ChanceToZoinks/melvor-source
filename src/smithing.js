...
class Smithing extends ArtisanSkill {
  ...
  get noCostsMessage() {
    return getLangString("TOASTS", "MATERIALS_REQUIRED_TO_SMITH");
  }
  get actionItem() {
  ...
  get isMakingBar() {
    return this.activeRecipe.category.id === "melvorD:Bars";
  }
  get recipeHasCoal() {
    return this.activeRecipe.itemCosts.some(
      (itemCost) => itemCost.item.id === "melvorD:Coal_Ore"
    );
  }
    ...
    this.actions.forEach((recipe) => {
      if (recipe.category.id === "melvorD:Bars") {
        recipe.itemCosts.forEach(({ item }) => {
          if (item.id !== "melvorD:Coal_Ore" && !this.oreToBarMap.has(item)) {
  ...
  getUncappedDoublingChance(action) {
    ...
    chance += Math.floor((masteryLevel + 10) / 20) * 5;
    if (masteryLevel >= 99) chance += 10;
  ...
  getPreservationChance(action, chance) {
    ...
    if (action.category.id === "melvorD:DragonGear")
      chance += this.game.modifiers.increasedSmithingDragonGearPreservation;
  ...
  modifyItemCost(item, quantity, recipe) {
    if (item.id === "melvorD:Coal_Ore") {
      quantity = applyModifier(
        quantity,
        this.game.modifiers.decreasedSmithingCoalCost,
        2
      );
      quantity -= this.game.modifiers.decreasedFlatSmithingCoalCost;
      return quantity;
    } else {
      return quantity;
    }
  ...
  recordCostPreservationStats(costs) {
    const statToTrack = this.isMakingBar
      ? SmithingStats.OresPreserved
      : SmithingStats.BarsPreserved;
    costs.recordBulkItemStat(this.game.stats.Smithing, statToTrack);
  }
  recordCostConsumptionStats(costs) {
    const statToTrack = this.isMakingBar
      ? SmithingStats.OresUsed
      : SmithingStats.BarsUsed;
    costs.recordBulkItemStat(this.game.stats.Smithing, statToTrack);
  }
  preAction() {}
  get actionRewards() {
    const rewards = new Rewards(this.game);
    const actionEvent = new SmithingActionEvent(this, this.activeRecipe);
    let qtyToAdd = this.actionItemQuantity;
    if (rollPercentage(this.game.modifiers.summoningSynergy_3_17)) qtyToAdd++;
    ...
    if (item.id === "melvorD:Silver_Bar") {
      let goldBarQty = 1;
      const activePotion = this.activePotion;
      if (
        this.game.modifiers.doubleSilverGoldSmithingWithSeeingGold > 0 &&
      ...
      if (
        rollPercentage(
          this.game.modifiers.increasedSeeingGoldChance -
            this.game.modifiers.decreasedSeeingGoldChance
        )
      ) {
        rewards.addItemByID("melvorD:Gold_Bar", goldBarQty);
        this.game.stats.Smithing.add(
    ...
    if (this.activeRecipe.category.id === "melvorD:Bars")
      extraItemChance +=
        this.game.modifiers.increasedChanceAdditionalBarSmithing;
    ...
    this.game.stats.Smithing.add(
      this.isMakingBar
        ? SmithingStats.TotalBarsSmelted
        : SmithingStats.TotalItemsSmithed,
      qtyToAdd
    );
  ...
  setFromOldOffline(offline, idMap) {
    const recipe = this.actions.getObjectByID(
      idMap.smithingOldOffline[offline.action]
    );
    if (recipe !== undefined) {
