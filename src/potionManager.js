...
class PotionManager {
  ...
  getActivePotionForAction(action) {
    ...
    return activePotion === null || activePotion === void 0
      ? void 0
      : activePotion.item;
  }
  ...
  getPotionCharges(item) {
    let potionCharges =
      item.charges +
      this.game.modifiers.increasedPotionChargesFlat -
      this.game.modifiers.decreasedPotionChargesFlat;
    if (
      this.game.modifiers.increasedFishermansPotionCharges > 0 &&
      (item.id === "melvorF:Fishermans_Potion_I" ||
        item.id === "melvorF:Fishermans_Potion_II" ||
        item.id === "melvorF:Fishermans_Potion_III" ||
        item.id === "melvorF:Fishermans_Potion_IV")
    )
      potionCharges = applyModifier(
        potionCharges,
        this.game.modifiers.increasedFishermansPotionCharges
      );
    else if (
      this.game.modifiers.increasedGenerousCookPotionCharges > 0 &&
      (item.id === "melvorF:Generous_Cook_Potion_I" ||
        item.id === "melvorF:Generous_Cook_Potion_II" ||
        item.id === "melvorF:Generous_Cook_Potion_III" ||
        item.id === "melvorF:Generous_Cook_Potion_IV")
    )
      potionCharges = applyModifier(
        potionCharges,
        this.game.modifiers.increasedGenerousCookPotionCharges
      );
    else if (
      this.game.modifiers.increasedCraftingPotionCharges > 0 &&
      (item.id === "melvorF:Crafting_Potion_I" ||
        item.id === "melvorF:Crafting_Potion_II" ||
        item.id === "melvorF:Crafting_Potion_III" ||
        item.id === "melvorF:Crafting_Potion_IV")
    )
      potionCharges = applyModifier(
        potionCharges,
        this.game.modifiers.increasedCraftingPotionCharges
      );
    return Math.max(1, potionCharges);
  }
  usePotion(item, loadPotions = false) {
    if (!this.game.bank.hasItem(item)) return;
    ...
    if (existingPotion !== undefined && existingPotion.item === item) {
      ...
      this.activePotions.set(item.action, { item, charges });
    ...
    const event = new PotionUsedEvent(item, charges);
    this.game.processEvent(event);
    if (loadPotions) this.openPotionSelectOnClick(item.action);
  }
  removePotion(action, loadPotions = false) {
    const existingPotion = this.activePotions.get(action);
    if (existingPotion === undefined) return;
    ...
    this.game.combat.notifications.add({
      ...
      args: [
        this.game.herblore,
        getLangString("TOASTS", "POTION_DEPLETED"),
        "danger",
      ],
    });
    this.renderRequired = true;
    if (loadPotions) this.openPotionSelectOnClick(action);
  }
  consumeCharges(event) {
    this.activePotions.forEach((potion, action) => {
      if (
        potion.item.consumesOn.some((matcher) => matcher.doesEventMatch(event))
      ) {
        const event = new PotionChargeUsedEvent(potion.item);
        const preservationChance = Math.min(
          this.game.modifiers.increasedChanceToPreservePotionCharge -
            this.game.modifiers.decreasedChanceToPreservePotionCharge,
          80
        );
        if (!rollPercentage(preservationChance)) {
          ...
          if (potion.charges <= 0) {
            if (
              this.autoReuseActions.has(potion.item.action) &&
              this.game.bank.hasItem(potion.item)
            ) {
  ...
  render() {
    ...
    if (
      ...
      this.updatePotionHeader(
        this.activePotions.get(
          (_b = this.game.openPage) === null || _b === void 0
            ? void 0
            : _b.action
        )
      );
  ...
  onLoad() {
    const potionButton = document.getElementById(
      "page-header-potions-dropdown"
    );
  ...
  updatePotionHeader(potion) {
    if (potion === undefined) {
      $("#header-potion-image").attr(
        "src",
        cdnMedia("assets/media/skills/herblore/potion_empty.svg")
      );
  ...
  encode(writer) {
    writer.writeMap(this.activePotions, writeNamespaced, (activePotion) => {
      writer.writeNamespacedObject(activePotion.item),
        writer.writeUint32(activePotion.charges);
    });
    writer.writeSet(this.autoReuseActions, writeNamespaced);
    return writer;
  }
  decode(reader, version) {
    this.activePotions = reader.getMap(
      readNamespacedReject(this.game.actions),
      (reader) => {
    ...
    this.autoReuseActions = reader.getSet(
      readNamespacedReject(this.game.actions)
    );
  }
  convertFromOldFormat(save, idMap) {
    if (save.herbloreBonuses !== undefined) {
      Object.entries(save.herbloreBonuses).forEach(([key, oldBonus]) => {
        ...
        const action = this.game.actions.getObjectByID(
          idMap.pageToAction[pageID]
        );
        if (
          action !== undefined &&
          oldBonus.charges > 0 &&
          ...
          this.activePotions.set(action, { item, charges: oldBonus.charges });
        }
      });
    }
    if (
    ...
    if (save.SETTINGS !== undefined) {
      ...
      save.SETTINGS.general.autoReusePotion.forEach((oldPageID) => {
        const action = this.game.actions.getObjectByID(
          idMap.pageToAction[oldPageID]
        );
        if (action !== undefined) this.autoReuseActions.add(action);
      });
    }
  }
}
class PotionSelectMenuItem extends HTMLElement {
  constructor() {
    super();
    ...
    this.potionQuantity = getElementFromFragment(
      this._content,
      "potion-quantity",
      "h5"
    );
    this.potionName = getElementFromFragment(
      this._content,
      "potion-name",
      "span"
    );
    this.useButton = getElementFromFragment(
      this._content,
      "use-button",
      "button"
    );
    this.potionDescription = getElementFromFragment(
      this._content,
      "potion-description",
      "h5"
    );
    this.potionCharges = getElementFromFragment(
      this._content,
      "potion-charges",
      "h5"
    );
  ...
  setPotion(potion, game) {
    ...
    this.potionQuantity.textContent = numberWithCommas(
      game.bank.getQty(potion)
    );
    this.potionName.textContent = potion.name;
    this.potionDescription.textContent = potion.description;
    this.potionCharges.textContent = templateLangString("MISC_STRING", "28", {
      num: `${potion.charges}`,
    });
    const isActive = game.potions.isPotionActive(potion);
    if (isActive) {
      this.useButton.textContent = getLangString("MISC_STRING", "30");
      this.useButton.onclick = () =>
        game.potions.removePotion(potion.action, true);
      this.useButton.classList.replace("btn-success", "btn-danger");
      this.potionName.classList.replace("text-success", "text-danger");
    } else {
      this.useButton.textContent = getLangString("MISC_STRING", "29");
...
window.customElements.define("potion-select-menu-item", PotionSelectMenuItem);
class PotionSelectMenu extends HTMLElement {
  constructor() {
    super();
    this.menuItems = [];
    this._content = new DocumentFragment();
    this._content.append(getTemplateNode("potion-select-menu-template"));
    this.potionContainer = getElementFromFragment(
      this._content,
      "potion-container",
      "div"
    );
    this.autoReuseCheckBox = getElementFromFragment(
      this._content,
      "auto-reuse-checkbox",
      "input"
    );
  ...
  showPotionSelection(potions, action, game) {
    while (this.menuItems.length < potions.length) {
      this.menuItems.push(
        createElement("potion-select-menu-item", {
          className:
            setLang == "de" || setLang == "es"
    ...
    this.autoReuseCheckBox.checked =
      game.potions.autoReusePotionsForAction(action);
    this.autoReuseCheckBox.onchange = () =>
      game.potions.toggleAutoReusePotion(action);
  }
}
