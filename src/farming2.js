...
class FarmingRecipe extends SingleProductRecipe {
  constructor(namespace, data, skill, game) {
    ...
    if (category === undefined)
      throw new Error(
        `Error constructing FarmingRecipe with id: ${this.id}. Category with id: ${data.categoryID} is not registered.`
      );
  ...
  get name() {
    if (this._grownNameLang !== undefined) {
      return getLangString(
        this._grownNameLang.category,
        this._grownNameLang.id
      );
...
class FarmingCategory extends SkillCategory {
  ...
  get singularName() {
    if (this.isModded) {
      ...
      return getLangString(
        "SKILL_CATEGORY",
        `${this.skill.localID}_${this.localID}_singular`
      );
    }
  }
  get description() {
    if (this.isModded) {
      ...
      return getLangString(
        "SKILL_CATEGORY",
        `${this.skill.localID}_${this.localID}_description`
      );
    }
  }
  get seedNotice() {
    if (this.isModded) {
      ...
      return getLangString(
        "SKILL_CATEGORY",
        `${this.skill.localID}_${this.localID}_seedNotice`
      );
    }
  }
}
class FarmingPlot extends NamespacedObject {
  constructor(namespace, data, farming) {
    ...
    this.compostLevel = 0;
    this.growthTime = 0;
    const category = farming.categories.getObjectByID(data.categoryID);
    if (category === undefined)
      throw new Error(
        `Error constructing FarmingPlot with id: ${this.id}. Category with id: ${data.categoryID} is not registered.`
      );
    this.category = category;
    this.level = data.level;
    this.gpCost = data.gpCost;
    this.state = this.gpCost === 0 && this.level === 1 ? 1 : 0;
  }
  encode(writer) {
  ...
  decode(reader, version) {
    ...
    if (reader.getBoolean()) {
      ...
      if (typeof selected === "string") {
        this.selectedRecipe = this.farming.actions.find(
    ...
    if (
      resetPlanted ||
      (this.state === 3 && this.plantedRecipe === undefined)
    ) {
      this.state = 1;
      this.growthTime = 0;
    }
...
class DummyFarmingPlot extends FarmingPlot {
  constructor(namespace, id, game) {
    super(
      namespace,
      { id, categoryID: "melvorD:Allotment", level: 1, gpCost: -1 },
      game.farming
    );
  }
  decode(reader, version) {
...
class Farming extends SkillWithMastery {
  constructor(namespace, game) {
    super(namespace, "Farming", game);
    this._media = "assets/media/skills/farming/farming.svg";
    ...
    this.plots = new NamespaceRegistry(game.registeredNamespaces);
  }
  get isPotionActive() {
    ...
    this.plots.forEach((plot) => {
      ...
      plotLog.push(
        `id: ${plot.id}; state: ${plot.state}; plantedRecipe: ${
          ...
          (_b = plot.compostItem) === null || _b === void 0 ? void 0 : _b.id
        }; compostLevel: ${plot.compostLevel}; selectedRecipe: ${
          (_c = plot.selectedRecipe) === null || _c === void 0 ? void 0 : _c.id
...
${plotLog.join("\n")}`;
    ...
    (_a = data.categories) === null || _a === void 0
      ...
      : _a.forEach((categoryData) => {
          this.categories.registerObject(
            new FarmingCategory(namespace, categoryData, this)
          );
        });
    (_b = data.recipes) === null || _b === void 0
      ...
      : _b.forEach((recipeData) => {
          this.actions.registerObject(
            new FarmingRecipe(namespace, recipeData, this, this.game)
          );
        });
  ...
  postDataRegistration() {
    ...
    this.actions.forEach((recipe) => {
      if (recipe.category.id === "melvorD:Herb")
    ...
    this.categories.forEach((category) => {
      ...
      const plots = this.plots
        .filter((plot) => plot.category === category)
        .sort((a, b) => a.level - b.level);
  ...
  growPlots(timer) {
    ...
    timer.plots.forEach((plot) => {
      ...
      (_a = this.renderQueue).growthIndicators ||
        (_a.growthIndicators = success);
    ...
    if (anyFailed) {
      this.game.combat.notifications.add({
        type: "Player",
        args: [this, getLangString("TOASTS", "CROP_FAILED"), "danger"],
      });
    }
    if (anyGrown) {
      this.game.combat.notifications.add({
        type: "Player",
        args: [this, getLangString("TOASTS", "CROP_READY")],
      });
    }
  ...
  getRecipesForCategory(category) {
    var _a;
    return (_a = this.categoryRecipeMap.get(category)) !== null && _a !== void 0
      ? _a
      : [];
  }
  getPlotsForCategory(category) {
    var _a;
    return (_a = this.categoryPlotMap.get(category)) !== null && _a !== void 0
      ? _a
      : [];
  }
  ...
  getRecipeSeedCost(recipe) {
    ...
    if (recipe.category.id === "melvorD:Allotment")
      quantity +=
        this.game.modifiers.increasedAllotmentSeedCost -
        this.game.modifiers.decreasedAllotmentSeedCost;
  ...
  getPercentageIntervalModifier(action) {
    ...
    if (masteryLevel >= 99) modifier -= 10;
    if (this.isPoolTierActive(3)) modifier -= 10;
    return modifier;
  }
  ...
  getPlotGrowthChance(plot) {
    ...
    if (
      (plot.plantedRecipe !== undefined &&
        this.getMasteryLevel(plot.plantedRecipe) >= 50) ||
  ...
  harvestPlot(plot) {
    ...
    let harvestQuantity = Math.floor(5 + masteryLevel * 0.4);
    let quantityMultiplier =
      this.game.modifiers.increasedFarmingYield -
      this.game.modifiers.decreasedFarmingYield;
    if (this.isPoolTierActive(2)) quantityMultiplier += 5;
    if (masteryLevel >= 99) quantityMultiplier += 20;
    if (plot.compostItem !== undefined)
      quantityMultiplier += plot.compostItem.harvestBonus;
    harvestQuantity = applyModifier(harvestQuantity, quantityMultiplier);
    if (rollPercentage(this.getDoublingChance(recipe))) harvestQuantity *= 2;
    harvestQuantity *= recipe.category.harvestMultiplier;
    harvestQuantity +=
      this.game.modifiers.increasedFlatFarmingYield -
      this.game.modifiers.decreasedFlatFarmingYield;
    if (!this.game.bank.addItem(recipe.product, harvestQuantity, false, true))
      return false;
    const rewards = new Rewards(this.game);
    this.rollForPets(plot.growthTime);
    if (recipe.category.returnSeeds && masteryLevel >= 15) {
      const maxSeeds = masteryLevel >= 99 ? 8 : masteryLevel / 15;
      const seedQty = Math.floor(Math.random() * maxSeeds);
      if (seedQty > 0) rewards.addItem(recipe.seedCost.item, seedQty);
    }
    this.rollForRareDrops(recipe.level, rewards);
    if (
    ...
    this.addMasteryForAction(
      recipe,
      (plot.growthTime * harvestQuantity) / recipe.category.masteryXPDivider
    );
    rewards.giveRewards();
    switch (recipe.category.id) {
      ...
      case "melvorD:Tree":
        ...
        this.game.stats.Farming.add(
          recipe.product instanceof FoodItem
            ? FarmingStats.FoodGained
            : FarmingStats.LogsGained,
          harvestQuantity
        );
        break;
      case "melvorD:Herb":
    ...
    this.game.stats.Items.add(
      recipe.product,
      ItemStats.HarvestAmount,
      harvestQuantity
    );
    this.game.stats.Items.add(
      recipe.seedCost.item,
      ItemStats.TimeWaited,
      plot.growthTime
    );
    this.resetPlot(plot);
  ...
  resetPlot(plot) {
    ...
    this.renderQueue.growthIndicators = true;
    if (
      !rollPercentage(this.game.modifiers.increasedCompostPreservationChance)
    ) {
    ...
    if (!costs.checkIfOwned()) {
      if (isSelected) {
        this.game.combat.notifications.add({
          ...
          args: [
            this,
            templateLangString("TOASTS", "SEED_PLANT_FAILURE", {
        ...
        this.game.combat.notifications.add({
          type: "Player",
          args: [this, getLangString("TOASTS", "NOT_ENOUGH_SEEDS"), "danger"],
        });
      }
      return -1;
    }
  ...
  plantAllPlots(category, forceRecipe) {
    if (forceRecipe !== undefined && forceRecipe.category !== category)
      throw new Error(
        "Tried to plant all plots with recipe that does not match plot category."
      );
    ...
    if (!this.game.gp.canAfford(cost)) {
      this.game.combat.notifications.add({
        type: "Player",
        args: [this, getLangString("TOASTS", "PLANT_ALL_GP"), "danger"],
      });
      return;
    }
    ...
    plots.forEach((plot) => {
      const recipe =
        forceRecipe !== null && forceRecipe !== void 0
          ? forceRecipe
          : plot.selectedRecipe;
    ...
    plotIntervalMap.forEach((plots, interval) =>
      this.createGrowthTimer(plots, interval)
    );
    if (anyPlanted) this.game.gp.remove(cost);
  }
  onMasteryLevelUp(action, oldLevel, newLevel) {
    super.onMasteryLevelUp(action, oldLevel, newLevel);
    if (newLevel >= 50 && oldLevel < 50) {
      this.plots.forEach((plot) => {
        if (plot.plantedRecipe === action) this.renderQueue.compost.add(plot);
      });
    }
  }
  onMasteryPoolBonusChange(oldBonusLevel, newBonusLevel) {
    super.onMasteryPoolBonusChange(oldBonusLevel, newBonusLevel);
    if (this.wasPoolBonusChanged(oldBonusLevel, newBonusLevel, 1)) {
      this.plots.forEach((plot) => {
  ...
  renderGrowthStatus() {
    ...
    this.renderQueue.growthTime.forEach((timer) => {
      timer.plots.forEach((plot) => {
        var _a;
        (_a = farmingMenus.plotMap.get(plot)) === null || _a === void 0
          ? void 0
          : _a.updateGrowthTime(plot, this);
      });
    });
  ...
  renderGrowthIndicators() {
    ...
    this.categories.forEach((category) => {
      ...
      const hasGrown = plots.some(
        (plot) => plot.state === 3 || plot.state === 4
      );
      (_a = farmingMenus.categoryButtons.get(category)) === null ||
      _a === void 0
        ? void 0
        : _a.updateNotice(hasGrown);
      anyGrown || (anyGrown = hasGrown);
    });
    skillNav.setGlowing(this, anyGrown);
    this.renderQueue.growthIndicators = false;
  }
  renderCompostQuantity() {
    if (!this.renderQueue.compostQuantity) return;
    farmingMenus.compostIcons.forEach((icon) => icon.updateQuantity());
  ...
  getUncappedDoublingChance(action) {
    ...
    chance +=
      this.game.modifiers.increasedChanceDoubleHarvest -
      this.game.modifiers.decreasedChanceDoubleHarvest;
    return chance;
  }
  showPlotsInCategory(category) {
    ...
    plots.forEach((plot) => {
      if (plot.state !== 0) {
        unlockedPlots++;
        if (farmingMenus.plots.length < unlockedPlots) {
          const newPlot = createElement("farming-plot", {
            className: "col-6 col-xl-3",
          });
          if (farmingMenus.plots.length === 0) {
            farmingMenus.categoryOptions.after(newPlot);
          } else {
            farmingMenus.plots[farmingMenus.plots.length - 1].after(newPlot);
          }
          farmingMenus.plots.push(newPlot);
        }
      } else {
        if (farmingMenus.lockedPlots.length === 0) {
          const lockedPlot = createElement("locked-farming-plot", {
            className: "col-6 col-xl-3",
          });
          farmingMenus.plotContainer.append(lockedPlot);
    ...
    plots.every((plot, i) => {
      if (plot.state !== 0) {
        farmingMenus.plots[i].setPlot(plot, this.game);
        farmingMenus.plotMap.set(plot, farmingMenus.plots[i]);
      } else {
        farmingMenus.lockedPlots[0].setPlot(plot, this);
      }
      return plot.state !== 0;
    });
    farmingMenus.plots.forEach((plot, i) => {
      if (unlockedPlots > i) showElement(plot);
      else hideElement(plot);
    });
    if (lockedPlots > 0) showElement(farmingMenus.lockedPlots[0]);
    else if (farmingMenus.lockedPlots.length > 0)
      hideElement(farmingMenus.lockedPlots[0]);
  ...
  harvestAllOnClick(category) {
    ...
    if (!this.game.gp.canAfford(cost)) {
      notifyPlayer(this, getLangString("TOASTS", "HARVEST_ALL_GP"), "danger");
      return;
    }
    const plots = this.getPlotsForCategory(category);
    let harvestCount = 0;
    ...
    if (harvestCount > 0) this.game.gp.remove(cost);
  }
  compostPlot(plot, compost, amount) {
    if (plot.state !== 1) return false;
    const freeCompost =
      compost.id === "melvorD:Compost" && this.game.modifiers.freeCompost > 0;
    const owned = freeCompost ? Infinity : this.game.bank.getQty(compost);
    if (
      plot.compostItem !== undefined &&
      plot.compostItem.harvestBonus >= compost.harvestBonus
    ) {
      if (
        plot.compostItem.harvestBonus > compost.harvestBonus ||
        plot.compostLevel === 100
      )
        return false;
      const amountNeeded = Math.ceil(
        (100 - plot.compostLevel) / compost.compostValue
      );
  ...
  notifyNoCompost(compost) {
    ...
    switch (compost.id) {
      case "melvorD:Compost":
        message = getLangString("TOASTS", "NEED_COMPOST");
        break;
      case "melvorD:Weird_Gloop":
        message = getLangString("TOASTS", "NEED_GLOOP");
        break;
      default:
        message = templateLangString("FARMING_MISC", "NEED_ITEM_TO_APPLY", {
  ...
  notifyCantAffordCompostAll(compost) {
    ...
    switch (compost.id) {
      case "melvorD:Compost":
        message = getLangString("TOASTS", "COMPOST_ALL_GP");
        break;
      case "melvorD:Weird_Gloop":
        message = getLangString("TOASTS", "GLOOP_ALL_GP");
        break;
      default:
        message = templateLangString("FARMING_MISC", "NO_GP_APPLY_ITEM", {
          itemName: compost.name,
        });
    }
    this.game.combat.notifications.add({
  ...
  compostAllOnClick(category, compost) {
    const cost = this.getCompostAllCost(category);
    if (!this.game.gp.canAfford(cost)) {
    ...
    if (appliedAny) this.game.gp.remove(cost);
  }
  plantAllOnClick(category) {
    const plots = this.getPlotsForCategory(category);
    if (!plots.some((plot) => plot.state === 1)) {
      notifyPlayer(this, getLangString("TOASTS", "NO_EMPTY_PATCHES"), "danger");
      return;
    }
    farmingMenus.seedSelect.setSeedSelection(category, this.game);
  ...
  destroyPlotOnClick(plot) {
    ...
    if (this.game.settings.showCropDestructionConfirmations) {
      SwalLocale.fire({
        title: getLangString("MENU_TEXT", "DESTROY_CROP"),
        html: `<h5 class="font-w600 text-danger font-size-sm mb-1">${getLangString(
          "MENU_TEXT",
          "CANNOT_UNDO"
        )}</h5>`,
  ...
  destroyPlot(plot) {
    ...
    if (plotIndex === -1)
      throw new Error(
        "Tried destroying plot, but plot is not a member of timer."
      );
    this.growthTimerMap.delete(plot);
  ...
  plantPlotOnClick(plot) {
    ...
    farmingMenus.seedSelect.setSeedSelection(plot.category, this.game, plot);
    farmingMenus.seedSelect.setUnselectedRecipe();
    $("#modal-farming-seed").modal("show");
  }
  ...
  unlockPlotOnClick(plot) {
    if (!this.game.gp.canAfford(plot.gpCost) || this.level < plot.level) return;
    this.game.gp.remove(plot.gpCost);
    plot.state = 1;
    ...
    if (save.newFarmingAreas !== undefined) {
      save.newFarmingAreas.forEach((area, catID) => {
        area.patches.forEach((patch, i) => {
          var _a;
          const plot = this.plots.getObjectByID(
            idMap.farmingPlots[`${catID}:${i}`]
          );
          ...
          if (patch.unlocked) {
            if (patch.seedID > 0) {
              const recipe = this.actions.getObjectByID(
                idMap.farmingSeedToRecipe[patch.seedID]
              );
              if (recipe === undefined) {
                plot.state = 1;
                return;
              ...
              if (patch.hasGrown) {
                ...
                plot.state = 2;
                const growthTime =
                  (_a = patch.setInterval) !== null && _a !== void 0
                    ? _a
                    : recipe.baseInterval / 1000;
                plot.growthTime = growthTime;
                let timeLeft =
                  patch.timePlanted + growthTime * 1000 - currentTime;
    ...
    if (save.plantAllSelected !== undefined) {
      Object.entries(save.plantAllSelected).forEach(([catID, seeds]) => {
        seeds.forEach((seedID, i) => {
          ...
          if (plot === undefined) return;
          const recipe = this.actions.getObjectByID(
            idMap.farmingSeedToRecipe[seedID]
          );
