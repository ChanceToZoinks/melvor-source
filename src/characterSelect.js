...
function getLocalInfoInSlot(slotID) {
  ...
  if (localSaveHeaders.length === maxSaveSlots) {
    ...
    if (!(typeof localHeader === "number")) localInfo = localHeader;
  }
  return localInfo;
}
  ...
  localSaveHeaders.forEach((localInfo, slotID) => {
    ...
    switch (localInfo) {
      case 0:
        ...
        break;
      case 1:
        saveSlotDisplay.setError(
          slotID,
          getLangString("MENU_TEXT", "SAVE_ERROR_MESSAGE_2"),
          false
        );
        break;
      case 2:
        saveSlotDisplay.setError(
          slotID,
          getLangString("MENU_TEXT", "SAVE_ERROR_MESSAGE_1"),
          false
        );
        break;
      default:
  ...
  cloudSaveHeaders.forEach((cloudInfo, slotID) => {
    ...
    switch (cloudInfo) {
      ...
      case 1:
        saveSlotDisplay.setError(
          slotID,
          getLangString("MENU_TEXT", "SAVE_ERROR_MESSAGE_3"),
          true
        );
        break;
      case 2:
        saveSlotDisplay.setError(
          slotID,
          getLangString("MENU_TEXT", "SAVE_ERROR_MESSAGE_4"),
          true
        );
        break;
      default:
...
function showSaveExpansionError(ns) {
  ...
  switch (ns) {
    case "melvorTotH":
      modalBody.append(
        createElement("p", {
          text: getLangString("CHARACTER_SELECT", "SAVE_CONTENT_ERROR_THRONE"),
        })
      );
      break;
    case "melvorF":
      modalBody.append(
        createElement("p", {
          text: getLangString("CHARACTER_SELECT", "SAVE_CONTENT_ERROR_FULL"),
        })
      );
      break;
    case "melvorD":
      modalBody.append(
        createElement("p", {
          text: getLangString("CHARACTER_SELECT", "SAVE_CONTENT_ERROR_DEMO"),
        })
      );
      break;
  }
  const instructions = createElement("p", { parent: modalBody });
  instructions.innerHTML = templateLangString(
    "CHARACTER_SELECT",
  ...
  SwalLocale.fire({
    titleText: getLangString("CHARACTER_SELECT", "ERROR_LOADING_SAVE"),
    icon: "error",
    html: modalBody,
  });
}
function forceLoadSaveOnClick(slotID, isCloud) {
  return __awaiter(this, void 0, void 0, function* () {
    const saveInfo = isCloud
      ? cloudSaveHeaders[slotID]
      : localSaveHeaders[slotID];
    if (typeof saveInfo === "number")
      throw new Error("Tried to force load invalid save.");
    const modalBody = createElement("div");
    modalBody.append(
      createElement("p", {
        text: getLangString("CHARACTER_SELECT", "FORCE_LOAD_WARNING"),
    ...
    createElement("label", {
      text: getLangString("CHARACTER_SELECT", "FORCE_LOAD_CONFIRMATION"),
    ...
    const userResponse = yield SwalLocale.fire({
      titleText: getLangString("CHARACTER_SELECT", "FORCE_LOAD_SAVE"),
      icon: "warning",
      html: modalBody,
      confirmButtonText: getLangString("CHARACTER_SELECT", "FORCE_LOAD"),
    ...
    if (isCloud) loadCloudSaveOnClick(slotID, true);
    else loadLocalSaveOnClick(slotID, true);
  });
}
function loadLocalSaveOnClick(slotID, force = false) {
  return __awaiter(this, void 0, void 0, function* () {
    if (isLoadingSave) return;
    if (showModManagerPrompts()) return;
    ...
    if (
      ...
      html.append(
        createElement("h5", {
          ...
          text: getLangString("CHARACTER_SELECT", "69"),
        }),
        getLocalSaveSummary(slotID),
        createElement("h5", {
          className: "font-w600 text-danger mb-3 mt-3",
          text: getLangString("MENU_TEXT", "WILL_OVERWRITE"),
        }),
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (localStorageSettings.enableSaveOverwriteWarning == "true") {
      ...
      html.append(
        createElement("h5", {
          className: "text-dark font-size-sm",
          text: getLangString("CHARACTER_SELECT", "70"),
        }),
        getCloudSaveSummary(slotID),
        createElement("h5", {
          className: "font-w600 text-danger mb-3 mt-3",
  ...
  switch (localInfo) {
    ...
    case 2:
      return createElement("h5", {
        ...
        text: getLangString("MENU_TEXT", "ERROR_DISPLAY_LOCAL"),
      });
    default: {
...
function getCloudSaveSummary(slotID) {
  ...
  switch (cloudInfo) {
    ...
    case 2:
      return createElement("h5", {
        className: "font-w600 text-danger font-size-sm mb-3",
        text: getLangString("MENU_TEXT", "ERROR_DISPLAY_CLOUD"),
      });
    default: {
      const characterDisplay = createElement("character-display");
      const localInfo = getLocalInfoInSlot(slotID);
...
function showImportSaveFromStringForm(slotID) {
  return __awaiter(this, void 0, void 0, function* () {
    const userResponse = yield SwalLocale.fire({
      title: getLangString("CHARACTER_SELECT", "38"),
      html: `<h5 class="font-w400 text-combat-smoke font-size-sm">${getLangString(
        "CHARACTER_SELECT",
        "43"
      )}</h5>
					<div class="form-group">
						<textarea class="form-control" id="import-save-character-selection" name="import-save-character-selection" rows="8" placeholder="${getLangString(
              "CHARACTER_SELECT",
              "91"
            )}" onclick="this.select();"></textarea>
					</div>`,
      showCancelButton: true,
      confirmButtonText: getLangString("CHARACTER_SELECT", "44"),
    });
    if (!userResponse.value) return;
    const saveString = $("#import-save-character-selection").val();
    if (
      saveString.startsWith("https://melvoridle.com/save/") &&
      saveString.length < 100
    ) {
      ...
      if (importSuccess) {
        SwalLocale.fire({
          icon: "success",
          title: getLangString("CHARACTER_SELECT", "73"),
        ...
        toggleSaveSelectionView(0);
        inCharacterSelection = true;
      } else {
        SwalLocale.fire({
          icon: "error",
          title: getLangString("CHARACTER_SELECT", "93"),
          html: `<span class='text-dark'>${getLangString(
            "CHARACTER_SELECT",
            "94"
          )}</span>`,
    ...
    const userResponse = yield SwalLocale.fire({
      ...
      html: `<h5 class="font-w400 text-combat-smoke font-size-sm">${getLangString(
        ...
        "43"
					...
					<div class="form-group">
						<input type="text" class="form-control" id="import-save-link-character-selection" name="import-save-link-character-selection" rows="1" placeholder="${getLangString(
              "CHARACTER_SELECT",
              "91"
            )}"/>
					</div>`,
...
function createLocalSaveOnClick(slotID) {
  ...
  html.append(
    createElement("h5", {
      ...
      text: getLangString(
        "MENU_TEXT",
        "EXISTING_CLOUD_SAVE_POPUP_BODY_CREATING"
      ),
    }),
    createElement("h5", {
      className: "font-w600 text-danger mb-3 mt-3",
  ...
  SwalLocale.fire({
    title: templateString(
      getLangString("MENU_TEXT", "EXISTING_CLOUD_SAVE_POPUP_TITLE"),
      { number: `${slotID + 1}` }
    ),
    html,
    icon: "warning",
    ...
    if (result.value) displayGamemodeSelection(slotID);
  });
}
function createNewSave() {
  ...
  window.setTimeout(function () {
    ...
    if (startingCharacterName === "") {
      startingCharacterName = getLangString("CHARACTER_SELECT", "75");
    }
    createNewCharacterInSlot(
...
function createLatestDeathNotification() {
  ...
  if (latestHCDeath !== undefined && latestHCDeath !== null) {
    ...
    html += `<div class="mt-2 font-w600 font-size-sm text-center text-danger">${templateString(
      getLangString("MENU_TEXT", "LATEST_PERMADEATH"),
      {
...
function createToggleCharacterSelectionViewBtn() {
  let disabled = "";
  if (!PlayFabClientSDK.IsClientLoggedIn()) disabled = "disabled";
  const html = `<div class="btn-group mt-1 mb-1 w-100" role="group" aria-label="Horizontal Primary">
					<button type="button" class="btn btn-lg btn-alt-primary mt-2 mb-2 w-100 character-selection-toggle" onclick="toggleSaveSelectionView();" ${disabled}>${getLangString(
    "CHARACTER_SELECT",
    currentSaveView === 0 ? "33" : "34"
  )}</button>
					<button type="button" class="btn btn-lg btn-alt-info mt-2 mb-2 w-100" id="character-selection-toggle-refresh"${
            currentSaveView === 0 || !PlayFabClientSDK.IsClientLoggedIn()
              ? ' disabled=""'
              : ""
          } style="max-width:15%;" onclick="refreshCloudSavesOnClick();">
...
function createCharacterSelectSettings() {
  let checked = "checked";
  if (localStorageSettings.enableSaveOverwriteWarning == "false") checked = "";
  const html = `<div class="col-12 text-center">
				<div class="custom-control custom-switch mb-2">
					...
					<label class="custom-control-label pointer-enabled text-light" for="enableSaveOverwriteWarning">${getLangString(
            "MENU_TEXT",
            "ENABLE_OVERWRITE_SAVE_POPUP"
          )}</label>
				</div>
			</div>`;
  return html;
...
function toggleCharacteSelectWarningPopup() {
  if (
    localStorageSettings.enableSaveOverwriteWarning === undefined ||
...
function setNewStartPage(page) {
  $("#character-selection-page-" + currentStartPage).attr(
    "class",
    "d-none animated fadeOutRight"
  );
  $("#character-selection-page-" + page).attr(
    "class",
    "w-100 animated fadeInRight"
  );
...
function displayGamemodeSelection(slotID) {
  ...
  const gamemodeDisplayContainer =
  ...
  game.gamemodes.forEach((gamemode) => {
    ...
    if (gamemode.startDate !== undefined && gamemode.startDate > Date.now())
      return;
    if (gamemode.endDate > 0 && Date.now() > gamemode.endDate) return;
...
const setStartingGamemode = function (gamemode) {
  ...
  $("#gamemode-selection-text").html(
    `<img class="skill-icon-xs mr-2" src="${gamemode.media}">${gamemode.name}`
  );
  changePageCharacterSelection(4);
};
function importSaveOnClick(slotID) {
  if (cloudManager.getPlayFabSave(slotID) !== "") {
    const html = createElement("div");
    html.append(
      getLangString("MENU_TEXT", "EXISTING_CLOUD_SAVE_POPUP_BODY"),
      createElement("h5", {
        className: "font-w600 text-danger mb-3 mt-3",
        text: getLangString("MENU_TEXT", "WILL_OVERWRITE"),
      }),
    ...
    SwalLocale.fire({
      ...
      if (result.value) showImportSaveFromStringForm(slotID);
    });
  } else showImportSaveFromStringForm(slotID);
}
function createSaveShareLink(characterID) {
  SwalLocale.fire({
    title: getLangString("MENU_TEXT", "SAVE_LINK"),
  ...
  const save = getLocalSaveString(true, characterID);
  $.ajax({
    ...
    $("#saveLink").html(
      `${d}<i class="fa fa-copy ml-3 pointer-enabled" onClick="copyToClipboard('${d}')"></i>`
    );
  });
}
...
function openExportSave(slotID) {
  SwalLocale.fire({
    title: getLangString("CHARACTER_SELECT", "37"),
    html: `<h5 class="font-w400 text-combat-smoke font-size-sm mb-1">${getLangString(
      "CHARACTER_SELECT",
      "40"
    )}</h5>
				<h5 class="font-w600 text-danger font-size-sm">${getLangString(
          "CHARACTER_SELECT",
          "41"
        )}</h5>
					<div class="form-group">
						<textarea class="form-control" id="export-save-character-selection" name="export-save-character-selection" rows="8" onclick="this.select();">${getLocalSaveString(
              true,
              slotID
            )}</textarea>
					</div>`,
...
function confirmLocalSaveDeletion(slotID) {
  return __awaiter(this, void 0, void 0, function* () {
    const userResponse = yield SwalLocale.fire({
      title: getLangString("CHARACTER_SELECT", "39"),
    ...
    <h5 class="font-w600 text-danger font-size-sm">${getLangString(
      ...
      showCancelButton: true,
      confirmButtonText: getLangString("CHARACTER_SELECT", "47"),
    });
    if (!userResponse.value) return;
...
function confirmCloudSaveDeletion(slotID) {
  return __awaiter(this, void 0, void 0, function* () {
    const userResponse = yield SwalLocale.fire({
      title: getLangString("CHARACTER_SELECT", "39"),
      html: `<h5 class="font-w600 text-danger font-size-sm">${getLangString(
        "CHARACTER_SELECT",
        "46"
      )}</h5>`,
      showCancelButton: true,
