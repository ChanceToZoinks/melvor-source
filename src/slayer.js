...
class SlayerTask {
  constructor(player, game) {
    ...
    this.killsLeft = 0;
    this.extended = false;
    this.tier = SlayerTierID.Easy;
    this.completion = [0, 0, 0, 0, 0, 0, 0];
  ...
  postDataRegistration() {
    this.game.items.equipment.forEach((item) => {
      var _a, _b;
      if (
        ((_a = item.modifiers) === null || _a === void 0
          ? void 0
          : _a.bypassSlayerItems) !== undefined
      )
        this.areaBypassItems.push(item);
      if (
        ((_b = item.modifiers) === null || _b === void 0
          ? void 0
          : _b.bypassAllSlayerItems) !== undefined
      )
        this.allAreaBypassItems.push(item);
    });
  }
  render() {
    if (this.renderRequired) this.renderTask();
    if (this.renderNewButton) this.renderButtonSpinner();
  }
  jumpToTaskOnClick() {
    if (this.game.combat.isEventActive) {
      notifyPlayer(
        this.game.attack,
        getLangString("TOASTS", "CANNOT_DURING_EVENT"),
        "danger"
      );
    } else if (this.monster !== undefined) {
      this.game.combat.selectMonster(
        this.monster,
        this.game.getMonsterArea(this.monster)
      );
    } else {
      console.warn(
        "Tried to jump to slayer task enemy, but no task monster is set."
      );
    }
  }
  renderTask() {
    if (!this.active || this.monster === undefined) {
      combatMenus.slayerTask.setNoTask();
    } else {
      combatMenus.slayerTask.setTaskMonster(
        this.monster,
        this.killsLeft,
        this.tier
      );
      combatMenus.slayerTask.updateTaskExtension(
        this.extended,
        this.getExtensionCost()
      );
    }
  ...
  renderButtonSpinner() {
    ...
    this.renderNewButton = false;
  }
  extendTask() {
    const cost = this.getExtensionCost();
    if (this.game.slayerCoins.canAfford(cost)) {
      this.game.slayerCoins.remove(cost);
      this.extended = true;
      this.killsLeft +=
        (this.tier + 1) * (10 + Math.floor(this.game.slayer.level / 5));
      this.renderTask();
    } else {
      notifyPlayer(
        this.game.slayer,
  ...
  addKill() {
    ...
    if (this.killsLeft <= 0) {
      this.active = false;
      this.completion[this.tier]++;
      this.game.queueRequirementRenders();
      this.selectTask(this.tier, false, false);
    }
    this.renderRequired = true;
  }
  tick() {
  ...
  getExtensionCost() {
    return 100 * Math.pow(this.tier + 1, 3);
  }
  selectTask(tier, costsCoins, render, fromClick = false) {
  ...
  getTaskLength(tier) {
    const taskMultiplier =
      this.player.modifiers.increasedSlayerTaskLength -
      this.player.modifiers.decreasedSlayerTaskLength;
    let taskLength =
      (tier + 1) * 10 +
      4 * Math.floor(Math.random() * this.game.slayer.level + 1);
  ...
  setTask() {
    this.active = true;
    this.extended = false;
    this.killsLeft = this.getTaskLength(this.tier);
    this.renderRequired = true;
    this.renderNewButton = true;
    ...
    if (
      this.autoSlayer &&
      this.autoStartNext &&
      !this.game.combat.isEventActive
    )
      this.game.combat.selectMonster(
        this.monster,
        this.game.getMonsterArea(this.monster)
  ...
  getMonsterSelection(tier) {
    ...
    return this.game.monsters.filter((monster) => {
      ...
      return (
        monster.canSlayer &&
        combatLevel >= data.minLevel &&
        combatLevel <= data.maxLevel &&
        this.checkRequirements(
          monsterArea.entryRequirements,
          !this.autoSlayer,
          slayerLevelReq
        ) &&
  ...
  checkRequirements(requirements, softReq, slayerLevelReq) {
    return requirements.every((requirement) => {
      if (requirement.type === "SlayerItem" && softReq) {
        ...
        if (slayerLevelReq < 100) itemList.push(...this.areaBypassItems);
  ...
  getTaskCompletionsForTierAndAbove(tier) {
    let total = 0;
    for (let i = tier; i < this.completion.length; i++) {
      total += this.completion[i];
    }
    return total;
  }
  resetTaskState() {
  ...
  encode(writer) {
    ...
    if (this.monster) writer.writeNamespacedObject(this.monster);
    writer.writeUint32(this.killsLeft);
    writer.writeBoolean(this.extended);
    writer.writeUint8(this.tier);
    writer.writeArray(this.completion, (count, writer) =>
  ...
  decode(reader, version) {
    ...
    if (reader.getBoolean()) {
      ...
      if (typeof monster === "string") this.shouldResetTaskState = true;
      else this.monster = monster;
    }
    this.killsLeft = reader.getUint32();
    this.extended = reader.getBoolean();
    this.tier = reader.getUint8();
    this.completion = reader.getArray((reader) => reader.getUint32());
    this.taskTimer.decode(reader, version);
    if (this.shouldResetTaskState) this.resetTaskState();
  }
  deserialize(reader, version, idMap) {
    ...
    if (monster === undefined) this.shouldResetTaskState = true;
    else this.monster = monster;
    this.killsLeft = reader.getNumber();
    this.extended = reader.getBool();
    this.tier = reader.getNumber();
    const numTiers = version < 18 ? 5 : SlayerTask.data.length;
    this.completion = reader.getChunk(numTiers);
    if (this.shouldResetTaskState) this.resetTaskState();
  }
  convertFromOldSaveFormat(oldTasks, oldCompletion, idMap) {
    ...
    if (oldTasks.length !== 0) {
      ...
      const monster = this.game.monsters.getObjectByID(
        idMap.monsters[oldTask.monsterID]
      );
      if (monster !== undefined) {
        ...
        this.extended =
          (_a = oldTask.extended) !== null && _a !== void 0 ? _a : false;
        this.tier = (_b = oldTask.tier) !== null && _b !== void 0 ? _b : 0;
      }
    }
...
SlayerTask.data = [
  ...
  {
    get display() {
      return getLangString("COMBAT_MISC", "27");
    },
    engDisplay: "Master",
    cost: 25000,
    minLevel: 375,
  ...
  {
    get display() {
      return getLangString("COMBAT_MISC", "SLAYER_TIER_5");
    },
    engDisplay: "Legendary",
    cost: 50000,
    minLevel: 790,
    maxLevel: 999,
    slayerLevel: 100,
  ...
  {
    get display() {
      return getLangString("COMBAT_MISC", "SLAYER_TIER_6");
    },
    engDisplay: "Mythical",
    cost: 100000,
    minLevel: 1000,
...
class SlayerTaskMenuElement extends HTMLElement {
  constructor() {
    super();
    this.selectTaskButtons = [];
    this.taskSelectionOpen = false;
    this._content = new DocumentFragment();
    this._content.append(getTemplateNode("slayer-task-menu-template"));
    this.slayerIcon = getElementFromFragment(
      this._content,
      "slayer-icon",
      "img"
    );
    this.newTaskSpinner = getElementFromFragment(
      this._content,
      "new-task-spinner",
      "div"
    );
    this.newTaskButton = getElementFromFragment(
      this._content,
      "new-task-button",
      "a"
    );
    this.newTaskButtonText = getElementFromFragment(
      this._content,
      "new-task-button-text",
      "small"
    );
    this.selectTaskContainer = getElementFromFragment(
      this._content,
      "select-task-container",
      "div"
    );
    this.locatingContent = getElementFromFragment(
      this._content,
      "locating-content",
      "div"
    );
    this.monsterContainer = getElementFromFragment(
      this._content,
      "monster-container",
      "h5"
    );
    ...
    this.monsterLevel = getElementFromFragment(
      this._content,
      "monster-level",
      "span"
    );
    this.taskTier = getElementFromFragment(this._content, "task-tier", "small");
    this.monsterName = getElementFromFragment(
      this._content,
      "monster-name",
      "div"
    );
    this.jumpToEnemyButton = getElementFromFragment(
      this._content,
      "jump-to-enemy-button",
      "button"
    );
    this.extendContainer = getElementFromFragment(
      this._content,
      "extend-container",
      "h5"
    );
    this.extendMessage = getElementFromFragment(
      this._content,
      "extend-message",
      "span"
    );
    this.extendTaskButton = getElementFromFragment(
      this._content,
      "extend-task-button",
      "a"
    );
    this.extendTaskCost = getElementFromFragment(
      this._content,
      "extend-task-cost",
      "small"
    );
    this.autoSlayerCheckBox = getElementFromFragment(
      this._content,
      "auto-slayer-checkbox",
      "settings-checkbox"
    );
  }
  connectedCallback() {
    ...
    this.slayerIconTooltip = tippy(this.slayerIcon, {
      content: getLangString("SKILL_NAME", "Slayer"),
      placement: "bottom",
      interactive: false,
      animation: false,
    });
  ...
  initialize(game) {
    SlayerTask.data.forEach((taskData, tier) => {
      if (taskData.slayerLevel > game.slayer.levelCap) return;
      ...
      this.selectTaskButtons.push(taskButton);
      this.selectTaskContainer.append(taskButton);
    });
    this.jumpToEnemyButton.onclick = () =>
  ...
  setTaskMonster(monster, killsLeft, tier) {
    ...
    this.monsterLevel.textContent = `${monster.combatLevel}`;
    this.taskTier.textContent = SlayerTask.data[tier].display;
    this.monsterName.textContent = `${killsLeft} x ${monster.name}`;
  }
  updateTaskExtension(isExtended, extensionCost) {
    if (isExtended) {
      ...
      this.extendTaskCost.innerHTML = templateLangString("COMBAT_MISC", "29", {
        coinImage: `<img class="skill-icon-xxs mr-1" src="${cdnMedia(
          "assets/media/main/slayer_coins.svg"
        )}">`,
        value: numberWithCommas(extensionCost),
      });
    }
  }
  openTaskSelection() {
    this.taskSelectionOpen = true;
    this.newTaskButtonText.textContent = getLangString(
      "CHARACTER_SELECT",
      "45"
    );
  ...
  closeTaskSelection() {
    this.taskSelectionOpen = false;
    this.newTaskButtonText.textContent = getLangString("COMBAT_MISC", "22");
  ...
  updateTaskSelectButtons(game) {
    ...
    SlayerTask.data.forEach((data, tier) => {
      ...
      if (slayerLevel >= data.slayerLevel) {
        ...
        const rangeText = `${data.minLevel}${
          data.maxLevel === Infinity ? "+" : ` - ${data.maxLevel}`
        }`;
        const costText =
          data.cost === 0
            ? getLangString("COMBAT_MISC", "COST_FREE")
            : numberWithCommas(data.cost);
        button.append(
          document.createTextNode(data.display),
          combatImage,
          document.createTextNode(rangeText),
          coinImage,
          createElement("span", { classList: [costClass], text: costText })
        );
        if (game.slayerCoins.canAfford(data.cost)) {
        ...
        button
          .appendChild(createElement("span", { classList: ["text-danger"] }))
          .append(
